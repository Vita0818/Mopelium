...
# `agents/openai.yaml` in Mopelium

This file is optional. Mopelium recognizes the Codex-shaped top-level sections `interface`,
`dependencies`, and `policy`, but their runtime meaning is intentionally narrower.

## What Mopelium consumes

Mopelium validates and freezes only `dependencies.tools` records of type `mcp` for activation
...
this file. Keep `agents/openai.yaml` within Mopelium' separate 16 KiB metadata limit.

## Compatibility-only sections

`interface` and `policy` are accepted as bounded metadata so a Skill can remain
Codex-compatible, but Mopelium currently does not use them for a Skill picker, icons, default
prompts, or implicit-invocation policy. Do not promise those behaviors in an Mopelium Skill.
...
16 KiB boundaries, but intentionally does not duplicate Mopelium' strict Swift MCP schema parser.
...
mismatch rejects activation before any Skill body or resource is returned. Mopelium does not
