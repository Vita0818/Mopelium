---
name: mopelium-skill-creator
description: Create, adapt, validate, and iterate on Mopelium or Codex-compatible Agent Skills. Use when a user asks to add a Skill, improve an existing SKILL.md, organize Skill scripts or references, validate discovery compatibility, or design a reusable workflow for Code or Cowork.
---

<!-- Modified by Mopelium from the OpenAI Codex skill-creator sample.
See ThirdPartyNotices/OpenAICodexSkillCreator.md. -->

# Mopelium Skill Creator
...
- Remember that Mopelium freezes a fresh Skill snapshot for each Code send or Cowork
...
Mopelium currently freezes text resources. Do not make a Skill depend on an unreadable binary
...
```sh
python3 .agents/skills/mopelium-skill-creator/scripts/init_skill.py my-skill
```
...
dependencies that the Mopelium host can attest in the same request-owned MCP snapshot.
...
```sh
python3 .agents/skills/mopelium-skill-creator/scripts/quick_validate.py \
...
`agents/openai.yaml` limit. It does not reimplement Mopelium' strict Swift MCP metadata parser;
when MCP dependencies are present, the actual Mopelium loader is authoritative for their schema
