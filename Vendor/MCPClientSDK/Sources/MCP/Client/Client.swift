...
public actor Client {
    ...
    public struct Capabilities: Hashable, Codable, Sendable {
        ...
        /// 2025-11-25 experimental task-augmented client requests.
        public var tasks: MCPClientTaskCapabilities?
        ...
        public init(
            ...
            tasks: MCPClientTaskCapabilities? = nil
        ) {
    ...
    private var serverCapabilities: RemoteServerCapabilities?
    /// The server version
    ...
    public func connect(transport: any Transport) async throws -> Initialize.Result {
        try await connect(
            ...
            allowedProtocolVersions: Version.supported
        )
    }

    /// Connect and initialize using an exact per-server protocol ceiling.
    ///
    /// The response must be one of `allowedProtocolVersions`; validation occurs
    /// before `notifications/initialized` is sent.
    @discardableResult
    public func connect(
        transport: any Transport,
        requestedProtocolVersion: String,
        allowedProtocolVersions: Set<String>,
        validateInitializeResult:
            @escaping @Sendable (Initialize.Result) async throws -> Void = {
                _ in
            }
    ) async throws -> Initialize.Result {
        guard allowedProtocolVersions.contains(requestedProtocolVersion) else {
            throw MCPError.invalidParams(
                "Requested protocol version is outside the allowed profile"
            )
        }
        ...
        do {
            return try await _initialize(
                ...
                validateInitializeResult: validateInitializeResult
            )
        } catch {
            await disconnect()
            throw error
    ...
    private func _initialize(
        ...
        try await validateInitializeResult(result)

        guard allowedProtocolVersions.contains(result.protocolVersion) else {
            throw MCPError.invalidRequest(
                "Remote server selected protocol version outside the allowed profile"
            )
        }
        ...
        {
            try await versionedTransport.updateNegotiatedProtocolVersion(
    ...
    private func validateServerCapability<T>(
        _ keyPath: KeyPath<RemoteServerCapabilities, T?>,
        _ name: String
    )
        throws
    {
