# Mopelium SwiftStreamingMarkdown Vendored Patch Ledger
...
This ledger describes the Mopelium-maintained vendored derivative of Microsoft
SwiftStreamingMarkdown. The vendored snapshot lives at
`Vendor/SwiftStreamingMarkdown` and is versioned by the containing Mopelium Git
revision. It is third-party-derived code, not independently authored Mopelium
renderer code.
...
- Reserve the public `@concurrent` transfer boundary for Mopelium' external
  parse scheduler. Mopelium must own the result on `@MainActor` and retain a
...
- Keep any still-compiled legacy image/citation compatibility source out of the
  Mopelium production parse profile; it is not a first-release capability.
...
- `Packages/MopeliumSharedUI/Tests/MessageRenderingTests.swift`
...
- Track the last valid laid-out paragraph width independently from the
  ...
  contract.
- Remove the Mopelium facade's whole-rich-document
...
- Run the full retained Release suite under strict concurrency and warnings as
  errors on macOS.
- Add Mopelium-level golden rendering, streaming replacement, selection/copy,
...
- containing Mopelium revision:
  `Packages/MopeliumSharedUI/Tests/MessageRenderingTests.swift`
- containing Mopelium revision:
  `Apps/MopeliumMac/Sources/RendererFixtureView.swift`
- containing Mopelium revision:
...
- Host accepted formulas through a live TextKit 2
  ...
  Mopelium' display revision so an old-size attachment cannot overwrite a
  newer request.
...
- Treat iosMath's eight unmodified OpenType fonts and accompanying math-table,
  ...
  They do not replace or determine Mopelium' product-interface font.
...
- The Mopelium root and derivative `Package.resolved` files currently agree on
...
- The interactive Computer Use soak recorded 18 AppKit negative-geometry
  issues in the Mopelium PID. Each cluster immediately followed system
...
- containing Mopelium revision:
  `Packages/MopeliumSharedUI/Tests/MessageRenderingTests.swift`
- containing Mopelium revision:
  `Apps/MopeliumMac/Sources/{MopeliumChatScreen,RendererFixtureView}.swift`
- containing Mopelium revision:
...
- Mopelium SharedUI's syntax-agnostic whole-message rich-admission boundary and
...
- Root `MessageRenderingTests` passed 41/41; `swift build --disable-sandbox
  --target MopeliumSharedUI`, `xcodegen generate`, MopeliumMac macOS Debug, and
  MopeliumiOS generic Simulator Debug all succeeded.
...
`Mopelium Renderer Validation` instances were accidentally allowed to coexist.
...
- MopeliumMac Debug/Release and MopeliumiOS generic Simulator Debug/Release
...
The Mopelium root revision is the immutable identity for a distributed source
...
notices and provenance, rerun the full Mopelium cutover suite, and delete the
