...
private final class ViewportAdmissionFixtureModel: ObservableObject {
    @Published var admission: MopeliumMessageViewportAdmission

    init(admission: MopeliumMessageViewportAdmission) {
...
private struct StableThreadWindowHostingFixture: View {
    @ObservedObject var coordinator: MopeliumThreadScrollCoordinator
    let rowCount: Int
    private let scope = MopeliumThreadPresentationScope(
        kind: "cowork",
        sessionID: "stable-window")
        .historyWindowScope(requestedUpperBound: nil)

    init(
        coordinator: MopeliumThreadScrollCoordinator,
        rowCount: Int = 13
    ) {
        ...
        ScrollViewReader { proxy in
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Color.clear
                        .frame(height: 1)
                        .id(MopeliumThreadBottomAnchorID(scope: scope))
            ...
            .onAppear {
                ...
                coordinator.request(
                    ...
                    proxy.scrollTo(
                        MopeliumThreadBottomAnchorID(scope: request.scope),
                        anchor: .bottom)
                }
            }
        }
...
private func waitForPublishedMarkdown(
    _ state: MopeliumMicrosoftMarkdownRenderState,
    revision: MopeliumMarkdownRenderRevision,
    attempts: Int = 4_000
) async throws -> MopeliumMicrosoftMarkdownRenderState.PublishedDocument {
    for _ in 0..<attempts {
...
private func waitForPublishedMarkdown(
    _ state: MopeliumMicrosoftMarkdownRenderState,
    request: MopeliumMarkdownRenderRequest,
    attempts: Int = 4_000
) async throws -> MopeliumMicrosoftMarkdownRenderState.PublishedDocument {
    for _ in 0..<attempts {
...
final class MessageRenderingTests: XCTestCase {
    ...
    private func rawRevision(
        ...
        lane: MopeliumRawTextProjectionLane = .plain,
        text: String,
    ...
    private func renderRequest(
        ...
        appearance: MopeliumMarkdownAppearanceRevision = .light,
    ...
    func testRichFacadeDoesNotWrapDocumentInASecondSelectionOverlay() throws {
        ...
        let source = try String(
            contentsOf: packageRoot.appendingPathComponent(
                "Sources/MessageRendering/MopeliumMessageContentView.swift"),
            encoding: .utf8)
        ...
        let plainEnd = try XCTUnwrap(
            source.range(
                of: ".accessibilityIdentifier(\"mopelium.message.plain.",
    ...
    func testProjectionLifecycleGateDeduplicatesAndRejectsLateInvisibleInput() {
        ...
        let rawState = MopeliumRawTextProjectionState(revision: initial)
        let richState = MopeliumMicrosoftMarkdownRenderState()
        let gate = MopeliumMessageProjectionLifecycleGate()
        let first = MopeliumMessageProjectionInput(
        ...
        let late = MopeliumMessageProjectionInput(
    ...
    func testProjectionLifecycleGateRichToPlainSwitchDropsPublishedDocument() async throws {
        ...
        let richInput = MopeliumMessageProjectionInput(
            rawRevision: initial,
        ...
        let plainInput = MopeliumMessageProjectionInput(
    ...
    func testViewportInteractionBlocksAdmissionUntilLatestExactDwell() {
        ...
        let request = renderRequest(
            ...
            text: "initial",
            isComplete: false)
        let suspended = MopeliumMessageProjectionInput(
            rawRevision: initial,
            richRequest: request,
            usesRichRenderer: true,
        ...
        let dwell = MopeliumMessageProjectionInput(
            rawRevision: initial,
            richRequest: request,
            usesRichRenderer: true,
    ...
    func testThirteenRowStableWindowKeepsFiveRichRowsRawUntilExactDwell() {
        let window = MopeliumThreadHistoryWindow.resolve(
            allItems: Array(0..<13),
        ...
        var rows: [(
            gate: MopeliumMessageProjectionLifecycleGate,
            raw: MopeliumRawTextProjectionState,
            rich: MopeliumMicrosoftMarkdownRenderState,
            input: MopeliumMessageProjectionInput
        )] = []
        ...
        for index in 0..<5 {
            ...
            let revision = rawRevision(
                ...
                text: text,
                isComplete: true)
            let rawState = MopeliumRawTextProjectionState(revision: revision)
            let richState = MopeliumMicrosoftMarkdownRenderState()
            let gate = MopeliumMessageProjectionLifecycleGate()
            let input = MopeliumMessageProjectionInput(
                rawRevision: revision,
                richRequest: renderRequest(
                    messageID: "entry-\(index)",
                    text: text),
        ...
        for row in rows {
            ...
            let dwellInput = MopeliumMessageProjectionInput(
    ...
    func testViewportDwellRejectsSupersededRevision() {
        ...
        let latestRevision = rawRevision(
            ...
            text: "first latest",
            isComplete: false)
        let latest = MopeliumMessageProjectionInput(
    ...
    func testSuspensionRetainsOnlyExactPublishedDocument() async throws {
        ...
        let exactRequest = renderRequest(
            ...
            text: "# exact")
        let immediate = MopeliumMessageProjectionInput(
            rawRevision: initial,
        ...
        let suspendedExact = MopeliumMessageProjectionInput(
            rawRevision: initial,
            richRequest: exactRequest,
            usesRichRenderer: true,
        ...
        let replacementRevision = rawRevision(
            ...
            text: "# replacement",
            isComplete: true)
        let suspendedReplacement = MopeliumMessageProjectionInput(
    ...
    func testNSHostingStableWindowRestoresRawBeforeMountingRichRows() async throws {
        let previousMode = UserDefaults.standard.string(
            forKey: MopeliumMessageRendererMode.defaultsKey)
        UserDefaults.standard.set(
            MopeliumMessageRendererMode.microsoft.rawValue,
            forKey: MopeliumMessageRendererMode.defaultsKey)
        defer {
            if let previousMode {
                UserDefaults.standard.set(
    ...
    func testStyleOnlyRequestChangeSupersedesOldParseAndDuplicateIsNoOp() async throws {
        ...
        let revision = MopeliumMarkdownRenderRevision(
            ...
            typography: .large,
            configurationRevision: MopeliumMarkdownRendererLimits.configurationRevision)
        let first = MopeliumMarkdownRenderRequest(
            revision: revision,
            style: MopeliumMarkdownStyleSnapshot(.standard(.light)))
        let replacementStyle = MopeliumThreadStyle(
            primaryText: .red,
            secondaryText: .green,
            tertiaryText: .blue,
            accent: .orange,
            ...
            cardStroke: .yellow,
            error: .pink)
        let replacement = MopeliumMarkdownRenderRequest(
            revision: revision,
            style: MopeliumMarkdownStyleSnapshot(replacementStyle))
        XCTAssertNotEqual(first, replacement)

        let state = MopeliumMicrosoftMarkdownRenderState()
    ...
    func testTypographyOnlyRequestChangeSupersedesOldParseAndScalesConfiguration() async throws {
        ...
        XCTAssertEqual(
            MopeliumMarkdownTypographyRevision(DynamicTypeSize.large),
            .large)
        XCTAssertEqual(
            MopeliumMarkdownTypographyRevision(DynamicTypeSize.accessibility3),
            .accessibility3)

        let baseline = MopeliumMicrosoftMarkdownRenderState.makeConfiguration(
            style: .standard(.light),
            typography: .large)
        let scaled = MopeliumMicrosoftMarkdownRenderState.makeConfiguration(
            style: .standard(.light),
            typography: .accessibility3)
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            ...
            baseline.paragraphStyle.textFonts.normal.pointSize
                * MopeliumMarkdownTypographyRevision.accessibility3.scale,
            accuracy: 0.001)
        XCTAssertEqual(
    ...
    func testRawProjectionUsesLeadingTrailingThrottleWithoutResettingDeadline() throws {
        var model = MopeliumRawTextProjectionModel(
    ...
    func testRawStateFirstFrameBypassesThrottleForSemanticChangesAndReentry() {
        let initial = rawRevision(
            lane: .richFallback,
            text: "stream")
        let state = MopeliumRawTextProjectionState(revision: initial)
    ...
    func testWholeMessageAdmissionIsSyntaxAgnosticAndUTF8Bounded() {
        let exact = MopeliumMarkdownRenderRevision(
            messageID: "exact",
        ...
        let oversized = MopeliumMarkdownRenderRevision(
            messageID: "oversized",
            ...
            typography: .large,
            configurationRevision: 1)
        let multiByteOversized = MopeliumMarkdownRenderRevision(
            messageID: "multibyte",
            ...
            typography: .large,
            configurationRevision: 1)
        let empty = MopeliumMarkdownRenderRevision(
            messageID: "empty",
            rawText: "",
            isComplete: false,
            appearance: .light,
    ...
    func testAdaptiveThreadStackRetainsItsNonProductionCompatibilityPolicy() {
        XCTAssertEqual(
            MopeliumThreadStackLayoutMode.resolve(visibleRowCount: 2),
            .eager)
        XCTAssertEqual(
            MopeliumThreadStackLayoutMode.resolve(visibleRowCount: 4),
            .eager)
        XCTAssertEqual(
            MopeliumThreadStackLayoutMode.resolve(visibleRowCount: 5),
            .lazy)
    }

    func testThreadHistoryWindowKeepsThirteenRowsInOneLatestPage() {
        XCTAssertEqual(MopeliumThreadHistoryWindowPolicy.capacity, 16)
    ...
    func testThreadHistoryWindowEarlierAndNewerUseStablePageBoundaries() {
        ...
        let middle = MopeliumThreadHistoryWindow.resolve(
            allItems: items,
        ...
        let oldest = MopeliumThreadHistoryWindow.resolve(
            allItems: items,
        ...
        let newer = MopeliumThreadHistoryWindow.resolve(
            allItems: items,
    ...
    func testThreadHistoryWindowExplicitOlderPageDoesNotDriftOnAppend() {
        let before = MopeliumThreadHistoryWindow.resolve(
            allItems: Array(0..<40),
            requestedUpperBound: 24)
        let after = MopeliumThreadHistoryWindow.resolve(
            allItems: Array(0..<41),
            requestedUpperBound: 24)
    ...
    func testThreadHistoryWindowLatestPageFollowsAppendWithoutExceedingCapacity() {
        ...
        for total in totals {
            ...
            XCTAssertLessThanOrEqual(
                window.items.count,
                MopeliumThreadHistoryWindowPolicy.capacity)
    ...
    func testThreadHistoryWindowScopesAreStableAndIsolatedByPage() {
        let base = MopeliumThreadPresentationScope(
            kind: "cowork",
            sessionID: "scope")
    ...
    func testThreadHistorySelectionCannotLeakAcrossSessions() {
        let first = MopeliumThreadPresentationScope(
            kind: "cowork",
            sessionID: "first")
        let second = MopeliumThreadPresentationScope(
            kind: "cowork",
            sessionID: "second")
        let selection = MopeliumThreadHistorySelection(
            scope: first,
            requestedUpperBound: 24)
    ...
    func testFirstReleaseConfigurationDisablesOptionalUnboundedFeatures() {
        ...
        XCTAssertEqual(
            ...
            MopeliumMarkdownRendererLimits.mathMode.renderConfig.mode)
    ...
    func testLatexMathIsDefaultAndHasAnIndependentLaunchKillSwitch() {
        XCTAssertEqual(
            MopeliumMarkdownMathMode.resolve(arguments: ["Mopelium"]),
            .latex)
        XCTAssertEqual(
            MopeliumMarkdownMathMode.resolve(arguments: [
                "Mopelium",
                MopeliumMarkdownRendererLimits.disableSingleDollarMathLaunchArgument,
            ]),
            .disabled)
        XCTAssertEqual(
            MopeliumMarkdownMathMode.latex.renderConfig.mode,
            .latex)
        XCTAssertEqual(
            MopeliumMarkdownMathMode.disabled.renderConfig.mode,
            .disabled)
    }

    func testLinkPolicyAllowsOnlyProductSchemes() throws {
        XCTAssertTrue(MopeliumMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "https://example.com"))))
        XCTAssertTrue(MopeliumMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "http://example.com"))))
        XCTAssertTrue(MopeliumMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "mailto:test@example.com"))))
        XCTAssertFalse(MopeliumMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "file:///tmp/message"))))
        XCTAssertFalse(MopeliumMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "data:text/plain,hello"))))
        XCTAssertFalse(MopeliumMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "javascript:alert(1)"))))
        XCTAssertFalse(MopeliumMarkdownLinkPolicy.allows(try XCTUnwrap(URL(string: "relative/path"))))
    }
    ...
    func testSingleConsumerKeepsOnlyTheLatestStreamingSnapshot() async throws {
        ...
        let finalRevision = MopeliumMarkdownRenderRevision(
            messageID: messageID,
            rawText: finalText,
            isComplete: true,
            appearance: .dark,
    ...
    func testSanitizedIncidentReplaysAll1249DeltasInOriginalOrder() async throws {
        ...
        var states: [MopeliumMicrosoftMarkdownRenderState] = []
        var expectedRevisions: [MopeliumMarkdownRenderRevision] = []
        var yieldedDeltas = 0
