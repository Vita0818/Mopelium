...
private func waitForMarkdownSchedulerSnapshot<Key: Hashable & Sendable>(
    _ scheduler: MopeliumLatestOnlyPermitScheduler<Key>,
    attempts: Int = 2_000,
    matching predicate: @Sendable (MopeliumLatestOnlyPermitSchedulerSnapshot<Key>) -> Bool
) async throws -> MopeliumLatestOnlyPermitSchedulerSnapshot<Key> {
    for _ in 0..<attempts {
...
final class MarkdownSchedulerTests: XCTestCase {
    func testRejectsNonPositiveBounds() {
        XCTAssertThrowsError(try MopeliumLatestOnlyPermitScheduler<String>(
            maxConcurrentPermits: 0,
            maxPendingAcquires: 1
        )) { error in
            XCTAssertEqual(
                error as? MopeliumLatestOnlyPermitSchedulerConfigurationError,
    ...
    func testGlobalConcurrencyAndPendingCapacityStayBounded() async throws {
        let scheduler = try MopeliumLatestOnlyPermitScheduler<Int>(
    ...
    func testSameKeyKeepsOneRunningAndOneReplaceablePending() async throws {
        let scheduler = try MopeliumLatestOnlyPermitScheduler<String>(
            maxConcurrentPermits: 1,
            maxPendingAcquires: 1)
