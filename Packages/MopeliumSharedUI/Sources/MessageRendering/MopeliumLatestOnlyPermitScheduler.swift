...
struct MopeliumLatestOnlySubmissionID: Hashable, Comparable, RawRepresentable, Sendable {
...
struct MopeliumLatestOnlyPermit<Key: Hashable & Sendable>: Sendable {
  public let key: Key
  public let submissionID: MopeliumLatestOnlySubmissionID

  fileprivate let schedulerID: UUID
}

/// The caller-reported reason a permit's real work ended.
enum MopeliumLatestOnlyFinishOutcome: Sendable {
  case success
  case cancelled
  case failed
}
...
struct MopeliumLatestOnlyPermitSchedulerSnapshot<Key: Hashable & Sendable>: Sendable {
...
enum MopeliumLatestOnlyPermitSchedulerConfigurationError: Error, Equatable, Sendable {
...
actor MopeliumLatestOnlyPermitScheduler<Key: Hashable & Sendable> {
  public typealias Permit = MopeliumLatestOnlyPermit<Key>
  public typealias Snapshot = MopeliumLatestOnlyPermitSchedulerSnapshot<Key>

  private struct PendingAcquire: Sendable {
    let submissionID: MopeliumLatestOnlySubmissionID
  ...
  private var latestSubmissionByKey: [Key: MopeliumLatestOnlySubmissionID] = [:]
  ...
  public init(
    ...
    guard maxConcurrentPermits > 0 else {
      throw MopeliumLatestOnlyPermitSchedulerConfigurationError.nonPositiveConcurrentMaximum(
        maxConcurrentPermits
      )
    }
    guard maxPendingAcquires > 0 else {
      throw MopeliumLatestOnlyPermitSchedulerConfigurationError.nonPositivePendingMaximum(
        maxPendingAcquires
      )
    }
  ...
  public func finish(
    _ permit: Permit,
    outcome: MopeliumLatestOnlyFinishOutcome = .success
  ) -> Bool {
    finishInternal(
      permit,
      outcome: outcome,
  ...
  private func makeSubmissionID() -> MopeliumLatestOnlySubmissionID {
    precondition(
      nextSubmissionRawValue < UInt64.max,
      "MopeliumLatestOnlySubmissionID space exhausted"
    )
    nextSubmissionRawValue += 1
    return MopeliumLatestOnlySubmissionID(rawValue: nextSubmissionRawValue)
  }

  private func suspendAcquire(
    for key: Key,
    submissionID: MopeliumLatestOnlySubmissionID
  ) async -> Permit? {
    await withCheckedContinuation { continuation in
      registerAcquire(
        for: key,
  ...
  private func finishInternal(
    _ permit: Permit,
    outcome: MopeliumLatestOnlyFinishOutcome,
