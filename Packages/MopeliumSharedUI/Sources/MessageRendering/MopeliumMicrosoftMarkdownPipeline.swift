...
private enum MopeliumMarkdownDiagnosticFlag {
...
private extension MopeliumMarkdownRenderRevision {
    ...
    var diagnosticFlags: Int64 {
        var result: Int64 = 0
        if isComplete {
            result |= MopeliumMarkdownDiagnosticFlag.complete
        }
        if isAdmitted {
            result |= MopeliumMarkdownDiagnosticFlag.admitted
        }
        return result
    }
}

private func mopeliumElapsedMilliseconds(since startNanoseconds: UInt64) -> Int64 {
    let now = DispatchTime.now().uptimeNanoseconds
...
final class MopeliumMicrosoftMarkdownRenderState: ObservableObject {
    ...
    var currentRequestSnapshot: MopeliumMarkdownRenderRequest? {
        currentRequest
    }
    ...
    /// document exists.
    func suspend(keepingExact request: MopeliumMarkdownRenderRequest) {
        suspensionCount += 1
    ...
    private func process(
        ...
        guard let permit = await Self.scheduler.acquire(for: schedulerKey) else {
            performanceDiagnostics.endInterval(
                ...
                fields: .init(
                    primary: 0,
                    secondary: mopeliumElapsedMilliseconds(
                        since: queueStarted),
                    sequence: request.revision.diagnosticSequence,
                    flags: request.revision.diagnosticFlags))
            return
        ...
        performanceDiagnostics.endInterval(
            ...
            fields: .init(
                ...
                sequence: request.revision.diagnosticSequence,
                flags: request.revision.diagnosticFlags
                    | MopeliumMarkdownDiagnosticFlag.schedulerPermitIssued))
        ...
        performanceDiagnostics.endInterval(
            ...
            fields: .init(
                ...
                flags: request.revision.diagnosticFlags
                    | (isStillCurrent
                        ? MopeliumMarkdownDiagnosticFlag.currentAfterParse
                        : 0)))
        ...
        performanceDiagnostics.endInterval(
            ...
            fields: .init(
                ...
                sequence: request.revision.diagnosticSequence,
                flags: request.revision.diagnosticFlags
                    | MopeliumMarkdownDiagnosticFlag.published))
    }
