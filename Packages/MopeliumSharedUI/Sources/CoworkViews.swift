...
public struct CoworkShell: View {
    ...
    public init(items: [CodeItem],
        ...
        self.displayedItems = MopeliumExecutionTracePresentation.displayedItems(items)
    ...
    private func content(rawWidth: CGFloat) -> some View {
        ...
        return threadColumn(
            layout: MopeliumThreadContentLayout(rawWidth: threadWidth),
    ...
    private func header(layout: MopeliumThreadContentLayout, showsCompactActions: Bool) -> some View {
        MopeliumWorkspaceThreadHeader(
            title: sessionTitle,
            subtitle: MopeliumLocalization.format(
    ...
    private func headerActions(showsCompactActions: Bool) -> [MopeliumThreadHeaderAction] {
        var actions: [MopeliumThreadHeaderAction] = []
    ...
    private var primaryWorkspaceName: String {
        primaryWorkspace?.displayName ?? MopeliumLocalization.string("No workspace")
    }

    private var primaryWorkspacePath: String {
        primaryWorkspace?.path
            ?? MopeliumLocalization.string("Attach a workspace to inspect git state.")
    }
                ...
                ScrollView {
                    MopeliumAdaptiveThreadStack(
                        ...
                        Color.clear
                            .frame(height: 1)
                            .padding(.bottom, 16)
                            .id(MopeliumThreadBottomAnchorID(scope: presentationScope))
                    }
    ...
    private func requestScroll(
        ...
        let perform: @MainActor (MopeliumThreadScrollRequest) -> Void = { request in
