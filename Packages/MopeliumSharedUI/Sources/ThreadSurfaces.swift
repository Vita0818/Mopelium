...
public enum MopeliumMessageHeaderPolicy {
...
struct MopeliumThreadErrorEntry: Identifiable, Equatable, Sendable {
    let id: String
    var title: String?
    var details: [String]
...
enum MopeliumThreadErrorPresentation {
    private struct Candidate {
        let fingerprint: String
        let entry: MopeliumThreadErrorEntry
    }

    static func errors(
        items: [CodeItem],
        errorTexts: [String]
    ) -> [MopeliumThreadErrorEntry] {
        let latestSubmissionID = items.reversed().first(where: {
            $0.kind == .user && $0.submissionID != nil
        })?.submissionID
        var orderedFingerprints: [String] = []
        var entriesByFingerprint: [String: MopeliumThreadErrorEntry] = [:]
        ...
        if item.kind == .user,
           item.submissionStatus == .failed || item.submissionFailure != nil {
            ...
            return makeCandidate(
                ...
                title: MopeliumLocalization.string("Needs attention"),
                primaryDetail: failure?.message
                    ?? advice?.detail
                    ?? MopeliumLocalization.string("Needs attention"),
    ...
    private static func makeCandidate(
        ...
        return Candidate(
            ...
            entry: MopeliumThreadErrorEntry(
                id: id,
                title: cleanTitle,
                details: details,
...
struct MopeliumThreadErrorList: View {
    ...
    private func errorRow(_ error: MopeliumThreadErrorEntry) -> some View {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    ...
                    if let submissionID = error.retrySubmissionID,
                       let onRetrySubmission {
                        Button(MopeliumLocalization.string("Retry")) {
...
enum MopeliumThreadScrollReason: Equatable, Sendable {
    ...
    var diagnosticReason: MopeliumScrollDiagnosticReason {
        switch self {
        case .initialRestore:
...
struct MopeliumThreadScrollGeometry: Equatable, Sendable {
    ...
    static func == (
        lhs: MopeliumThreadScrollGeometry,
        rhs: MopeliumThreadScrollGeometry
    ) -> Bool {
...
enum MopeliumThreadFollowState: Equatable, Sendable {
...
public enum MopeliumMessageViewportAdmission: Equatable, Sendable {
    case immediate
...
private struct MopeliumMessageViewportAdmissionEnvironmentKey: EnvironmentKey {
    static let defaultValue = MopeliumMessageViewportAdmission.immediate
}

extension EnvironmentValues {
    public var mopeliumMessageViewportAdmission:
        MopeliumMessageViewportAdmission {
        get { self[MopeliumMessageViewportAdmissionEnvironmentKey.self] }
        set { self[MopeliumMessageViewportAdmissionEnvironmentKey.self] = newValue }
    }
}

enum MopeliumThreadRichSettleToken: Hashable, Sendable {
    case finalDocument(
        messageID: String,
...
struct MopeliumThreadRichSettleSource: Equatable, Sendable {
    let scope: MopeliumThreadPresentationScope
    let activationGeneration: UInt64
}

private struct MopeliumThreadScrollCoordinatorEnvironmentKey: EnvironmentKey {
    static let defaultValue: MopeliumThreadScrollCoordinator? = nil
}

private struct MopeliumThreadRichSettleSourceEnvironmentKey: EnvironmentKey {
    static let defaultValue: MopeliumThreadRichSettleSource? = nil
}

extension EnvironmentValues {
    var mopeliumThreadScrollCoordinator: MopeliumThreadScrollCoordinator? {
        get { self[MopeliumThreadScrollCoordinatorEnvironmentKey.self] }
        set { self[MopeliumThreadScrollCoordinatorEnvironmentKey.self] = newValue }
    }

    var mopeliumThreadRichSettleSource: MopeliumThreadRichSettleSource? {
        get { self[MopeliumThreadRichSettleSourceEnvironmentKey.self] }
        set { self[MopeliumThreadRichSettleSourceEnvironmentKey.self] = newValue }
    }
}
...
struct MopeliumThreadScrollCadenceModel: Sendable {
    ...
    private(set) var executorRequest: MopeliumThreadScrollRequest?
    ...
    var outstandingRequests: [MopeliumThreadScrollRequest] {
        [executorRequest, pendingRequest].compactMap { $0 }
    }

    mutating func submit(
        _ request: MopeliumThreadScrollRequest,
...
struct MopeliumThreadRichSettleModel: Sendable {
    ...
    struct Epoch: Equatable, Sendable {
        let token: MopeliumThreadRichSettleToken
    ...
    enum Check: Equatable, Sendable {
        ...
        case settled(token: MopeliumThreadRichSettleToken)
    }

    private(set) var epoch: Epoch?
    private(set) var lastOpenedToken: MopeliumThreadRichSettleToken?
...
final class MopeliumThreadScrollCoordinator: ObservableObject {
    ...
    @Published private(set) var followState: MopeliumThreadFollowState =
        .followingBottom
    @Published private(set) var viewportAdmission:
        MopeliumMessageViewportAdmission = .suspended(generation: 0)
    @Published private(set) var richSettleSource:
        MopeliumThreadRichSettleSource? = nil

    private var pendingTask: Task<Void, Never>?
    private var pendingWakeDeadlineNanoseconds: UInt64?
    private var pendingWakeToken: UInt64 = 0
    private var cadence = MopeliumThreadScrollCadenceModel()
    ...
    private var richSettle = MopeliumThreadRichSettleModel()
    private var richSettleTask: Task<Void, Never>?
    private var richSettleTaskToken: UInt64 = 0
    private let performanceDiagnostics: MopeliumPerformanceDiagnostics

    init(
        performanceDiagnostics: MopeliumPerformanceDiagnostics = .shared
    ) {
    ...
    func effectiveViewportAdmission(
        for scope: MopeliumThreadPresentationScope,
    ...
    func activate(
        ...
        richSettleSource = MopeliumThreadRichSettleSource(
            scope: scope,
    ...
    func jumpToLatest(
        ...
        guard activeScope == scope else {
            ...
            performanceDiagnostics.recordScrollRequest(
                reason: MopeliumThreadScrollReason.jumpToLatest.diagnosticReason,
                outcome: .stale,
    ...
    func richDocumentDidCommit(
        token: MopeliumThreadRichSettleToken,
        source: MopeliumThreadRichSettleSource
    ) {
    ...
    private func synchronizeRichSettleWake(
        _ check: MopeliumThreadRichSettleModel.Check
    ) {
        switch check {
        case .inactive:
            return
...
public struct MopeliumJumpToLatestButton: View {
    ...
    public var body: some View {
        Button(action: action) {
            Label(
                MopeliumLocalization.string("Jump to latest"),
...
public enum MopeliumThreadHistoryWindowPolicy {
    public static let capacity = 16
}

enum MopeliumThreadRichEntryPolicy {
...
struct MopeliumThreadHistorySelection: Equatable {
    let scope: MopeliumThreadPresentationScope
    let requestedUpperBound: Int?

    func upperBound(
        for currentScope: MopeliumThreadPresentationScope
    ) -> Int? {
        scope == currentScope ? requestedUpperBound : nil
    }
}

public struct MopeliumThreadHistoryWindow<Element> {
    ...
    public static func resolve(
        allItems: [Element],
        requestedUpperBound: Int?,
        capacity: Int = MopeliumThreadHistoryWindowPolicy.capacity
    ) -> Self {
...
public struct MopeliumThreadHistoryPager: View {
        ...
        HStack(spacing: 8) {
            if hasEarlier {
                Button(action: onEarlier) {
                    Label(
                        MopeliumLocalization.string("Earlier"),
            ...
            if hasLater {
                Button(action: onNewer) {
                    Label(
                        MopeliumLocalization.string("Newer"),
                ...
                Button(action: onLatest) {
                    Label(
                        MopeliumLocalization.string("Latest"),
                        systemImage: "arrow.down.to.line")
                }
...
private struct MopeliumClearLiquidGlassBackdrop: View, Equatable {
    ...
    static func == (
        lhs: MopeliumClearLiquidGlassBackdrop,
        rhs: MopeliumClearLiquidGlassBackdrop
    ) -> Bool {
...
public extension View {
    ...
    func mopeliumSubtleContentSurface(cornerRadius: CGFloat = 12) -> some View {
        modifier(MopeliumSubtleContentSurfaceModifier(cornerRadius: cornerRadius))
    }
    ...
    func mopeliumClearLiquidGlass(cornerRadius: CGFloat = 16,
                                 interactive: Bool = false) -> some View {
        modifier(MopeliumLiquidGlassModifier(
            cornerRadius: cornerRadius,
            isInteractive: interactive,
    ...
    @ViewBuilder func mopeliumComposerIconButton(
        prominent: Bool = false
    ) -> some View {
        #if os(iOS)
        labelStyle(.iconOnly)
            .controlSize(MopeliumComposerControlMetrics.iconControlSize(
                for: .iOS))
            .buttonBorderShape(.circle)
            .mopeliumGlassButton(prominent: prominent)
            .frame(
                width: MopeliumComposerControlMetrics.controlHeight,
                height: MopeliumComposerControlMetrics.controlHeight)
            .contentShape(Circle())
        #else
        mopeliumCompactIconButton(prominent: prominent)
            .frame(
                width: MopeliumComposerControlMetrics.controlHeight,
                height: MopeliumComposerControlMetrics.controlHeight)
            .contentShape(Circle())
...
public enum MopeliumComposerControlMetrics {
    ...
    static func glassEffectSpacing(
        for platform: MopeliumComposerGlassPlatform
    ) -> CGFloat {
        switch platform {
        case .iOS:
            return 0
...
enum MopeliumComposerGlassPlatform: Sendable {
    case iOS
    case macOS
}

public enum MopeliumSidebarGesturePolicy {
...
public struct MopeliumThreadComposer: View {
    ...
    private let trailingAction: MopeliumThreadComposerSecondaryAction?
    ...
    private var inputPointSize: CGFloat = MopeliumTypography.spec(for: .chat).nominalPointSize

    private var inputFont: Font {
        MopeliumTypography.chat(inputPointSize)
    }
    ...
    public init(placeholder: String,
                ...
                trailingAction: MopeliumThreadComposerSecondaryAction? = nil,
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            MopeliumGlassEffectGroup(spacing: composerGlassEffectSpacing) {
                composerControls
            }
        }
    }
    ...
    private var sendButton: some View {
        ...
        .mopeliumComposerIconButton(prominent: true)
        .disabled(!canSend)
...
public enum MopeliumThreadHeaderActionPresentation: Sendable {
    case glass
    case compactSystemIcon
}

public struct MopeliumThreadHeaderAction {
    let title: String
    let systemImage: String
    let isDisabled: Bool
    let isIconOnly: Bool
    let presentation: MopeliumThreadHeaderActionPresentation
    let help: String
    ...
    public init(title: String,
                ...
                presentation: MopeliumThreadHeaderActionPresentation = .glass,
                help: String? = nil,
...
struct MopeliumWorkspaceThreadHeader: View {
    ...
    private var titleBlock: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title)
                .font(MopeliumTypography.largeTitle())
                .foregroundStyle(style.primaryText)
            if let subtitle, !subtitle.isEmpty {
                Text(subtitle)
                    .font(MopeliumTypography.caption(13, .medium))
                    .foregroundStyle(style.secondaryText)
                    .lineLimit(2)
            }
        }
    ...
    @ViewBuilder private func actionButton(
        _ action: MopeliumThreadHeaderAction
    ) -> some View {
