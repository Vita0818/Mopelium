...
public struct MopeliumComposerDraftAttachment: Identifiable, Equatable, Sendable {
...
public struct MopeliumComposerAttachmentFile: Equatable, Sendable {
    public var name: String
...
public enum MopeliumComposerAttachmentFileReader {
    public static func read(_ url: URL) throws -> MopeliumComposerAttachmentFile {
        ...
        #endif
        return MopeliumComposerAttachmentFile(
...
public enum MopeliumComposerAttachmentResolutionError: LocalizedError, Sendable {
...
public struct MopeliumComposerAttachmentStore: Sendable {
    ...
    public func preserve(
        _ file: MopeliumComposerAttachmentFile
    ) async throws -> MopeliumComposerDraftAttachment {
        ...
        try Task.checkCancellation()
        return MopeliumComposerDraftAttachment(
            id: ref.id,
            name: file.name,
            mime: file.mime)
    }
    ...
    public func imageAttachments(
        ...
        for id in ids {
            ...
            guard let ref = await store.ref(for: id) else {
                throw MopeliumComposerAttachmentResolutionError.missing(id)
            }
            guard ref.mime.hasPrefix("image/") else {
                throw MopeliumComposerAttachmentResolutionError.unsupported(
                    id,
                    mime: ref.mime,
                    surface: surface)
            }
            ...
            do {
                ...
                throw MopeliumComposerAttachmentResolutionError.unreadable(
                    id,
