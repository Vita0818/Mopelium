#ifndef MOPELIUM_MCP_STDIO_GUARD_H
#define MOPELIUM_MCP_STDIO_GUARD_H
...
typedef struct mopelium_mcp_stdio_guard mopelium_mcp_stdio_guard_t;
...
typedef struct {
    ...
    uint64_t file_id;
    uint64_t byte_count;
} mopelium_mcp_stdio_exec_identity_t;

typedef struct {
    int mode;
    uint32_t gateway_ipv4_network_order;
    uint16_t gateway_port_host_order;
} mopelium_mcp_stdio_network_policy_t;

enum {
    MOPELIUM_MCP_STDIO_NETWORK_DENIED = 0,
    MOPELIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY = 1
};
...
} mopelium_mcp_stdio_guard_spawn_result_t;
...
typedef struct {
    ...
    int error_number;
} mopelium_mcp_stdio_guard_poll_result_t;

enum {
    MOPELIUM_MCP_STDIO_GUARD_OK = 0,
    MOPELIUM_MCP_STDIO_GUARD_UNAVAILABLE = 1,
    MOPELIUM_MCP_STDIO_GUARD_PIPE_FAILED = 2,
    MOPELIUM_MCP_STDIO_GUARD_FORK_FAILED = 3,
    MOPELIUM_MCP_STDIO_GUARD_TRACE_FAILED = 4,
    MOPELIUM_MCP_STDIO_GUARD_IDENTITY_FAILED = 5,
    MOPELIUM_MCP_STDIO_GUARD_POLICY_VIOLATION = 6
};
