...
#include "MopeliumMCPStdioGuard.h"
...
#define MOPELIUM_PTRACE_SYSCALL_INFO_NONE 0U
#define MOPELIUM_PTRACE_SYSCALL_INFO_ENTRY 1U
#define MOPELIUM_PTRACE_SYSCALL_INFO_EXIT 2U
#define MOPELIUM_PTRACE_SYSCALL_INFO_SECCOMP 3U
...
#define MOPELIUM_AUDIT_ARCH AUDIT_ARCH_X86_64
#elif defined(__aarch64__)
#define MOPELIUM_AUDIT_ARCH AUDIT_ARCH_AARCH64
#else
#define MOPELIUM_AUDIT_ARCH 0
#endif

#define MOPELIUM_SECCOMP_ALLOW SECCOMP_RET_ALLOW
#define MOPELIUM_SECCOMP_TRACE SECCOMP_RET_TRACE
#define MOPELIUM_SECCOMP_DENY \
    (SECCOMP_RET_ERRNO | (EPERM & SECCOMP_RET_DATA))
#define MOPELIUM_SECCOMP_NOT_IMPLEMENTED \
...
typedef struct {
    ...
    uint64_t file_id;
    uint64_t byte_count;
} mopelium_owned_identity_t;

struct mopelium_mcp_stdio_guard {
    pid_t group_id;
    pid_t initial_pid;
    pid_t target_pid;
    int wrapper_seen;
    ...
    int fatal_errno;
    mopelium_mcp_stdio_network_policy_t network_policy;
    mopelium_owned_identity_t wrapper;
    mopelium_owned_identity_t primary;
    mopelium_owned_identity_t *helpers;
    size_t helper_count;
    mopelium_tracee_t *tracees;
    size_t tracee_count;
    size_t tracee_capacity;
};

#define MOPELIUM_MAXIMUM_TRACEE_COUNT 4096U
#define MOPELIUM_FREEZE_TIMEOUT_MILLISECONDS 2000U
#define MOPELIUM_NETWORK_SYSCALL_TIMEOUT_MILLISECONDS 5000U

typedef struct {
    struct sock_filter *items;
    size_t count;
    size_t capacity;
} mopelium_filter_builder_t;

static void mopelium_close_fd(int *fd) {
    if (*fd >= 0) {
        (void)close(*fd);
        *fd = -1;
    }
...
static int mopelium_set_cloexec(int fd) {
...
static int mopelium_set_nonblocking(int fd) {
    int flags = fcntl(fd, F_GETFL);
    if (flags < 0 || fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0) {
        return -1;
    }
    return mopelium_set_cloexec(fd);
}

static int mopelium_pipe_cloexec(int descriptors[2]) {
    ...
    if (mopelium_set_cloexec(descriptors[0]) < 0 ||
        mopelium_set_cloexec(descriptors[1]) < 0) {
        int saved = errno;
        close(descriptors[0]);
        close(descriptors[1]);
        errno = saved;
...
static int mopelium_append_filter(
    mopelium_filter_builder_t *builder,
...
#define MOPELIUM_STMT(code_value, k_value) \
    ((struct sock_filter)BPF_STMT((code_value), (k_value)))
#define MOPELIUM_JUMP(code_value, k_value, true_jump, false_jump) \
    ((struct sock_filter)BPF_JUMP( \
        (code_value), (k_value), (true_jump), (false_jump)))

static int mopelium_append_deny_syscall(
    mopelium_filter_builder_t *builder,
    int syscall_number,
    uint32_t denial) {
    return mopelium_append_filter(
               builder,
               MOPELIUM_JUMP(
                   ...
                   0,
                   1)) ||
           mopelium_append_filter(
               builder,
               MOPELIUM_STMT(BPF_RET | BPF_K, denial));
}

static int mopelium_append_trace_syscall(
    mopelium_filter_builder_t *builder,
    int syscall_number) {
    return mopelium_append_deny_syscall(
        builder, syscall_number, MOPELIUM_SECCOMP_TRACE);
}

static int mopelium_build_seccomp_filter(
    int helpers_enabled,
    struct sock_filter **items,
    size_t *count) {
    if (MOPELIUM_AUDIT_ARCH == 0) {
        errno = ENOTSUP;
        return -1;
    }
    mopelium_filter_builder_t builder = {0};
    if (mopelium_append_filter(
            &builder,
            MOPELIUM_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct mopelium_seccomp_data, arch))) ||
        mopelium_append_filter(
            &builder,
            MOPELIUM_JUMP(
                BPF_JMP | BPF_JEQ | BPF_K,
        ...
        mopelium_append_filter(
            &builder,
            MOPELIUM_STMT(BPF_RET | BPF_K, SECCOMP_RET_KILL_PROCESS)) ||
        mopelium_append_filter(
            &builder,
            MOPELIUM_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct mopelium_seccomp_data, nr)))) {
        free(builder.items);
        return -1;
    }

#ifdef __NR_setsid
    if (mopelium_append_deny_syscall(
            &builder, __NR_setsid, MOPELIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_setpgid
    if (mopelium_append_deny_syscall(
            &builder, __NR_setpgid, MOPELIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_unshare
    if (mopelium_append_deny_syscall(
            &builder, __NR_unshare, MOPELIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_setns
    if (mopelium_append_deny_syscall(
            &builder, __NR_setns, MOPELIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_ptrace
    if (mopelium_append_deny_syscall(
            &builder, __NR_ptrace, MOPELIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_clone3
    ...
    if (mopelium_append_deny_syscall(
            &builder,
            __NR_clone3,
            MOPELIUM_SECCOMP_NOT_IMPLEMENTED)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_fork
    if (!helpers_enabled &&
        mopelium_append_deny_syscall(
            &builder, __NR_fork, MOPELIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_vfork
    if (!helpers_enabled &&
        mopelium_append_deny_syscall(
            &builder, __NR_vfork, MOPELIUM_SECCOMP_DENY)) {
        goto allocation_failure;
    }
#endif
    ...
    if (mopelium_append_filter(
        ...
        mopelium_append_filter(
            ...
            MOPELIUM_STMT(
                BPF_LD | BPF_W | BPF_ABS,
                offsetof(struct mopelium_seccomp_data, args[0]))) ||
        mopelium_append_filter(
            &builder,
            MOPELIUM_JUMP(
        ...
        mopelium_append_filter(
            &builder,
            MOPELIUM_STMT(
                BPF_RET | BPF_K, MOPELIUM_SECCOMP_DENY)) ||
        mopelium_append_filter(
            &builder,
            MOPELIUM_STMT(
                BPF_RET | BPF_K, MOPELIUM_SECCOMP_ALLOW))) {
        goto allocation_failure;
    }
    if (helpers_enabled) {
    ...
    } else if (mopelium_append_filter(
                   &builder,
                   MOPELIUM_STMT(
                       BPF_RET | BPF_K, MOPELIUM_SECCOMP_DENY))) {
        goto allocation_failure;
     ...
     */
#ifdef __NR_socket
    if (mopelium_append_trace_syscall(&builder, __NR_socket)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_connect
    if (mopelium_append_trace_syscall(&builder, __NR_connect)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_bind
    if (mopelium_append_trace_syscall(&builder, __NR_bind)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_listen
    if (mopelium_append_trace_syscall(&builder, __NR_listen)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_accept
    if (mopelium_append_trace_syscall(&builder, __NR_accept)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_accept4
    if (mopelium_append_trace_syscall(&builder, __NR_accept4)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendto
    if (mopelium_append_trace_syscall(&builder, __NR_sendto)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendmsg
    if (mopelium_append_trace_syscall(&builder, __NR_sendmsg)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_sendmmsg
    if (mopelium_append_trace_syscall(&builder, __NR_sendmmsg)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_setup
    if (mopelium_append_trace_syscall(&builder, __NR_io_uring_setup)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_enter
    if (mopelium_append_trace_syscall(&builder, __NR_io_uring_enter)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_io_uring_register
    if (mopelium_append_trace_syscall(&builder, __NR_io_uring_register)) {
        goto allocation_failure;
    }
#endif
#ifdef __NR_bpf
    if (mopelium_append_trace_syscall(&builder, __NR_bpf)) {
        goto allocation_failure;
    }
#endif
    ...
    if (mopelium_append_filter(
            &builder,
            MOPELIUM_STMT(BPF_RET | BPF_K, MOPELIUM_SECCOMP_ALLOW))) {
        goto allocation_failure;
    }
    *items = builder.items;
    *count = builder.count;
...
static int mopelium_make_seccomp_fd(
    ...
    size_t count = 0;
    if (mopelium_build_seccomp_filter(
...
static int mopelium_close_from_four(void) {
...
static int mopelium_write_handshake_byte(int fd) {
...
static int mopelium_read_handshake_byte(int fd) {
...
static void mopelium_guarded_child(
     ...
     */
    if (mopelium_write_handshake_byte(startup_child) < 0 ||
        mopelium_read_handshake_byte(startup_child) < 0) {
        _exit(126);
    }
    (void)close(startup_child);
    if (mopelium_close_from_four() < 0 ||
...
static int mopelium_capture_owned_identity(
    const char *path,
    mopelium_owned_identity_t *identity) {
...
static int mopelium_copy_identity(
    const mopelium_mcp_stdio_exec_identity_t *source,
    mopelium_owned_identity_t *destination) {
...
static int mopelium_add_tracee(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t pid,
    int is_wrapper_root) {
    ...
    if (guard->tracee_count >= MOPELIUM_MAXIMUM_TRACEE_COUNT) {
        errno = E2BIG;
        return -1;
    }
    if (guard->tracee_count == guard->tracee_capacity) {
        ...
        mopelium_tracee_t *tracees = (mopelium_tracee_t *)realloc(
            guard->tracees, next * sizeof(mopelium_tracee_t));
        if (tracees == NULL) {
            return -1;
        }
...
static mopelium_tracee_t *mopelium_find_tracee(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].pid == pid) {
...
static int mopelium_identity_matches_pid(
    pid_t pid,
    const mopelium_owned_identity_t *expected) {
    char proc_path[64];
    char resolved[4096];
    struct stat metadata;
    int length = snprintf(
...
static int mopelium_match_helper(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    for (size_t index = 0; index < guard->helper_count; index++) {
        if (mopelium_identity_matches_pid(pid, &guard->helpers[index])) {
            return 1;
        }
    }
    return 0;
...
typedef struct {
    ...
    uint64_t instruction_pointer;
    uint64_t stack_pointer;
} mopelium_ptrace_syscall_info_prefix_t;

static int mopelium_ptrace_syscall_operation(
    pid_t pid,
    uint8_t *operation) {
    mopelium_ptrace_syscall_info_prefix_t information;
    ...
    if (count < (long)sizeof(information) ||
        information.operation
            == MOPELIUM_PTRACE_SYSCALL_INFO_NONE) {
        errno = ENOTSUP;
        return -1;
    }
...
static int mopelium_read_syscall_snapshot(
    pid_t pid,
    mopelium_syscall_snapshot_t *snapshot) {
#if defined(__x86_64__) || defined(__aarch64__)
    mopelium_native_registers_t registers;
...
static int mopelium_rewrite_syscall_number(
    pid_t pid,
    long syscall_number) {
#if defined(__x86_64__) || defined(__aarch64__)
    mopelium_native_registers_t registers;
...
static int mopelium_rewrite_syscall_result(
    pid_t pid,
    long result) {
#if defined(__x86_64__) || defined(__aarch64__)
    mopelium_native_registers_t registers;
...
static int mopelium_read_tracee_memory(
    pid_t pid,
...
static int mopelium_validate_network_syscall(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    mopelium_syscall_snapshot_t snapshot;
    memset(&snapshot, 0, sizeof(snapshot));
    if (mopelium_read_syscall_snapshot(pid, &snapshot) < 0) {
        return -1;
    }
    ...
    if (snapshot.syscall_number == __NR_socket) {
        ...
        return guard->network_policy.mode
                    == MOPELIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY &&
               family == AF_INET &&
    ...
    if (snapshot.syscall_number == __NR_connect) {
        ...
        if (mopelium_read_tracee_memory(
                pid,
...
static uint64_t mopelium_monotonic_milliseconds(void);

static int mopelium_thread_group_leader(
    pid_t thread_id,
    pid_t *leader_id) {
 ...
 */
static int mopelium_emulate_exact_connect(
    const mopelium_mcp_stdio_guard_t *guard,
    pid_t pid,
    const mopelium_syscall_snapshot_t *snapshot,
     ...
     */
    pid_t leader_id = 0;
    if (mopelium_thread_group_leader(pid, &leader_id) < 0) {
        return -1;
    }
    int pid_descriptor =
    ...
    if (connect_result < 0 &&
        ...
        uint64_t start = mopelium_monotonic_milliseconds();
        if (start == 0) {
            connect_error = EIO;
            goto connect_failure;
        }
        for (;;) {
            uint64_t now = mopelium_monotonic_milliseconds();
            if (now == 0 ||
                now - start >=
                    MOPELIUM_NETWORK_SYSCALL_TIMEOUT_MILLISECONDS) {
...
static void mopelium_mark_violation(
    mopelium_mcp_stdio_guard_t *guard,
    int error_number) {
    if (!guard->violation) {
        guard->violation = 1;
...
static void mopelium_mark_network_violation(
    mopelium_mcp_stdio_guard_t *guard,
    int error_number) {
    guard->network_violation = 1;
    mopelium_mark_violation(guard, error_number);
}

static int mopelium_decode_exit_status(int status);
static int mopelium_handle_exec_event(
    mopelium_mcp_stdio_guard_t *guard,
    mopelium_tracee_t *tracee);
...
static void mopelium_poll_pause(void) {
    struct timespec value;
    value.tv_sec = 0;
...
static void mopelium_record_tracee_exit(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t pid,
    int status) {
    mopelium_tracee_t *tracee =
        mopelium_find_tracee(guard, pid);
    if (tracee == NULL) {
        return;
    }
    tracee->alive = 0;
    ...
    if (pid == guard->target_pid) {
        ...
        guard->target_exit_status =
            mopelium_decode_exit_status(status);
    }
}

static int mopelium_all_live_tracees_frozen(
    const mopelium_mcp_stdio_guard_t *guard) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].alive &&
...
static int mopelium_capture_frozen_stop(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t pid,
    int status) {
    if (WIFEXITED(status) || WIFSIGNALED(status)) {
        mopelium_record_tracee_exit(guard, pid, status);
        return 0;
    }
 ...
 */
static int mopelium_execute_frozen_network_syscall(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t pid) {
    mopelium_tracee_t *tracee =
        mopelium_find_tracee(guard, pid);
    ...
    mopelium_syscall_snapshot_t expected;
    memset(&expected, 0, sizeof(expected));
    if (mopelium_read_syscall_snapshot(
            pid, &expected) < 0) {
        return -1;
    }
    ...
    if (expected.syscall_number == __NR_connect) {
        if (mopelium_emulate_exact_connect(
                guard,
                pid,
                &expected,
                &emulated_result) < 0 ||
            mopelium_rewrite_syscall_number(pid, -1) < 0) {
            return -1;
        }
    ...
    for (;;) {
        ...
        if (waited == pid) {
            if (WIFEXITED(status) || WIFSIGNALED(status)) {
                mopelium_record_tracee_exit(
                    guard, pid, status);
                errno = EIO;
                return -1;
            }
            tracee = mopelium_find_tracee(guard, pid);
            ...
            uint8_t operation = 0;
            mopelium_syscall_snapshot_t observed;
            memset(&observed, 0, sizeof(observed));
            if (mopelium_ptrace_syscall_operation(
                    pid, &operation) < 0 ||
                mopelium_read_syscall_snapshot(
                    pid, &observed) < 0 ||
            ...
            if (operation
                    == MOPELIUM_PTRACE_SYSCALL_INFO_EXIT) {
                if (has_emulated_result &&
                    mopelium_rewrite_syscall_result(
            ...
            if ((operation
                    != MOPELIUM_PTRACE_SYSCALL_INFO_ENTRY &&
                 operation
                    != MOPELIUM_PTRACE_SYSCALL_INFO_SECCOMP) ||
                ++syscall_stops > 2) {
                errno = EIO;
                return -1;
            }
 ...
 */
static int mopelium_freeze_validate_and_execute_network(
    mopelium_mcp_stdio_guard_t *guard,
    pid_t triggering_pid,
    int triggering_status) {
    mopelium_tracee_t *trigger =
        mopelium_find_tracee(guard, triggering_pid);
    if (trigger == NULL || !trigger->alive) {
        mopelium_mark_network_violation(guard, ECHILD);
        return -1;
    }
    trigger->frozen = 1;
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        mopelium_tracee_t *tracee = &guard->tracees[index];
        ...
        if (ptrace(
            ...
            mopelium_mark_network_violation(guard, errno);
            return -1;
        }
    }
    ...
    uint64_t start = mopelium_monotonic_milliseconds();
    if (start == 0) {
        mopelium_mark_network_violation(guard, EIO);
        return -1;
    }
    while (!mopelium_all_live_tracees_frozen(guard)) {
        int made_progress = 0;
        ...
        for (size_t index = 0; index < observed_count; index++) {
            ...
            if (waited < 0 ||
                mopelium_capture_frozen_stop(
                    guard, pid, status) < 0) {
                int saved = errno;
                mopelium_mark_network_violation(
        ...
        if (!made_progress) {
            uint64_t now = mopelium_monotonic_milliseconds();
            if (now == 0 || now - start
                    >= MOPELIUM_FREEZE_TIMEOUT_MILLISECONDS) {
                mopelium_mark_network_violation(
                    guard, ETIMEDOUT);
                return -1;
            }
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        ...
        if (event == PTRACE_EVENT_EXEC &&
            mopelium_handle_exec_event(guard, tracee) < 0) {
            mopelium_mark_violation(guard, EACCES);
            return -1;
        }
    }
    ...
    for (size_t index = 0; index < guard->tracee_count; index++) {
        ...
        if (mopelium_ptrace_syscall_operation(
            ...
            operation
                != MOPELIUM_PTRACE_SYSCALL_INFO_SECCOMP ||
            mopelium_validate_network_syscall(
                guard, pid) < 0 ||
            mopelium_execute_frozen_network_syscall(
                guard, pid) < 0) {
            int saved = errno;
            mopelium_mark_network_violation(
...
static int mopelium_all_tracees_exited(
    const mopelium_mcp_stdio_guard_t *guard) {
    for (size_t index = 0; index < guard->tracee_count; index++) {
        if (guard->tracees[index].alive) {
            return 0;
...
static int mopelium_handle_exec_event(
    ...
    if (!guard->wrapper_seen) {
        if (!tracee->is_wrapper_root ||
            !mopelium_identity_matches_pid(tracee->pid, &guard->wrapper)) {
            return -1;
        }
    ...
    if (!guard->target_seen) {
        if (!mopelium_identity_matches_pid(tracee->pid, &guard->primary)) {
            return -1;
        }
        guard->target_seen = 1;
    ...
    if (!tracee->may_exec_helper ||
        ...
        !mopelium_match_helper(guard, tracee->pid)) {
        return -1;
    }
...
static void mopelium_execution_guard_probe_child(void) {
    struct sock_filter filter[] = {
        BPF_STMT(
            BPF_LD | BPF_W | BPF_ABS,
            offsetof(struct mopelium_seccomp_data, nr)),
...
int mopelium_mcp_stdio_execution_guard_probe(void) {
    ...
    pid_t child = fork();
    if (child < 0) {
        mopelium_close_fd(&startup_pair[0]);
        mopelium_close_fd(&startup_pair[1]);
        return 0;
    }
    if (child == 0) {
        mopelium_close_fd(&startup_pair[0]);
        if (mopelium_read_handshake_byte(startup_pair[1]) < 0) {
            _exit(111);
        }
        mopelium_close_fd(&startup_pair[1]);
        mopelium_execution_guard_probe_child();
    }
    ...
    if (ptrace(
        ...
        mopelium_write_handshake_byte(startup_pair[0]) < 0 ||
        waitpid(child, &status, __WALL) != child ||
        !WIFSTOPPED(status) ||
        (((unsigned int)status) >> 16)
            != PTRACE_EVENT_SECCOMP ||
        mopelium_ptrace_syscall_operation(
            child, &operation) < 0 ||
        operation
            != MOPELIUM_PTRACE_SYSCALL_INFO_SECCOMP ||
        mopelium_rewrite_syscall_number(child, -1) < 0 ||
        ...
        operation
            != MOPELIUM_PTRACE_SYSCALL_INFO_EXIT ||
        mopelium_rewrite_syscall_result(child, 4242) < 0 ||
        ptrace(PTRACE_CONT, child, NULL, NULL) < 0 ||
        waitpid(child, &status, __WALL) != child ||
        !WIFEXITED(status) ||
            WEXITSTATUS(status) != 0) {
        mopelium_close_fd(&duplicated_descriptor);
        mopelium_close_fd(&pid_descriptor);
        mopelium_close_fd(&startup_pair[0]);
...
int mopelium_mcp_stdio_guard_spawn(
    const char *wrapper_path,
    char *const argv[],
    char *const envp[],
    const char *working_directory,
    const char *runtime_directory,
    const mopelium_mcp_stdio_exec_identity_t *primary,
    const mopelium_mcp_stdio_exec_identity_t *helpers,
    size_t helper_count,
    const mopelium_mcp_stdio_network_policy_t *network_policy,
    mopelium_mcp_stdio_guard_spawn_result_t *result) {
    if (result == NULL) {
        return MOPELIUM_MCP_STDIO_GUARD_UNAVAILABLE;
    }
    ...
    if (wrapper_path == NULL || argv == NULL || envp == NULL ||
        ...
        (network_policy->mode != MOPELIUM_MCP_STDIO_NETWORK_DENIED &&
         network_policy->mode
            != MOPELIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY) ||
        (network_policy->mode
            == MOPELIUM_MCP_STDIO_NETWORK_EXACT_GATEWAY &&
        ...
        (network_policy->mode
            == MOPELIUM_MCP_STDIO_NETWORK_DENIED &&
         (network_policy->gateway_ipv4_network_order != 0 ||
          network_policy->gateway_port_host_order != 0)) ||
        MOPELIUM_AUDIT_ARCH == 0) {
    ...
    if (mopelium_pipe_cloexec(stdin_pipe) < 0 ||
        mopelium_pipe_cloexec(stdout_pipe) < 0 ||
        mopelium_pipe_cloexec(stderr_pipe) < 0) {
        result->error_number = errno;
        mopelium_close_fd(&stdin_pipe[0]);
        mopelium_close_fd(&stdin_pipe[1]);
        mopelium_close_fd(&stdout_pipe[0]);
        mopelium_close_fd(&stdout_pipe[1]);
        mopelium_close_fd(&stderr_pipe[0]);
        mopelium_close_fd(&stderr_pipe[1]);
        return MOPELIUM_MCP_STDIO_GUARD_PIPE_FAILED;
    }
    int seccomp_fd =
        mopelium_make_seccomp_fd(runtime_directory, helper_count > 0);
    if (seccomp_fd < 0) {
        result->error_number = errno;
        mopelium_close_fd(&stdin_pipe[0]);
        mopelium_close_fd(&stdin_pipe[1]);
    ...
    mopelium_mcp_stdio_guard_t *guard =
        (mopelium_mcp_stdio_guard_t *)calloc(1, sizeof(*guard));
    if (guard == NULL ||
        mopelium_capture_owned_identity(
            wrapper_path, &guard->wrapper) < 0 ||
        mopelium_copy_identity(primary, &guard->primary) < 0) {
        result->error_number = errno;
        mopelium_close_fd(&seccomp_fd);
        mopelium_close_fd(&stdin_pipe[0]);
        mopelium_close_fd(&stdin_pipe[1]);
        mopelium_close_fd(&stdout_pipe[0]);
        mopelium_close_fd(&stdout_pipe[1]);
        mopelium_close_fd(&stderr_pipe[0]);
        mopelium_close_fd(&stderr_pipe[1]);
        mopelium_mcp_stdio_guard_destroy(guard);
        return MOPELIUM_MCP_STDIO_GUARD_IDENTITY_FAILED;
    }
    guard->network_policy = *network_policy;
    if (helper_count > 0) {
        guard->helpers = (mopelium_owned_identity_t *)calloc(
            helper_count, sizeof(mopelium_owned_identity_t));
    ...
    if (child < 0) {
        ...
        mopelium_close_fd(&stderr_pipe[1]);
        mopelium_mcp_stdio_guard_destroy(guard);
        return MOPELIUM_MCP_STDIO_GUARD_FORK_FAILED;
    }
    if (child == 0) {
        mopelium_guarded_child(
            wrapper_path,
            argv,
            envp,
            working_directory,
    ...
    if (mopelium_add_tracee(guard, child, 1) < 0) {
        result->error_number = ENOMEM;
        (void)kill(-child, SIGKILL);
        (void)kill(child, SIGKILL);
        (void)waitpid(child, NULL, 0);
        ...
        mopelium_close_fd(&stderr_pipe[0]);
        mopelium_mcp_stdio_guard_destroy(guard);
        return MOPELIUM_MCP_STDIO_GUARD_TRACE_FAILED;
    }
    ...
    if (mopelium_read_handshake_byte(startup_pair[0]) < 0 ||
        getpgid(child) != child ||
        ptrace(
            PTRACE_SEIZE,
            child,
        ...
        mopelium_set_nonblocking(stdin_pipe[1]) < 0 ||
        mopelium_set_nonblocking(stdout_pipe[0]) < 0 ||
        mopelium_set_nonblocking(stderr_pipe[0]) < 0 ||
        mopelium_write_handshake_byte(startup_pair[0]) < 0) {
        result->error_number = errno;
        (void)kill(-child, SIGKILL);
        (void)kill(child, SIGKILL);
    ...
    result->guard = guard;
    return MOPELIUM_MCP_STDIO_GUARD_OK;
}

int mopelium_mcp_stdio_guard_poll(
    mopelium_mcp_stdio_guard_t *guard,
    mopelium_mcp_stdio_guard_poll_result_t *result) {
    ...
    do {
        ...
        for (size_t index = 0; index < observed_count; index++) {
            ...
            if (waited < 0) {
                ...
                if (errno == ECHILD) {
                    mopelium_mark_violation(guard, ECHILD);
                    continue;
                }
                mopelium_mark_violation(guard, errno);
                continue;
            }
            made_progress = 1;
            if (WIFEXITED(status) || WIFSIGNALED(status)) {
            ...
            if (event == PTRACE_EVENT_EXEC) {
                if (mopelium_handle_exec_event(guard, tracee) < 0) {
                    mopelium_mark_violation(guard, EACCES);
                }
            } else if (event == PTRACE_EVENT_SECCOMP) {
                if (mopelium_freeze_validate_and_execute_network(
    ...
    if (!guard->target_seen &&
        mopelium_all_tracees_exited(guard)) {
        guard->target_exited = 1;
    ...
    result->all_tracees_exited = mopelium_all_tracees_exited(guard);
    ...
    return guard->violation
        ? MOPELIUM_MCP_STDIO_GUARD_POLICY_VIOLATION
        : MOPELIUM_MCP_STDIO_GUARD_OK;
}

void mopelium_mcp_stdio_guard_destroy(
    mopelium_mcp_stdio_guard_t *guard) {
    if (guard == NULL) {
        return;
    }
