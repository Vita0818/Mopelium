...
public actor ManagedPipeProcess: Transport {
    ...
    private init(
        ...
        self.logger = Logger(
            label: "com.vitemis.mopelium.mcp.stdio.\(ticket.request.authority.server.serverID.rawValue)")
    ...
    @unchecked Sendable
{
    ...
    func poll() -> MCPStdioLinuxGuardObservation {
        ...
        var result = mopelium_mcp_stdio_guard_poll_result_t()
        let status = mopelium_mcp_stdio_guard_poll(
            pointer,
            &result)
        return MCPStdioLinuxGuardObservation(
    ...
    deinit {
        if let pointer {
            mopelium_mcp_stdio_guard_destroy(pointer)
            self.pointer = nil
        }
    }
}
...
private func spawnLinuxGuardedPipeProcess(
    ...
    var helperIdentities = helpers.map { helper in
        var value = mopelium_mcp_stdio_exec_identity_t()
                            ...
                            helperIdentities
                                .withUnsafeMutableBufferPointer {
                                    ...
                                    withUnsafePointer(
                                        ...
                                        mopelium_mcp_stdio_guard_spawn(
                                            wrapperPointer,
                                            argvPointer,
                                            envPointer,
    ...
    guard status == MOPELIUM_MCP_STDIO_GUARD_OK,
          result.process_id > 0,
        ...
        if let guardPointer = result.guard {
            mopelium_mcp_stdio_guard_destroy(guardPointer)
        }
        if status == MOPELIUM_MCP_STDIO_GUARD_UNAVAILABLE
            || status
                == MOPELIUM_MCP_STDIO_GUARD_TRACE_FAILED {
            throw MCPManagedPipeError
                .descendantExecutionGuardUnavailable
        }
