...
import Crypto
#else
#error("MopeliumMCP requires CryptoKit or swift-crypto")
#endif
...
private struct ImportParserState {
    ...
    mutating func parseImportedValue(
        ...
        if let object = rawValue as? [String: Any] {
            let allowed: Set<String> = [
                "$mopeliumSecretRef", "storage", "sourceBindingFingerprint",
            ]
            guard Set(object.keys).isSubset(of: allowed),
                  let identifier = object["$mopeliumSecretRef"] as? String,
...
public enum MCPConfigurationExporter {
    /// Emits a stable Mopelium MCP export. Secret values are structurally
    ...
    public static func sanitizedMCPJSON(
        ...
        let root: [String: Any] = [
            "$schema": "https://mopelium.invalid/schemas/mcp-import-v1.json",
            "version": 1,
            "mcpServers": servers,
        ]
    ...
    private static func exportedReference(_ reference: MCPSecretReference) -> [String: Any] {
        var result: [String: Any] = [
            "$mopeliumSecretRef": reference.identifier,
