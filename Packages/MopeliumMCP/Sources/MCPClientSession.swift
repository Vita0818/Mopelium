            ...
            let value = try await race(
                ...
                onTimeout: {
                    ...
                    try? await self.client.cancelRequest(
                        context.requestID,
                        reason: "Mopelium request timed out")
                },
                onCancellation: {
                    if let progressToken {
                        await broker?.finishRequest(
                    ...
                    try? await self.client.cancelRequest(
                        context.requestID,
                        reason: "Mopelium request cancelled")
                },
            ...
            _ = try await race(
                ...
                onTimeout: {
                    ...
                    try? await self.client.cancelRequest(
                        context.requestID,
                        reason:
                            "Mopelium logging configuration timed out")
                },
                onCancellation: {
                    await broker.finishRequest(
                        progressToken: progressToken,
                    ...
                    try? await self.client.cancelRequest(
                        context.requestID,
                        reason:
                            "Mopelium logging configuration cancelled")
                },
                timeoutError: .requestTimedOut(
                    method: request.method,
                    milliseconds: timeout),
