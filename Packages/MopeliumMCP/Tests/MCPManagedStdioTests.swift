...
import MopeliumCore
import MopeliumMCP
@testable import MopeliumMCPStdio
...
final class MCPManagedStdioTests: XCTestCase {
        ...
        throws
    {
        ...
        XCTAssertTrue(gateway.proxyURL.hasPrefix("http://mopelium:"))
        ...
        async throws
    {
        ...
        let outside = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-mcp-outside-\(UUID().uuidString)",
                isDirectory: true)
        try FileManager.default.createDirectory(
            at: outside,
            withIntermediateDirectories: false)
    ...
    private func makeLinuxMaskWorkspace() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-mcp-linux-mask-\(UUID().uuidString)",
                isDirectory: true)
        try FileManager.default.createDirectory(
            at: root,
            withIntermediateDirectories: false,
...
private func runRelativeImportFixture(
    ...
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent(
            "mopelium-mcp-relative-import-\(UUID().uuidString)",
            isDirectory: true)
    try FileManager.default.createDirectory(
        at: root,
        withIntermediateDirectories: false)
...
private struct StdioFixture {
    ...
    init(mode: Mode, networkProbePort: UInt16 = 9) throws {
        ...
        self.rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-mcp-fixture-\(UUID().uuidString)",
                isDirectory: true)
