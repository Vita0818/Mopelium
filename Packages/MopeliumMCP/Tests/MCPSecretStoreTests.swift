...
final class MCPSecretStoreTests: XCTestCase {
        ...
        async throws
    {
        ...
        let service =
            "com.vitemis.mopelium.mcp.tests.\(UUID().uuidString)"
    ...
    func testMacOSProductionKeychainQueryUsesDataProtectionAndAccessClass() {
        let query = MCPKeychainSecretStore.makeAddQuery(
            service: "com.vitemis.mopelium.mcp.tests.query",
            account: "mcp:test",
            accessGroup: nil,
    ...
    private func requireHostKeychainIntegration() throws {
        guard ProcessInfo.processInfo.environment[
            "MOPELIUM_RUN_HOST_KEYCHAIN_TESTS"
        ] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_RUN_HOST_KEYCHAIN_TESTS=1 in an unsandboxed, "
    ...
    private func makeFixture() throws -> (root: URL, store: URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-mcp-secret-store-\(UUID().uuidString)",
                isDirectory: true)
