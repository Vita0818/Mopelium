...
final class MCPOAuthTests: XCTestCase {
        ...
        async throws
    {
        ...
        let components = makeCoordinator(client: client)
        let document = URL(
            string: "https://client.example/mopelium.json")!
        let attempt = try await components.coordinator.beginLogin(
            serverID: .init(rawValue: "mcpserver_oauth"),
            accountReference: .init(rawValue: "account_one"),
...
extension MCPOAuthTests {
        ...
        let root =
            FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "mopelium-oauth-edit-r2-\(UUID().uuidString)")
        defer {
        ...
        async throws
    {
        let root =
            FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "mopelium-oauth-staged-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: root)
        }
        ...
        async throws
    {
        let root =
            FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "mopelium-oauth-conflict-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: root)
        }
        let store = MCPServerCatalogStore(
        ...
        async throws
    {
        let root =
            FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "mopelium-oauth-authority-mismatch-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: root)
        }
        let store = MCPServerCatalogStore(
