...
final class KnowledgeSnapshotStoreTests: XCTestCase {
    func testReadOnlyOpenDoesNotCreateMissingStoreInfrastructure() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-knowledge-readonly-open-\(UUID().uuidString)",
            isDirectory: true)
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: storeRoot
            .appendingPathComponent(".mopelium-rag-host").path))
        XCTAssertFalse(FileManager.default.fileExists(atPath: storeRoot
    ...
    func testPointerCrashStatesExposeOnlyCompleteOldOrNewValue() throws {
        ...
        try DurableOwnerOnlyFile.writeAtomically(
            ...
            to: fixture.store.root.appendingPathComponent(
                ".mopelium-rag-store.json"),
            temporaryPrefix: ".mopelium-rag-store-test-")
    ...
    func testUnsafePointerSnapshotNameFailsClosedWithoutPathResolution() throws {
        ...
        let bytes = Data(
            """
            {"schema":"mopelium-rag-store/1","store_id":"kb_fixture","revision":1,"current_snapshot":"../outside","current_snapshot_revision":"\(SnapshotStoreFixture.digest(9))"}
            """.utf8)
...
private final class SnapshotStoreFixture {
    ...
    static func make(
        ...
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-knowledge-store-tests-\(UUID().uuidString)",
            isDirectory: true)
        try FileManager.default.createDirectory(
            at: root,
            withIntermediateDirectories: true,
    ...
    static func validatedSnapshot(root: URL,
        ...
        let embedding = KnowledgeEmbeddingModelIdentity(
            identity: "org.vita.mopelium.fixture-embedding",
            revision: "1",
            tokenizerRevision: "1",
        ...
        let dense = KnowledgeEmbeddingIndexProfile(
            ...
            indexPath: ".mopelium-rag/dense/exact-knn.json",
            backend: backend,
            model: embedding,
        ...
        let profile = KnowledgeProfile(
            ...
            chunking: .init(
                manifest: ".mopelium-rag/chunks.jsonl",
            ...
            integrity: .init(
                algorithm: "sha256",
                inventory: ".mopelium-rag/checksums.json"))
