...
public struct KnowledgeSecureFileSystem: Sendable {
    ...
    public static func canonicalBundleDigest(_ entries: [KnowledgeChecksumEntry]) throws -> String {
        ...
        return try KnowledgeDigest.canonical(
            Projection(version: "mopelium-okf-bundle-digest/1", files: selected))
    }
    ...
    public func snapshotSealDigest(root: URL) throws -> String {
        ...
        return try KnowledgeDigest.canonical(Projection(
            version: "mopelium-knowledge-snapshot-content-seal/1",
            files: files))
    }
    ...
    private static func role(for path: String) -> String {
        if path == ".mopelium-rag/profile.json" { return "profile" }
        if path == ".mopelium-rag/chunks.jsonl" { return "chunk_manifest" }
        if path.hasPrefix(".mopelium-rag/dense/") { return "dense_index" }
        if path.hasPrefix(".mopelium-rag/lexical/") { return "lexical_index" }
        if path.hasPrefix(".mopelium-rag/auxiliary/") { return "auxiliary" }
