...
final class MopeliumHangDiagnosticsTests: XCTestCase {
    ...
    func testIntervalDurationRecordsOnceEvenWhenTokenIsCopied() {
        ...
        XCTAssertEqual(
            MopeliumDiagnosticDurationBucket.allCases.reduce(0) {
    ...
    func testMetricsSnapshotDecodesLegacyCountersWithoutDurations() throws {
        ...
        let snapshot = try JSONDecoder().decode(
            MopeliumDiagnosticMetricsSnapshot.self,
            from: legacy)

        XCTAssertEqual(snapshot.value(for: .projectionBatches), 2)
        for metric in MopeliumDiagnosticDurationMetric.allCases {
            XCTAssertEqual(
    ...
    func testHangManifestRoundTripCarriesDurationSummaries() throws {
        ...
        let decoded = try JSONDecoder().decode(
            MopeliumHangDiagnosticManifest.self,
            from: data)
    ...
    func testSanitizerRemovesPathsURLsAndCredentialShapes() {
        ...
        """
        let sanitized = MopeliumHangDiagnosticTextSanitizer.sanitize(raw)
    ...
    func testHangBundleIsOwnerOnlyAtomicAndBoundedByCount() async throws {
        ...
        let root = parent.appendingPathComponent(
            "HangDiagnostics",
            isDirectory: true)
        let store = MopeliumHangDiagnosticBundleStore(
            rootURL: root,
        ...
        for index in 0..<3 {
            let manifest = MopeliumHangDiagnosticManifest(
                ...
                metrics: .init(counters: [
                    MopeliumDiagnosticCounter.mainThreadIncidents.rawValue:
                        UInt64(index + 1),
                ]))
    ...
    func testProductionRetentionIsFiveBundlesAndTwentyMegabytes() {
        XCTAssertEqual(
            MopeliumHangDiagnosticRetentionPolicy.production.maximumBundleCount,
            5)
        XCTAssertEqual(
            MopeliumHangDiagnosticRetentionPolicy.production.maximumTotalBytes,
            20 * 1_024 * 1_024)
    }
    ...
    func testHangBundleRetentionAlsoPrunesByTotalBytes() async throws {
        ...
        for index in 0..<2 {
            let attachment = MopeliumHangDiagnosticAttachment.sanitizedText(
                kind: .sample,
                rawData: Data(
    ...
    func testHangBundleSanitizesExternalTextAndLimitsAttachments() async throws {
        ...
        let store = MopeliumHangDiagnosticBundleStore(rootURL: root)
    ...
    func testHangBundleRejectsSymlinkRoot() async throws {
        ...
        let store = MopeliumHangDiagnosticBundleStore(rootURL: link)
        ...
        } catch let error as MopeliumHangDiagnosticStoreError {
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-hang-diagnostics-\(UUID().uuidString)",
            isDirectory: true)
        try FileManager.default.createDirectory(
            at: url,
            withIntermediateDirectories: false)
