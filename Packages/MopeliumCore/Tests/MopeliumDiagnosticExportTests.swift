...
final class MopeliumDiagnosticExportTests: XCTestCase {
    func testEventLogRedactorPreservesDiagnosticStructureWithoutPrivateContent() throws {
        ...
        let snapshot = MopeliumDiagnosticFileSnapshot(
            data: Data((raw + "\n").utf8),
    ...
    func testEventLogRedactorKeepsNewestCompleteRecordsWhenOutputIsBounded() throws {
        ...
        let redacted = MopeliumDiagnosticEventLogRedactor.redact(
            snapshot,
    ...
    func testSnapshotReaderAcceptsOwnedReadOnlyFileAndRejectsLeafSymlink() throws {
        ...
        XCTAssertThrowsError(
            try MopeliumDiagnosticSnapshotReader.readTail(
                from: link,
                maximumBytes: 1_024)
        ) { error in
            XCTAssertEqual(
                error as? MopeliumDiagnosticSnapshotError,
                .unsafeFile)
        }
    }
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-diagnostic-export-tests-\(UUID().uuidString)",
            isDirectory: true)
