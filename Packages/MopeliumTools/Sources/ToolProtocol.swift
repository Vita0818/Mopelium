...
public struct ToolArgs: Sendable {
    ...
    public func decode<T: Decodable>(_ type: T.Type) throws -> T {
        guard let data = raw.data(using: .utf8) else {
            throw MopeliumError.decoding("tool args are not valid UTF-8")
        }
        do {
            return try JSONDecoder().decode(T.self, from: data)
        } catch {
            throw MopeliumError.decoding("tool args: \(error.localizedDescription)")
        }
    }
}
...
/// prevents clients outside MopeliumTools from manufacturing a process launch.
...
/// implementations resolve the executable from a fixed Mopelium runtime and
...
/// callers cannot turn this into a general shell: only Mopelium browser tools
...
struct BrowserBackendInvocation: Sendable {
    ...
    var injectedShellCommand: String {
        ...
        MOPELIUM_BROWSER_ARGS='\(encodedArguments)' node <<'MOPELIUM_BROWSER_INJECTED_NODE'
        \(javaScript)
        MOPELIUM_BROWSER_INJECTED_NODE
        """
    }
}
...
public struct ToolContext: Sendable {
    ...
    /// Dedicated backend for Mopelium-generated Playwright/CDP browser
...
public struct ToolRegistry: Sendable {
    ...
    public init(_ tools: [any Tool], registryVersion: String = "mopelium.standard.v1") {
        self.init(
    ...
    public static func standard(
        ...
        return ToolRegistry(
            registrations: registrations,
            registryVersion: "mopelium.standard.v8")
    }
}
