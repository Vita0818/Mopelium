...
public actor ProcessTerminalSessionManager: TerminalSessionManaging {
    ...
    public func execute(_ request: TerminalExecRequest,
        ...
        guard !isShutDown else {
            throw MopeliumError.config("the terminal manager is shut down")
        }
        pruneCompletionTombstones()
        guard sessions.count < maximumSessions else {
            throw MopeliumError.io("the terminal session limit (\(maximumSessions)) has been reached")
        }
        guard request.command.isEmpty == false else {
            throw MopeliumError.decoding("exec_command requires a non-empty command")
        }
        ...
        guard (250...30_000).contains(request.yieldMilliseconds) else {
            throw MopeliumError.decoding("yield_time_ms must be between 250 and 30000")
        }
        guard (1_000...1_800_000).contains(request.timeoutMilliseconds) else {
            throw MopeliumError.decoding("timeout_ms must be between 1000 and 1800000")
        }
        guard ["/bin/zsh", "/bin/bash", "/bin/sh"].contains(request.shellPath),
            ...
            throw MopeliumError.permissionDenied("the requested shell is not available")
        }
        ...
        guard lease.rootIdentity == owner.workspaceRootIdentity else {
            throw MopeliumError.permissionDenied("terminal owner does not match the workspace lease")
        }
        ...
        guard FileManager.default.fileExists(
            ...
            throw MopeliumError.notFound("terminal working directory does not exist")
        }

        let runtime = FileManager.default.temporaryDirectory
            .appendingPathComponent("mopelium-terminal-\(UUID().uuidString)", isDirectory: true)
        ...
        do {
            ...
            throw MopeliumError.io("could not create the terminal runtime directory")
        }
    ...
    public func interact(sessionID: String,
        ...
        guard (250...30_000).contains(request.yieldMilliseconds) else {
            throw MopeliumError.decoding("yield-time_ms must be between 250 and 30000")
        }
        pruneCompletionTombstones()
        guard var session = sessions[sessionID] else {
            ...
            throw MopeliumError.notFound("terminal session is not active")
        }
        guard session.owner == owner else {
            throw MopeliumError.permissionDenied("terminal session belongs to another agent or task")
        }
        ...
        if let characters = request.characters, characters.isEmpty == false {
            guard characters.utf8.count <= 32 * 1_024 else {
                throw MopeliumError.decoding("terminal input exceeds 32768 UTF-8 bytes")
            }
            ...
            guard !session.inputClosed, let descriptor = session.stdinDescriptor else {
                throw MopeliumError.io("terminal input is already closed")
            }
    ...
    private func launch(_ request: TerminalExecRequest,
        ...
        guard FileManager.default.createFile(atPath: startupMarker.path, contents: Data()) else {
            throw MopeliumError.io("could not create the terminal startup marker")
        }
        ...
        #else
        throw MopeliumError.config("terminal execution is unsupported on this platform")
        #endif
        ...
        if request.usesTTY {
            #if !os(macOS)
            throw MopeliumError.config("PTY mode is currently unavailable on this platform")
            #endif
        } else {
            var pipeDescriptors = [Int32](repeating: -1, count: 2)
            guard systemPipe(&pipeDescriptors) == 0 else {
                throw MopeliumError.io("could not create terminal input pipe")
            }
            ...
            guard systemPipe(&outputPipe) == 0 else {
                throw MopeliumError.io("could not create terminal output pipe")
            }
            ...
            guard systemPipe(&errorPipe) == 0 else {
                throw MopeliumError.io("could not create terminal error pipe")
            }
        ...
        guard let stdoutDrain else {
            killAndReapManagedProcess(pid)
            throw MopeliumError.io("could not start terminal output supervision")
        }
...
private func terminalEnvironment(home: URL, temporary: URL) -> [String: String] {
    ...
    environment["MOPELIUM_MANAGED_TERMINAL"] = "1"
    return environment
}
...
private func duplicateDescriptor(_ descriptor: Int32) throws -> Int32 {
    ...
    guard duplicate >= 0 else {
        throw MopeliumError.io("could not duplicate terminal output descriptor")
    }
    ...
    guard descriptorFlags >= 0,
        ...
        throw MopeliumError.io("could not isolate terminal output descriptor")
    }
    return duplicate
}
...
public struct ExecCommandTool: Tool {
    ...
    public func execute(_ args: ToolArgs,
        ...
        guard let terminal = context.terminal else {
            throw MopeliumError.config("this runtime does not own a terminal manager")
        }
