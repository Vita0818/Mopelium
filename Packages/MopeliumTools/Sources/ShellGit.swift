...
/// renderer boundary, so Mopelium instead preserves Chromium's native sandbox
...
func validatedWorkspace(_ cwd: URL) throws -> URL {
    ...
    guard FileManager.default.fileExists(atPath: workspace.path, isDirectory: &isDirectory),
        ...
        throw MopeliumError.io("shell workspace does not exist or is not a directory")
    }
    return workspace
}
...
func effectiveWorkspaceLease(_ candidate: WorkspaceLease?,
    ...
    guard leaseRoot.path == workspace.path else {
        throw MopeliumError.permissionDenied("workspace lease root does not match the managed process workspace")
    }
    guard let rootIdentity = lease.rootIdentity else {
        throw MopeliumError.permissionDenied("workspace lease is missing its reviewed root identity")
    }
    guard rootIdentity.canonicalPath == leaseRoot.path,
        ...
        throw MopeliumError.permissionDenied("workspace lease root identity changed after review")
    }
    guard allowEmptyPathRules || lease.allowedPathRules.isEmpty == false else {
        throw MopeliumError.permissionDenied("workspace lease process allow-list is empty")
    }
    for pattern in lease.allowedPathRules.map(\.pattern) + lease.deniedPatterns {
        ...
        guard normalized.isEmpty == false,
            ...
            throw MopeliumError.permissionDenied("workspace lease contains an unsafe process path rule")
        }
    }
    return lease
}
...
func documentProcessLease(
    ...
    for rawPath in internalWritablePaths {
        let resolved = try PathConfinement.resolve(rawPath, within: workspace)
        guard resolved.lastPathComponent.hasPrefix(".mopelium-document-stage-") else {
            throw MopeliumError.permissionDenied(
...
private func validateDocumentBackendArguments(
    ...
    guard invocation.arguments.allSatisfy({ !$0.contains("\0") }),
        ...
        throw MopeliumError.config("document backend invocation contains an unsafe argv or environment value")
    }
    let allowedEnvironment = Set([
        "MOPELIUM_DOCUMENT_REQUEST",
        "MOPELIUM_DOCUMENT_OPERATION",
        "PYTHONHASHSEED",
    ])
...
private func trustedDocumentExecutable(
    ...
    let runtime = mopeliumDocumentRuntimeRoot()
    let path: String?
    switch executable {
    case .pythonRuntime:
    ...
    case .libreOffice:
        #if os(macOS)
        path = mopeliumLibreOfficeRuntimeAppURL()?
...
private func runWorkspaceProcess(executable: URL,
    ...
    let runtime = FileManager.default.temporaryDirectory
        .appendingPathComponent("mopelium-process-\(UUID().uuidString)", isDirectory: true)
    ...
    guard FileManager.default.createFile(
        ...
        contents: Data()) else {
        throw MopeliumError.io("could not create managed process startup marker")
    }
    ...
    guard markerFlags >= 0,
        ...
        throw MopeliumError.io("could not protect managed process startup marker")
    }
    ...
    guard FileManager.default.isExecutableFile(atPath: "/usr/bin/sandbox-exec") else {
        throw MopeliumError.config("process execution is disabled because the macOS workspace sandbox is unavailable")
    }
    ...
    guard let bubblewrap = bubblewrapExecutable() else {
        throw MopeliumError.config("process execution is disabled because Bubblewrap is unavailable; install bwrap")
    }
...
private func runMacOSBrowserBackendProcess(
    ...
    let runtime = FileManager.default.temporaryDirectory
        .appendingPathComponent("mopelium-browser-process-\(UUID().uuidString)", isDirectory: true)
    let home = runtime.appendingPathComponent("home", isDirectory: true)
    let temporary = runtime.appendingPathComponent("tmp", isDirectory: true)
    try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
    try FileManager.default.createDirectory(at: temporary, withIntermediateDirectories: true)
    ...
    guard FileManager.default.createFile(
        ...
        throw MopeliumError.io("could not create the managed browser program")
    }

    var environment = sanitizedProcessEnvironment(home: home, temporary: temporary)
    environment["MOPELIUM_BROWSER_ARGS"] = invocation.encodedArguments
    environment["MOPELIUM_BROWSER_PROCESS_BOUNDARY"] = "1"
...
private func trustedNodeExecutable() throws -> URL {
    ...
    throw MopeliumError.config(
        "node is not installed in a trusted system location; install Node.js to use Mopelium browser tools")
}
#endif
...
private func makeManagedOutputPipe(maximumBytes: Int) throws -> ManagedOutputPipe {
    ...
    guard systemPipe(&descriptors) == 0 else {
        throw MopeliumError.io("could not create bounded process output pipe")
    }
...
private func runManagedProcess(spec: ManagedProcessSpec,
    ...
    case .timedOut:
        throw MopeliumError.io("shell command timed out after \(timeoutSeconds.formattedForError)s")
    case .resourceLimit(.generatedOutput):
        throw MopeliumError.io("document backend exceeded its generated output budget")
    case .resourceLimit(.residentMemory):
        throw MopeliumError.io("document backend exceeded its resident-memory budget")
    }
}
...
func limitedExecutionArguments(executable: URL,
    ...
    return [
        ...
        "mopelium-managed", startupMarker.path, executable.path,
    ] + arguments
}
...
func managedTerminalExecutionArguments(executable: URL,
    ...
    [
        ...
        "mopelium-managed-terminal", startupMarker.path, executable.path,
    ] + arguments
}
...
func structuredRuntimeReadRoots() -> [URL] {
    ...
    if let documentRuntime = mopeliumDocumentRuntimeRoot(),
...
/// user-managed location as a development fallback; Mopelium never installs
/// into or mutates that directory while running a tool.
func mopeliumDocumentRuntimeRoot() -> URL? {
    let bundled = mopeliumBundledDocumentRuntimeRoot()
    #if os(macOS)
    ...
    return selectDocumentRuntimeRoot(
        ...
        requiresBundledRuntime: Bundle.main.bundleIdentifier == "com.Vita0818.Mopelium")
    #elseif os(Linux)
    let canonical = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".local/share/mopelium/document-runtime", isDirectory: true)
    let legacy = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".local/share/intatis/document-runtime", isDirectory: true)
    let userManaged = FileManager.default.fileExists(atPath: canonical.path)
        || !FileManager.default.fileExists(atPath: legacy.path)
        ? canonical : legacy
    return selectDocumentRuntimeRoot(
        bundledRoot: bundled,
...
func mopeliumBundledDocumentRuntimeRoot() -> URL? {
    #if os(macOS)
    #if arch(arm64)
...
#if os(macOS)
func mopeliumLibreOfficeRuntimeAppURL() -> URL? {
    mopeliumDocumentRuntimeRoot()?
...
func macOSSandboxProfile(workspace: URL,
    ...
    case (.libreOfficeHeadless, .some(let socketRoot)):
        ...
        let suffix = String(name.dropFirst("mopelium-lo-".count))
        let hasExpectedParent = canonical.deletingLastPathComponent().path == "/private/tmp"
        let hasExpectedPrefix = name.hasPrefix("mopelium-lo-")
    ...
    case .libreOfficeHeadless:
        ...
        guard let libreOfficeApp = mopeliumLibreOfficeRuntimeAppURL() else {
            throw MopeliumError.config(
    ...
    case (.denied, .libreOfficeHeadless):
        guard let validatedLibreOfficeSocketRoot else {
            throw MopeliumError.config("LibreOffice socket root is unavailable")
        }
...
private func createLibreOfficeSocketRoot() throws -> URL {
    ...
    let suffix = UUID().uuidString
        ...
        .lowercased()
        .prefix(12)
    let root = parent.appendingPathComponent("mopelium-lo-\(suffix)", isDirectory: true)
    do {
        ...
        throw MopeliumError.io("could not create LibreOffice socket root")
    }
    var status = stat()
...
func bubblewrapArguments(workspace: URL,
    ...
    if allowsWholeWorkspace == false {
        addParents(of: workspace.path + "/.mopelium-placeholder")
...
func spawnManagedProcess(spec: ManagedProcessSpec,
    ...
    guard posix_spawn_file_actions_init(&actions) == 0,
        ...
        throw MopeliumError.io("could not initialize managed process attributes")
    }
    defer {
    ...
    if let stdinDescriptor {
        ...
        guard descriptor >= 0 else { throw MopeliumError.io("could not open null stdin") }
    ...
    guard posix_spawn_file_actions_adddup2(
        ...
        throw MopeliumError.io("could not configure managed process file descriptors")
    }
    #if os(macOS)
    guard posix_spawn_file_actions_addchdir(&actions, cwd.path) == 0 else {
        throw MopeliumError.io("could not confine managed process working directory")
    }
    #endif
    ...
    guard posix_spawnattr_setflags(&attributes, flags) == 0,
        ...
        throw MopeliumError.io("could not configure managed process isolation")
    }
    ...
    guard result == 0 else {
        throw MopeliumError.io("managed process launch failed: \(String(cString: strerror(result)))")
    }
    guard getpgid(pid) == pid else {
        _ = kill(pid, SIGKILL)
        _ = waitpid(pid, nil, 0)
        throw MopeliumError.io("managed process did not start in an isolated process group")
    }
    return pid
}
...
func spawnManagedPTYProcess(spec: ManagedProcessSpec,
    ...
    var spawned = MopeliumPTYSpawnResult(
        pid: -1,
        master_fd: -1,
        error_stage: Int32(MOPELIUM_PTY_STAGE_NONE),
        error_number: 0)
    let result: Int32 = withMutableCStrings(argv) { argvPointer in
        withMutableCStrings(envp) { envPointer in
            mopelium_spawn_managed_pty(
    ...
    guard result == 0, spawned.pid > 0, spawned.master_fd >= 0 else {
        ...
        case Int32(MOPELIUM_PTY_STAGE_CHDIR):
            stage = "working-directory setup"
        case Int32(MOPELIUM_PTY_STAGE_EXEC):
...
private func gitWorkspaceLayout(_ workspace: URL) throws -> GitWorkspaceLayout {
    ...
    guard FileManager.default.fileExists(atPath: dotGit.path, isDirectory: &isDirectory) else {
        throw MopeliumError.permissionDenied("git repository metadata must be inside the agent workspace")
    }
    let dotGitValues = try dotGit.resourceValues(forKeys: [.isSymbolicLinkKey])
    guard dotGitValues.isSymbolicLink != true else {
        throw MopeliumError.permissionDenied("git metadata symlink escapes are not allowed")
    }
    if isDirectory.boolValue {
        ...
        guard resolved == root.path || resolved.hasPrefix(prefix) else {
            throw MopeliumError.permissionDenied("git metadata directory escapes the agent workspace")
        }
    ...
    guard data.count <= 4_096,
        ...
        throw MopeliumError.permissionDenied("git worktree metadata pointer is invalid")
    }
    ...
    guard let markerRange = target.path.range(of: marker, options: .backwards) else {
        throw MopeliumError.permissionDenied("git worktree metadata must stay under the owning workspace repository")
    }
    ...
    let expectedWorkspace = owner
        ...
        .resolvingSymlinksInPath()
        .standardizedFileURL
    let legacyExpectedWorkspace = owner
        .appendingPathComponent(".intatis", isDirectory: true)
        .appendingPathComponent("git-worktrees", isDirectory: true)
        .appendingPathComponent(worktreeSuffix, isDirectory: true)
        .resolvingSymlinksInPath()
        .standardizedFileURL
    guard expectedWorkspace.path == root.path
            || legacyExpectedWorkspace.path == root.path else {
        throw MopeliumError.permissionDenied(
            "git linked worktree must use .mopelium/git-worktrees under its owning workspace")
    }
    ...
    guard target.path == expectedTargetPrefix || target.path.hasPrefix(expectedTargetPrefix + "/") else {
        throw MopeliumError.permissionDenied("git worktree metadata pointer does not match its owning workspace")
    }
    return GitWorkspaceLayout(
        workspace: root,
...
public struct ProcessGitService: GitService {
    ...
    public func pullFastForward(remote: String, branch: String, workspace: URL) async throws -> String {
        ...
        guard current == safeBranch else {
            throw MopeliumError.permissionDenied("git_pull_ff branch must match current branch (\(current))")
        }
    ...
    public func push(remote: String, branch: String, setUpstream: Bool, workspace: URL) async throws -> String {
        ...
        let current = try await currentBranch(workspace: workspace)
        guard current == safeBranch else {
            throw MopeliumError.permissionDenied("git_push branch must match current branch (\(current))")
        }
        var args = ["push"]
        if setUpstream {
            args.append("-u")
    ...
    private func validateRepository(workspace: URL) async throws {
        ...
        guard repoRoot == workspacePath else {
            throw MopeliumError.permissionDenied("git repository root must match the agent workspace root")
        }
    ...
    private func validateRepositoryConfiguration(workspace: URL) async throws {
        ...
        guard result.exitCode == 0 else {
            throw MopeliumError.permissionDenied("git repository config could not be inspected safely")
        }
    ...
    private func validateRepositoryConfigFile(_ layout: GitWorkspaceLayout) throws {
        for configURL in layout.configURLs {
            ...
            guard (entry.st_mode & mode_t(S_IFMT)) != mode_t(S_IFLNK) else {
                throw MopeliumError.permissionDenied("git repository config symlinks are not allowed")
            }
            ...
            guard isConfined else {
                throw MopeliumError.permissionDenied("git repository config escapes approved metadata roots")
            }
            let byteCount = Int(entry.st_size)
            guard byteCount >= 0, byteCount <= 1_048_576 else {
                throw MopeliumError.permissionDenied("git repository config is too large to inspect safely")
            }
            let data = try Data(contentsOf: configURL, options: [.mappedIfSafe])
            guard let text = String(data: data, encoding: .utf8) else {
                throw MopeliumError.permissionDenied("git repository config is not valid UTF-8")
            }
            var section = ""
            var unsafe: [String] = []
    ...
    private func runRawGit(_ args: [String],
        ...
        guard let executable = gitExecutable() else {
            throw MopeliumError.config("git is unavailable in a trusted fixed runtime path")
        }
        ...
        if checked, result.exitCode != 0 {
            ...
            throw MopeliumError.io("git \(command) failed (\(renderedArgs)): \(message)")
        }
        return result
        #else
        throw MopeliumError.io("git execution is unavailable on this platform")
        #endif
    }
    ...
    private func ensureRemoteExists(_ remote: String, workspace: URL) async throws {
        guard try await remoteNames(workspace: workspace).contains(remote) else {
            throw MopeliumError.decoding("git remote is not configured: \(remote)")
        }
    }
    ...
    private func currentBranch(workspace: URL) async throws -> String {
        ...
        guard !current.isEmpty else {
            throw MopeliumError.permissionDenied("git operation requires a checked-out local branch")
        }
        return current
    }
    ...
    private func ensureCleanWorktree(workspace: URL, operation: String) async throws {
        ...
        guard GitStatus.parse(statusText).isEmpty else {
            throw MopeliumError.permissionDenied("\(operation) requires a clean working tree")
        }
    }
...
private enum GitToolInput {
    static func normalizedPaths(_ paths: [String], workspace: URL) throws -> [String] {
        guard !paths.isEmpty else {
            throw MopeliumError.decoding("git paths must not be empty")
        }
        return try paths.map { rawPath in
            let path = rawPath.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !path.isEmpty else {
                throw MopeliumError.decoding("git paths must not contain empty entries")
            }
    ...
    static func branchName(_ raw: String) throws -> String {
        ...
        guard !name.isEmpty else {
            throw MopeliumError.decoding("branch name must not be empty")
        }
        guard name.count <= 255 else {
            throw MopeliumError.decoding("branch name is too long")
        }
        let forbidden = ["..", "@{", "\\", "~", "^", ":", "?", "*", "[", "\n", "\r", "\t"]
        guard !forbidden.contains(where: { name.contains($0) }),
              ...
              !name.contains("//"),
              name != "@" else {
            throw MopeliumError.decoding("branch name contains characters Git refs cannot safely use")
        }
        return name
    }
    ...
    static func remoteName(_ raw: String) throws -> String {
        let name = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !name.isEmpty else {
            throw MopeliumError.decoding("remote name must not be empty")
        }
        guard name.count <= 80 else {
            throw MopeliumError.decoding("remote name is too long")
        }
        ...
        guard name.unicodeScalars.allSatisfy({ allowed.contains($0) }),
              ...
              !name.hasPrefix("."),
              !name.contains("..") else {
            throw MopeliumError.decoding("remote must be a configured remote name, not a URL or refspec")
        }
        return name
    }
    ...
    static func optionalRef(_ raw: String?) throws -> String? {
        ...
        guard value.count <= 255,
              ...
              !value.contains("\r"),
              !value.contains("\t") else {
            throw MopeliumError.decoding("git start point is not a safe ref")
        }
        return value
    }
    ...
    static func requiredRef(_ raw: String) throws -> String {
        guard let value = try optionalRef(raw) else {
            throw MopeliumError.decoding("git ref must not be empty")
        }
        return value
    }
    ...
    static func worktreeName(_ raw: String) throws -> String {
        let name = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !name.isEmpty else {
            throw MopeliumError.decoding("worktree name must not be empty")
        }
        guard name.count <= 80 else {
            throw MopeliumError.decoding("worktree name is too long")
        }
        let allowed = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "._-"))
        guard name.unicodeScalars.allSatisfy({ allowed.contains($0) }),
              !name.hasPrefix("."),
              !name.hasPrefix("-"),
              !name.contains("..") else {
            throw MopeliumError.decoding("worktree name must be a simple safe directory name")
        }
        return name
    }
    ...
    private static func patchPaths(_ diff: String) throws -> [String] {
        guard !diff.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            throw MopeliumError.decoding("git patch must not be empty")
        }
        var result: [String] = []
        var seen = Set<String>()
        ...
        guard !result.isEmpty else {
            throw MopeliumError.decoding("git patch does not expose any changed paths")
        }
        return result
    }
...
public struct GitCommitTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard !message.isEmpty else {
            throw MopeliumError.decoding("commit message must not be empty")
        }
        ...
        if let sensitive = stagedEntries.first(where: { PathConfinement.isSensitivePath($0.path) }) {
            throw MopeliumError.permissionDenied("refusing to commit staged sensitive path: \(sensitive.path)")
        }
...
public struct GitRevertPatchTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard a.confirmRevert else {
            throw MopeliumError.permissionDenied("git_revert_patch requires confirmRevert=true")
        }
        _ = try GitToolInput.normalizedPatchPaths(a.diff, workspace: context.workspaceRoot)
        let result = try await context.git.applyPatch(diff: a.diff,
                                                       reverse: true,
...
public struct GitWorktreeRemoveTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard name == confirmName else {
            throw MopeliumError.permissionDenied("git_worktree_remove confirmName must match name")
        }
...
public struct GitPullFastForwardTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard remote == confirmRemote, branch == confirmBranch else {
            throw MopeliumError.permissionDenied("git_pull_ff confirmation must match remote and branch")
        }
...
public struct GitPushTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        let confirmBranch = try GitToolInput.branchName(a.confirmBranch)
        guard remote == confirmRemote, branch == confirmBranch else {
            throw MopeliumError.permissionDenied("git_push confirmation must match remote and branch")
        }
...
public struct GitSwitchBranchTool: Tool {
    ...
    public func execute(_ args: ToolArgs, in context: ToolContext) async throws -> ToolObservation {
        ...
        guard branch == confirmBranch else {
            throw MopeliumError.permissionDenied("git_switch confirmBranch must match branch")
        }
