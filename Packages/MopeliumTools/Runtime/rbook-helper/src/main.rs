...
fn run() -> HelperResult<Value> {
    ...
    let encoded = env::var("MOPELIUM_DOCUMENT_REQUEST").map_err(|_| HelperError::validation())?;
    ...
    let operation_env =
        env::var("MOPELIUM_DOCUMENT_OPERATION").map_err(|_| HelperError::validation())?;
...
fn set_metadata(epub: &mut Epub, field: &str, value: &str) -> HelperResult<()> {
    ...
    if field == "identifier" {
        let mut id = String::from("mopelium_unique_identifier");
        let mut suffix = 1_usize;
        while package_id_in_use(epub, &id) {
            id = format!("mopelium_unique_identifier_{suffix}");
            suffix += 1;
        }
        entry = entry.id(&id);
...
fn unique_toc_id(epub: &Epub, operation_index: usize) -> String {
    let base = format!("mopelium_toc_{operation_index}");
...
mod tests {
    ...
    fn create_payload(directory: &TestDirectory, output: &Path) -> WritePayload {
        ...
        WritePayload {
            ...
            operations: vec![
                metadata("identifier", "urn:mopelium:test"),
