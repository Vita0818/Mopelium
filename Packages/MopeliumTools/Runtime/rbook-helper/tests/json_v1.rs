...
fn invoke(operation: &str, request: Value) -> Value {
    let output = Command::new(env!("CARGO_BIN_EXE_mopelium-rbook-helper"))
        .arg("json-v1")
        .env("MOPELIUM_DOCUMENT_OPERATION", operation)
        .env("MOPELIUM_DOCUMENT_REQUEST", request.to_string())
        .output()
        .expect("run helper");
