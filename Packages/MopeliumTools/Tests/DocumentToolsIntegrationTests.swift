...
final class DocumentToolsIntegrationTests: XCTestCase {
    ...
    func testShippingDocumentRuntimeSelectionNeverFallsBackToUserManagedRoot() {
        let bundled = URL(fileURLWithPath: "/Applications/Mopelium.app/Contents/Resources/DocumentRuntime/arm64")
        let userManaged = URL(fileURLWithPath: "/Users/example/Library/Application Support/Mopelium/document-runtime")
    ...
    func testInstalledDocumentRuntimeExactDOCXChainWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["MOPELIUM_REAL_DOCUMENT_RUNTIME_SMOKE"] == "1" else {
            throw XCTSkip(
                "set MOPELIUM_REAL_DOCUMENT_RUNTIME_SMOKE=1 to run the installed document runtime smoke")
        }
        guard let runtime = mopeliumDocumentRuntimeRoot(),
              let libreOffice = mopeliumLibreOfficeRuntimeAppURL()?
        ...
        let marker = "MOPELIUM EXACT DOCUMENT RUNTIME 73921"
    ...
    func testInstalledDocumentRuntimeReadsUserCorpusWhenConfigured() async throws {
        guard let rawRoot = ProcessInfo.processInfo.environment[
            "MOPELIUM_REAL_DOCUMENT_CORPUS_ROOT"],
              !rawRoot.isEmpty else {
            throw XCTSkip(
                "set MOPELIUM_REAL_DOCUMENT_CORPUS_ROOT to run the external document corpus smoke")
        }
        guard let runtime = mopeliumDocumentRuntimeRoot(),
              fileManager.isExecutableFile(
    ...
    func testInstalledDocumentRuntimePDFCPUAndOCRWhenEnabled() async throws {
        ...
        let stage = workspace.appendingPathComponent(
            ".mopelium-document-stage-runtime-smoke",
            isDirectory: true)
        ...
        let digest = try DocumentInputFile.freeze(
            path: ".mopelium-document-stage-runtime-smoke/ocr-source.pdf",
            expectedFormat: .pdf,
            workspace: workspace).identity.sha256
        let observation = try await OCRPDFTool().execute(
            ToolArgs(raw: """
            {"input_path":".mopelium-document-stage-runtime-smoke/ocr-source.pdf",
        ...
        XCTAssertTrue(normalized.contains("MOPELIUMOCR48291"), observation.text)
        #else
    ...
    func testPDFRenderCommitsOnePNGWithoutLeakingStageDirectory() async throws {
        ...
        XCTAssertFalse(rootChildren.contains { $0.hasPrefix(".mopelium-document-stage-") })
        #else
        throw XCTSkip("PDFKit integration requires Apple PDF frameworks")
        #endif
    }
    ...
    func testDocumentProcessLeaseNarrowsBroadWorkspaceAuthorityToReviewedInputAndStage() throws {
        ...
        let narrowed = try documentProcessLease(
            ...
            internalWritablePaths: ["out/.mopelium-document-stage-test"])

        XCTAssertEqual(narrowed.access, .readWrite)
        XCTAssertEqual(
            Set(narrowed.allowedPathRules.map(\.pattern)),
            ["source.docx", "out/.mopelium-document-stage-test"])
        ...
        XCTAssertTrue(profile.contains("mopelium-document-stage-test"))
    ...
    func testLibreOfficeSandboxAllowsOnlyInvocationPrivateUnixSocket() throws {
        ...
        defer { try? fileManager.removeItem(at: workspace) }
        let socketRoot = URL(
            fileURLWithPath: "/private/tmp/mopelium-lo-0123456789ab",
            isDirectory: true)
        ...
        XCTAssertTrue(profile.contains(#"(subpath "/private/tmp/mopelium-lo-0123456789ab")"#))
    ...
    private func makeWorkspace() throws -> URL {
        let url = fileManager.temporaryDirectory.appendingPathComponent(
            "mopelium-document-tool-integration-\(UUID().uuidString)",
            isDirectory: true)
