...
private actor ShellResultQueue {
    ...
    func next() throws -> ShellResult {
        guard results.isEmpty == false else {
            throw MopeliumError.io("no fake shell results remain")
        }
...
final class MopeliumToolsTests: XCTestCase {
    ...
    private func browserPayload(from command: String) throws -> [String: Any] {
        ...
        guard let markerRange = command.range(of: marker) else {
            throw MopeliumError.decoding("missing browser args payload")
        }
        let payloadStart = markerRange.upperBound
        guard let payloadEnd = command[payloadStart...].firstIndex(of: "'") else {
            throw MopeliumError.decoding("unterminated browser args payload")
        }
        let encoded = String(command[payloadStart..<payloadEnd])
        guard let data = Data(base64Encoded: encoded),
              let object = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw MopeliumError.decoding("browser args payload is not JSON")
        }
        return object
    }
    ...
    private func makeBlankPDF(pageCount: Int, at url: URL) throws {
        ...
        for index in 0..<pageCount {
            ...
            guard let page = PDFPage(image: image) else {
                throw MopeliumError.io("could not create test PDF page \(index)")
            }
            document.insert(page, at: index)
        }
        guard document.write(to: url) else {
            throw MopeliumError.io("could not write test PDF")
        }
    }
    #endif
    ...
    private func freeLoopbackPort() throws -> Int {
        ...
        guard descriptor >= 0 else {
            throw MopeliumError.io("could not create loopback socket")
        }
        ...
        guard bindResult == 0 else {
            throw MopeliumError.io("could not bind loopback socket")
        }
        ...
        guard nameResult == 0 else {
            throw MopeliumError.io("could not inspect loopback socket")
        }
    ...
    private func startStaticHTTPServer(directory: URL, port: Int) throws -> Process {
        guard let python = python3Executable() else {
            throw MopeliumError.io("python3 is required for the real browser persistence smoke")
        }
    ...
    private func waitForHTTPServer(port: Int, path: String = "/login.html") async throws {
        ...
        throw MopeliumError.io("local browser smoke server did not start")
    }
    #endif
    ...
    func testOrdinaryFileToolsCannotBypassManagedKnowledgePublication()
        ...
        let snapshots = store.appendingPathComponent(
            ".mopelium-rag-snapshots",
            isDirectory: true)
        try FileManager.default.createDirectory(
            at: snapshots,
            withIntermediateDirectories: true)
        let pointer = store.appendingPathComponent(".mopelium-rag-store.json")
        ...
        do {
            _ = try await ReadFileTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.mopelium-rag-store.json"}"#),
                in: context)
        ...
        do {
            _ = try await ListFilesTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.mopelium-rag-snapshots"}"#),
                in: context)
        ...
        do {
            _ = try await SearchTextTool().execute(
                ToolArgs(raw: #"{"query":"snapshot","path":"knowledge/.mopelium-rag-snapshots"}"#),
                in: context)
        ...
        do {
            _ = try await WriteFileTool().execute(
                ToolArgs(raw: #"{"path":"knowledge/.mopelium-rag-store.json","content":"changed"}"#),
                in: context)
        ...
        let diff = [
            "--- a/knowledge/.mopelium-rag-snapshots/profile.json",
            "+++ b/knowledge/.mopelium-rag-snapshots/profile.json",
            "@@ -1 +1 @@",
            "-snapshot-original",
            "+changed",
        ].joined(separator: "\n")
    ...
    func testStructuredProcessShellRunnerUsesSanitizedEnvironmentAndWorkspaceLease() async throws {
        ...
        setenv("MOPELIUM_HOST_SECRET_MARKER", "host-secret-marker", 1)
        defer { unsetenv("MOPELIUM_HOST_SECRET_MARKER") }
        let runner = StructuredProcessShellRunner(timeoutSeconds: 5)

        let ordinary = try await runner.run("/bin/cat ordinary.txt; printf '|%s' \"$MOPELIUM_HOST_SECRET_MARKER\"", cwd: ws)
    ...
    func testBrowserBackendProcessRunnerUsesSanitizedEnvironment() async throws {
        let ws = try tempWorkspace()
        defer { try? FileManager.default.removeItem(at: ws) }
        setenv("MOPELIUM_BROWSER_HOST_SECRET_MARKER", "host-secret-marker", 1)
        defer { unsetenv("MOPELIUM_BROWSER_HOST_SECRET_MARKER") }

        let invocation = BrowserBackendInvocation(
            javaScript: #"process.stdout.write(`${process.env.MOPELIUM_BROWSER_HOST_SECRET_MARKER || ""}|${process.env.MOPELIUM_BROWSER_PROCESS_BOUNDARY || ""}`);"#,
    ...
    func testProcessShellRunnerDeniesWorkspaceExternalFile() async throws {
        ...
        let outside = ws.deletingLastPathComponent()
            .appendingPathComponent("mopelium-outside-\(UUID().uuidString).txt")
        defer {
            try? FileManager.default.removeItem(at: ws)
        ...
        let escapedWrite = ws.deletingLastPathComponent()
            .appendingPathComponent("mopelium-escaped-write-\(UUID().uuidString).txt")
    ...
    func testProcessGitServiceRejectsExternalDiffWithoutExecutingIt() async throws {
        ...
        let outsideConfig = ws.deletingLastPathComponent().appendingPathComponent("mopelium-included-\(UUID().uuidString).gitconfig")
    ...
    func testProcessGitServiceAuditsWorktreeConfigAndRejectsSymlink() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("mopelium-config-worktree-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
        }
    ...
    func testProcessGitServiceHonorsReadOnlyAndDeniedLeaseData() async throws {
        ...
        let setup = try await shell.run(
            "'\(git)' init; '\(git)' config user.email mopelium@example.invalid; '\(git)' config user.name Mopelium; '\(git)' add ordinary.txt .env; '\(git)' commit -m base",
            cwd: ws)
        XCTAssertEqual(setup.exitCode, 0, setup.stderr)
    ...
    func testProcessGitServiceStagesAndCommitsTempRepoWhenGitAvailable() async throws {
        #if canImport(Darwin)
        guard ProcessInfo.processInfo.environment["MOPELIUM_REAL_GIT_SMOKE"] == "1" else {
            throw XCTSkip("set MOPELIUM_REAL_GIT_SMOKE=1 to run the real ProcessGitService smoke")
        }
        ...
        _ = try await shell.run("git config user.email mopelium@example.invalid", cwd: patchSource)
        _ = try await shell.run("git config user.name 'Mopelium Test'", cwd: patchSource)
        ...
        let worktreeURL = ws.appendingPathComponent(".mopelium/git-worktrees/task-1", isDirectory: true)
        ...
        XCTAssertTrue(worktreeInfo.contains("/.mopelium/git-worktrees/task-1"), worktreeInfo)
    ...
    func testCompileLatexCannotReadOutsideWorkspace() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("mopelium-tex-outside-\(UUID().uuidString).tex")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
        }
    ...
    func testRealCDPBrowserIgnoresStaleDevToolsActivePortWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment["MOPELIUM_REAL_BROWSER_SMOKE"] == "1" else {
            throw XCTSkip("set MOPELIUM_REAL_BROWSER_SMOKE=1 to run the stale DevToolsActivePort smoke")
        }
        ...
        let profileDirectory = ws
            .appendingPathComponent(".mopelium/browser/profiles/stale-port-smoke", isDirectory: true)
        try FileManager.default.createDirectory(
            at: profileDirectory,
            withIntermediateDirectories: true)
    ...
    func testBrowserExecutionRejectsReadOnlyLeaseBeforeCreatingState() async throws {
        ...
        XCTAssertFalse(
            FileManager.default.fileExists(
                atPath: ws.appendingPathComponent(".mopelium").path))
    }
    ...
    func testBrowserExecutionRejectsNarrowOrDeniedManagedPathsBeforeCreatingState() async throws {
        ...
        let narrowLease = WorkspaceLease(
            ...
            access: .readWrite,
            allowedPathRules: [
                PathRule(pattern: ".mopelium/browser/profiles/**"),
            ],
            deniedPatterns: [])
    ...
    func testBrowserDiagnosticsIgnoresWorkspacePlaywrightModule() async throws {
        ...
        let ws = try tempWorkspace()
        let outside = ws.deletingLastPathComponent().appendingPathComponent("mopelium-node-outside-\(UUID().uuidString).txt")
        defer {
            try? FileManager.default.removeItem(at: ws)
            try? FileManager.default.removeItem(at: outside)
            unsetenv("MOPELIUM_NODE_HOST_MARKER")
        }
        try Data("outside-node-marker".utf8).write(to: outside)
        setenv("MOPELIUM_NODE_HOST_MARKER", "host-node-marker", 1)
        ...
        fs.writeFileSync('workspace-playwright-loaded', `${process.env.MOPELIUM_NODE_HOST_MARKER || ''}|${outside}`);
    ...
    func testStandardRegistry() {
        ...
        XCTAssertEqual(reg.registryVersion, "mopelium.standard.v8")
