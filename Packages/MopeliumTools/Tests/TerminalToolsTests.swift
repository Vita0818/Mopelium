...
final class TerminalToolsTests: XCTestCase {
    private func workspace() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("mopelium-terminal-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testManagedTerminalTTYRejectsCursorEditingBeforeDangerousCommand() async throws {
        ...
        _ = try await manager.interact(
            sessionID: sessionID,
            request: TerminalInteractionRequest(
                characters: "m -rf -- MOPELIUM_NONEXISTENT_TARGET",
                yieldMilliseconds: 250),
            owner: owner)
        do {
            _ = try await manager.interact(
    ...
    func testManagedTerminalPreservesToolchainPathButFiltersSecretEnvironment() async throws {
        ...
        let owner = try owner(workspace: workspace)
        let lease = WorkspaceLease(rootPath: workspace.path, access: .readWrite)
        setenv("MOPELIUM_TERMINAL_TEST_TOKEN", "must-not-reach-child", 1)
        setenv("DATABASE_URL", "postgres://must-not-reach-child", 1)
        defer {
            unsetenv("MOPELIUM_TERMINAL_TEST_TOKEN")
        ...
        let result = try await manager.execute(
            TerminalExecRequest(
                command: """
                if [ -z "$MOPELIUM_TERMINAL_TEST_TOKEN" ] && [ -z "$DATABASE_URL" ]; then
    ...
    func testManagedTerminalCannotMutateKnowledgePublicationWithEmptyDenyList()
        ...
        let result = try await manager.execute(
            TerminalExecRequest(
                command: """
                printf changed > knowledge/.mopelium-rag-store.json 2>/dev/null || true
                printf changed > knowledge/.mopelium-rag-snapshots/profile.json 2>/dev/null || true
                printf changed > knowledge/.mopelium-rag-host/store.lock 2>/dev/null || true
                """,
                loginShell: false,
                yieldMilliseconds: 1_000,
                timeoutMilliseconds: 5_000),
