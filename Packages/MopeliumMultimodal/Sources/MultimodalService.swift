...
public actor MultimodalService {
    ...
    public func generateImage(using provider: ImageGenerationProvider,
        ...
        guard !images.isEmpty else { throw MopeliumError.provider("no image returned") }
    ...
    public func generateVideo(using provider: VideoGenerationProvider,
        ...
        for _ in 0..<maxPolls {
            ...
            switch status.state {
            case .completed:
                guard let data = status.resultData else { throw MopeliumError.provider("video completed without data") }
                ...
                return ref
            case .failed:
                throw MopeliumError.provider("video generation failed")
            case .queued, .running:
                try? await Task.sleep(nanoseconds: pollInterval)
            }
        }
        throw MopeliumError.provider("video generation timed out")
    }
