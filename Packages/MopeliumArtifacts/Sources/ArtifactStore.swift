...
public actor ArtifactStore {
    ...
    public init(root: URL) throws {
        ...
        guard canonicalBlobs.deletingLastPathComponent().standardizedFileURL.path
            ...
            throw MopeliumError.io("artifact blobs directory is outside the store root")
        }
    ...
    public func data(for id: ArtifactID) throws -> Data {
        guard let ref = index[id] else { throw MopeliumError.notFound("artifact \(id)") }
        let blobURL = try Self.validatedBlobURL(for: ref, root: root)
        guard let data = try Self.readArtifactFile(
            from: blobURL) else {
            throw MopeliumError.notFound("artifact \(id)")
        }
        return data
    }
    ...
    private static func decodeIndex(
        ...
        do {
            ...
            throw MopeliumError.io("artifact index is invalid: \(error.localizedDescription)")
        }

        var result: [ArtifactID: ArtifactRef] = [:]
        for ref in refs {
            guard result[ref.id] == nil else {
                throw MopeliumError.io("artifact index contains a duplicate id")
            }
    ...
    private static func validatedBlobURL(
        ...
        guard components.count == 2,
            ...
            throw MopeliumError.io("artifact index contains an unsafe blob path")
        }
        ...
        guard liveBlobs.path == expectedBlobs.path else {
            throw MopeliumError.io("artifact blobs directory identity changed")
        }
