...
@main
private enum MopeliumMCPConformanceClient {
    static func main() async {
        do {
            ...
            FileHandle.standardError.write(
                Data("Mopelium MCP conformance client failed: \(summary)\n".utf8))
            exit(EXIT_FAILURE)
        }
    }
}
...
private struct Runner {
    ...
    func run() async throws {
        ...
        case "mopelium/task-complete":
            try await runTaskAugmented(.complete)
        case "mopelium/task-timeout":
            try await runTaskAugmented(.timeout)
        case "mopelium/task-cancel":
    ...
    private func makeSession(
        ...
        return MCPClientSession(
            configuration: MCPClientSessionConfiguration(
                ...
                clientName: "mopelium-conformance-client",
                clientVersion: "1.0.0",
    ...
    static func make() async throws -> ConformanceResources {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-mcp-conformance-\(UUID().uuidString)",
                isDirectory: true)
        ...
        try await store.initialize(
            passphrase: Data(
                "mopelium-conformance-disposable-key".utf8))
...
private struct OAuthResources {
    ...
    static func make(
        ...
        try await store.initialize(
            passphrase: Data(
                "mopelium-oauth-conformance-key".utf8))
