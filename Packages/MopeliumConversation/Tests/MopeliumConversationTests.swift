...
final class MopeliumConversationTests: XCTestCase {

    private func tmpFile() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("mopelium-conv-\(UUID().uuidString)", isDirectory: true)
        ...
        let loop = ChatLoop(
            ...
            attachmentResolver: { ids in
                guard ids == [firstID] else {
                    throw MopeliumError.notFound("unexpected attachment set")
                }
                return [firstImage]
            })
