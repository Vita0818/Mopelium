...
} MopeliumPTYChildError;

static int mopelium_set_cloexec(int descriptor) {
...
static int mopelium_maximum_descriptor(void) {
    struct rlimit limit;
...
static void mopelium_report_child_error(
    int descriptor,
    int32_t stage,
    int32_t error_number,
    int exit_code) {
    MopeliumPTYChildError payload = {
        .stage = stage,
...
static void mopelium_reset_child_signals(void) {
    sigset_t empty_mask;
...
static void mopelium_kill_and_reap(pid_t pid) {
...
int32_t mopelium_spawn_managed_pty(
    ...
    *result = (MopeliumPTYSpawnResult){
        .pid = -1,
        .master_fd = -1,
        .error_stage = MOPELIUM_PTY_STAGE_PREPARE,
        .error_number = 0
    };
    ...
    int pipe_error = mopelium_set_cloexec(error_pipe[0]);
    if (pipe_error == 0) {
        pipe_error = mopelium_set_cloexec(error_pipe[1]);
    }
    if (pipe_error != 0) {
    ...
    int maximum_descriptor = mopelium_maximum_descriptor();
    ...
    if (pid == 0) {
        ...
        if (error_descriptor != 3) {
            if (dup2(error_descriptor, 3) == -1) {
                mopelium_report_child_error(
                    error_descriptor,
                    MOPELIUM_PTY_STAGE_PREPARE,
                    errno,
                    126);
            }
        ...
        int cloexec_error = mopelium_set_cloexec(error_descriptor);
        if (cloexec_error != 0) {
            mopelium_report_child_error(
                error_descriptor,
                MOPELIUM_PTY_STAGE_PREPARE,
        ...
        mopelium_report_child_error(
            error_descriptor,
            MOPELIUM_PTY_STAGE_EXEC,
            errno,
            127);
    }

    (void)close(error_pipe[1]);
    MopeliumPTYChildError child_error = {
        .stage = MOPELIUM_PTY_STAGE_NONE,
        .error_number = 0
    };
    ...
    int master_error = mopelium_set_cloexec(master_descriptor);
    if (master_error != 0) {
        (void)close(master_descriptor);
        mopelium_kill_and_reap(pid);
        return master_error;
    ...
    result->error_stage = MOPELIUM_PTY_STAGE_NONE;
