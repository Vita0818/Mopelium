...
3. For a short-lived read-only task that should inherit the current exact profile,
   prefer `delegate_task` with `to` omitted or `auto`; Mopelium may reuse or create a
