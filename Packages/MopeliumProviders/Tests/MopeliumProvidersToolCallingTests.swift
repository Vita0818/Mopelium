...
private final class SequencedToolHTTP: HTTPByteStreaming, @unchecked Sendable {
    private let queue = DispatchQueue(label: "mopelium.tests.sequenced-tool-http")
    private var index = 0
...
private let nonHTTPToolEndpoint = ProviderEndpoint(
    id: "bad-agent",
    baseURL: URL(string: "https://example.test/v1")!,
    chatEndpoint: URL(fileURLWithPath: "/tmp/mopelium-agent"),
    apiKeyRef: KeychainRef(service: "s", account: "a"),
    wire: .openai
)

final class MopeliumProvidersToolCallingTests: XCTestCase {
