        ...
        do {
            for try await chunk in provider.stream(request) {
                ...
                case .toolCalls(let calls):
                    if !calls.isEmpty {
                        throw MopeliumError.provider("Provider returned tool calls during a no-tool health check.")
                    }
                case .usage(let value):
                    usage = Usage.merging(usage, with: value)
                case .done:
    ...
    private static func failureReport(for error: Error,
        ...
        if error is CancellationError {
            return ProviderHealthReport(
                ...
                elapsedMillis: elapsedMillis(since: start),
                code: "cancelled",
                message: MopeliumError.cancelled.localizedDescription)
        }
        return ProviderHealthReport(
            status: .failed,
            role: role,
        ...
        return try await withThrowingTaskGroup(of: T.self) { group in
            ...
            do {
                guard let value = try await group.next() else {
                    throw MopeliumError.provider("Provider health check did not produce a result.")
                }
                group.cancelAll()
                return value
            } catch {
