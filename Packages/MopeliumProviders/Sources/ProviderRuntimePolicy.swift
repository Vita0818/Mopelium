...
enum ProviderRuntime {
    ...
    static func exhausted(_ error: Error, attempts: Int, operation: String) -> Error {
        ...
        if let mopelium = normalized as? MopeliumError {
            switch mopelium {
            case .provider(let message):
                return MopeliumError.provider(message + suffix)
            case .config(let message):
                return MopeliumError.config(message + suffix)
            case .decoding(let message):
                return MopeliumError.decoding(message + suffix)
            case .io(let message):
                return MopeliumError.io(message + suffix)
            case .notFound(let message):
                return MopeliumError.notFound(message + suffix)
            case .permissionDenied(let message):
                return MopeliumError.permissionDenied(message + suffix)
            case .cancelled:
                return mopelium
            }
        }
        return MopeliumError.provider("\(operation) failed. \(normalized.localizedDescription)\(suffix)")
    }
        ...
        return try await withThrowingTaskGroup(of: T.self) { group in
            ...
            do {
                guard let value = try await group.next() else {
                    throw MopeliumError.provider("\(operation) did not produce a result.")
                }
                group.cancelAll()
                return value
            } catch {
