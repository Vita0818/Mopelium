...
/// Maps Mopelium chat requests onto the OpenAI-compatible `/chat/completions`
...
public struct OpenAIWireProvider: ChatProvider {
    ...
    /// Model options are an open JSON extension point. Mopelium protects only
    ...
    /// Historical Mopelium endpoints retain their previous wire behavior.
    ...
    public func stream(_ request: URLRequest) -> AsyncThrowingStream<Data, Error> {
        AsyncThrowingStream { continuation in
            let task = Task {
                ...
                #else
                continuation.finish(throwing: MopeliumError.provider("Streaming HTTP is unavailable on this platform"))
                #endif
            }
            continuation.onTermination = { _ in task.cancel() }
        }
