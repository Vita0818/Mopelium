...
enum ProviderErrorFormatting {
    ...
    static func httpStatus(_ status: Int,
        ...
        return MopeliumError.provider(parts.joined(separator: " "))
    }
    ...
    static func transport(_ error: Error) -> Error {
        ...
        if let mopelium = error as? MopeliumError {
            return sanitized(mopelium)
        }
        if let urlError = error as? URLError {
            return MopeliumError.provider(transportMessage(urlError))
        }
        return MopeliumError.provider(clean(error.localizedDescription))
    }
    ...
    static func streamErrorPayload(_ data: Data) -> Error? {
        ...
        return MopeliumError.provider(
            "Provider rejected the streaming request. Provider said: \(message)")
    }

    static func invalidStreamPayload(_ payload: String, underlying: Error) -> MopeliumError {
    ...
    static func incompleteStream(operation: String) -> MopeliumError {
        .decoding(
            "\(operation) ended before a completion marker. " +
            "Check endpoint compatibility, proxy buffering, or provider stability.")
    }

    static func invalidToolCallStream(_ message: String) -> MopeliumError {
        .decoding(
    ...
    static func invalidResponsePayload(_ data: Data,
                                       operation: String,
                                       expected: String,
                                       underlying: Error? = nil) -> MopeliumError {
        var parts = [
    ...
    private static func sanitized(_ error: MopeliumError) -> MopeliumError {
        switch error {
