...
///
/// Mopelium remains Swift-native and does not execute npm packages. The package
    ...
    Sendable
{
    ...
    public static let siliconFlowV1 = ProviderRequestAdapter(
        rawValue: "mopelium:siliconflow-v1")
    ...
    public static let cohereV2 = ProviderRequestAdapter(
        rawValue: "mopelium:cohere-v2")

    /// Missing adapter fields in previously persisted Mopelium values decode to
    ...
    public static let legacyOpenAIWire = ProviderRequestAdapter(
        rawValue: "mopelium:legacy-openai-wire")

    public let rawValue: String

    public init(rawValue: String) {
        // Package identity is configuration data, not a display label.
        // Preserve unknown values byte-for-byte. The three predecessor-owned
        // values are a bounded compatibility exception: old config decodes to
        // the new canonical adapter and any later encoding emits Mopelium.
        switch rawValue {
        case "intatis:siliconflow-v1":
            self.rawValue = Self.siliconFlowV1.rawValue
        case "intatis:cohere-v2":
            self.rawValue = Self.cohereV2.rawValue
        case "intatis:legacy-openai-wire":
            self.rawValue = Self.legacyOpenAIWire.rawValue
        default:
            // An empty or whitespace-only explicit value remains unsupported
            // and fails closed instead of being fixed up.
