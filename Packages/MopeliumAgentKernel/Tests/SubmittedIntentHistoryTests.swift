...
final class SubmittedIntentHistoryTests: XCTestCase {
    func testRetryHistoryKeepsEarlierCompletedTurnAndExcludesCurrentAndLaterSubmissions() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-submission-history-\(UUID().uuidString)",
            isDirectory: true)
    ...
    func testCoworkRootHistoryMatchesCodexTurnOrderWithContextBundleAndReplay() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-cowork-thread-history-\(UUID().uuidString)",
            isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let quotedContextLines = Set(contextData.components(separatedBy: .newlines))
        XCTAssertFalse(quotedContextLines.contains("| A1"))
        XCTAssertFalse(quotedContextLines.contains("| A2"))
    }
