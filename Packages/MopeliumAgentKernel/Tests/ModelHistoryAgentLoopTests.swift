...
final class ModelHistoryAgentLoopTests: XCTestCase {
    ...
    func testRepeatedProviderCallIDAcrossToolRoundsIsUniquedAndReplayable() async throws {
        ...
        XCTAssertEqual(callIDs, ["call_0", "call_mopelium_0"])
        ...
        XCTAssertEqual(
            messages[4],
            .tool(id: "call_mopelium_0", content: "probe:B"))
    ...
    private func makeWorkspace() throws -> URL {
        let workspace = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-model-history-\(UUID().uuidString)",
                isDirectory: true)
        try FileManager.default.createDirectory(
            at: workspace,
