...
private struct PartialThenFailingToolProvider: ToolCallingProvider {
    func stream(_ request: AgentRequest) -> AsyncThrowingStream<AgentChunk, Error> {
        AsyncThrowingStream { continuation in
            ...
            continuation.finish(throwing: MopeliumError.decoding(
...
private struct SecretEchoFailingProvider: ToolCallingProvider {
    ...
    func stream(_ request: AgentRequest) -> AsyncThrowingStream<AgentChunk, Error> {
        AsyncThrowingStream { continuation in
            continuation.finish(throwing: MopeliumError.provider(
...
final class MopeliumAgentKernelTests: XCTestCase {

    private func workspaceAndLog() throws -> (URL, EventLog) {
        let ws = FileManager.default.temporaryDirectory
            .appendingPathComponent("mopelium-kernel-\(UUID().uuidString)", isDirectory: true)
    ...
    func testAgentLoopRunsBrowserSearchThroughPermissionFlow() async throws {
        ...
        let searchArgs = """
        {"query":"Mopelium browser tools","engine":"duckduckgo","profile":"agent-web","channel":"msedge","waitMillis":0,"maxCharacters":2000}
        """
        let provider = ScriptedProvider([
        ...
        let browserStdout = #"{"action":"search","profile":"agent-web","backend":"cdp","backendDetail":"edge","url":"https://duckduckgo.com/?q=Mopelium%20browser%20tools","title":"Search Results","text":"Search results for Mopelium browser tools","links":[{"text":"Mopelium browser tool docs","href":"https://example.com/mopelium-browser"}],"elements":[{"role":"textbox","name":"Search","selector":"input[name=q]","tag":"input","type":"search"}]}"#
        ...
        let answer = try await loop.send("search the web for Mopelium browser tools")
        ...
        XCTAssertTrue(result.observation.contains("Mopelium browser tool docs - https://example.com/mopelium-browser"), result.observation)
        XCTAssertTrue(result.observation.contains(#"textbox "Search" selector=input[name=q] type=search"#), result.observation)

        let stateURL = ws.appendingPathComponent(".mopelium/browser/state/agent-web.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
        XCTAssertEqual(state["url"] as? String, "https://duckduckgo.com/?q=Mopelium%20browser%20tools")
    ...
    func testAgentLoopCompletesBrowserFormTaskThroughPermissionFlow() async throws {
        ...
        let stateURL = ws.appendingPathComponent(".mopelium/browser/state/agent-task.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
    ...
    func testAgentLoopBrowsesDynamicFeedThroughPermissionFlow() async throws {
        ...
        let stateURL = ws.appendingPathComponent(".mopelium/browser/state/social-feed.json")
        let state = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(contentsOf: stateURL)) as? [String: Any])
    ...
    func testAgentLoopRunsBrowserProfileDeleteThroughDestructivePermissionFlow() async throws {
        ...
        XCTAssertTrue(result.observation.contains("removed profile data: yes (.mopelium/browser/profiles/work)"), result.observation)
        XCTAssertTrue(result.observation.contains("removed state file: yes (.mopelium/browser/state/work.json)"), result.observation)
        XCTAssertTrue(result.observation.contains("removed downloads: yes (.mopelium/browser/downloads/work)"), result.observation)
    ...
    func testUnknownToolArgumentsDoNotRequestPermissionOrExecuteTool() async throws {
        ...
        XCTAssertEqual(durableCall.args, #"{"_mopelium":"arguments_redacted"}"#)
    ...
    func testUnknownToolNameAndArgumentsAreRedactedBeforeEventLog() async throws {
        ...
        XCTAssertEqual(call.args, #"{"_mopelium":"arguments_redacted"}"#)
