...
private func leaseTempLog() throws -> EventLog {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("mopelium-lease-\(UUID().uuidString)", isDirectory: true)
...
final class ToolRegistryLeaseTests: XCTestCase {
    ...
    func testHostedSearchRequiresSeparateLeaseCapabilityAndBoundService() {
        ...
        XCTAssertEqual(hostedOnly.registryVersion, "mopelium.cowork.v8")
        XCTAssertEqual(
    ...
    func testApplyPatchCapabilityRegistersBothConcreteEditToolsFromOneSource() throws {
        ...
        XCTAssertEqual(writeAuthorization.concreteToolID, "mopelium.cowork.v8/write_file")
        XCTAssertEqual(patchAuthorization.concreteToolID, "mopelium.cowork.v8/apply_patch")
    ...
    func testReadWorkspaceCapabilityRegistersTextAndImageReads() {
        let registry = Orchestrator.toolRegistry(
            for: CapabilityLease(tools: [.readWorkspace]))

        XCTAssertEqual(
            Set(registry.descriptors().map(\.name)),
            ["read_file", "view_image"])
        XCTAssertEqual(
            registry.registration(named: "read_file")?
                .grantingCapabilities,
            [.readWorkspace])
        XCTAssertEqual(
            registry.registration(named: "view_image")?
                .grantingCapabilities,
            [.readWorkspace])
    }
    ...
    func testDocumentCapabilitiesRegisterOnlyTheirExactReplacementTools() {
        ...
        let expected: [(ToolCapability, Set<String>)] = [
            (.readPDF, ["inspect_pdf", "read_pdf"]),
            (.readDOCX, ["read_docx", "continue_docx_read"]),
            (.readPPTX, ["read_pptx", "continue_pptx_read"]),
            (.readXLSX, ["read_xlsx", "continue_xlsx_read"]),
            (.readHTML, ["read_html", "continue_html_read"]),
            (.readEPUB, ["read_epub", "continue_epub_read"]),
            (.documentOCR, ["ocr_pdf"]),
            (.documentRender, ["pdf_render_page"]),
            (.documentExportPDF, exportNames),
            (.documentWrite, writeNames),
        ]
        let registry = Orchestrator.toolRegistry(
            for: CapabilityLease(tools: Set(expected.map { $0.0 })))

        XCTAssertEqual(registry.registryVersion, "mopelium.cowork.v8")
        XCTAssertEqual(
            Set(registry.descriptors().map(\.name)),
            expected.reduce(into: Set<String>()) { $0.formUnion($1.1) })
        for (capability, names) in expected {
            for name in names {
                XCTAssertEqual(
    ...
    func testAuthorizationSnapshotValidationFailsClosedForIdentityDrift() throws {
        ...
        let wrongRegistry = try replacing("registryVersion", with: "mopelium.cowork.other")
        XCTAssertThrowsError(try registry.validateAuthorizationSnapshot(
            wrongRegistry,
            toolName: "write_file",
            normalizedArguments: arguments,
        ...
        let wrongConcreteTool = try replacing("concreteToolID", with: "mopelium.cowork.v8/other")
        XCTAssertThrowsError(try registry.validateAuthorizationSnapshot(
            wrongConcreteTool,
            toolName: "write_file",
            normalizedArguments: arguments,
    ...
    func testWorkerLeaseDoesNotExposeCoordinatorTools() {
        ...
        XCTAssertTrue(toolNames.contains("view_image"))
        XCTAssertTrue(toolNames.contains("inspect_pdf"))
        ...
        XCTAssertTrue(toolNames.contains("ocr_pdf"))
        XCTAssertFalse(toolNames.contains("pdf_render_page"))
    ...
    func testWorkerTaskUpdateSchemaExposesOnlyBoundProgressAndSettlementFields() throws {
        ...
        XCTAssertEqual(workerRegistry.registryVersion, "mopelium.cowork.v8")
        XCTAssertEqual(
    ...
    func testCoordinatorLeaseCanExposeDelegationTools() {
        ...
        XCTAssertFalse(toolNames.contains("document_read"))
        XCTAssertTrue(toolNames.contains("ocr_pdf"))
        XCTAssertTrue(toolNames.contains("pdf_render_page"))
        XCTAssertTrue([
        ...
        XCTAssertTrue([
            ...
            "xlsx_create_workbook", "xlsx_append_row",
        ].allSatisfy(toolNames.contains))
        XCTAssertFalse(toolNames.contains("document_export_pdf"))
        XCTAssertFalse(toolNames.contains("document_write"))
        XCTAssertFalse(toolNames.contains("read_document"))
    ...
    func testTaskLeasePreservesCoordinatorCapabilityAndPrompt() async throws {
        ...
        XCTAssertTrue(systemPrompt.contains("Inspect the bounded MOPELIUM_SKILL_CATALOG"))
