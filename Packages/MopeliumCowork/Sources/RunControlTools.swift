...
private func executeRunClose(
    ...
    guard !reason.isEmpty else {
        throw MopeliumError.decoding("run close reason must not be empty")
    }
    guard let controller = context.runController else {
        throw MopeliumError.io("run control is not available in this invocation")
    }
    let result = await controller.requestClose(outcome: outcome, reason: reason)
    if result.hasPrefix("error:") {
        throw MopeliumError.io(String(result.dropFirst("error:".count)).trimmingCharacters(in: .whitespaces))
    }
...
public struct FinishRunTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        name: "finish_run",
        description: "Declare the exact current Cowork run complete after the requested outcome is verified. Mopelium durably closes admission and drains remaining run-scoped work. The host binds all identifiers; supply only a concise reason.",
        sideEffect: .write,
        parameters: .object([
            "type": .string("object"),
            "properties": .object([
...
public struct StopRunTool: Tool {
    ...
    public static let descriptor = ToolDescriptor(
        name: "stop_run",
        description: "Stop the exact current Cowork run when no further useful progress is possible or a genuine blocker remains. Mopelium durably closes admission and drains remaining run-scoped work. The host binds all identifiers; supply only a concise reason.",
        sideEffect: .write,
        parameters: .object([
            "type": .string("object"),
            "properties": .object([
