...
public actor Orchestrator {
    ...
    func delegateTask(from: AgentID,
        ...
        guard let resolution = await resolveDelegationTarget(
            ...
            return "error: no delegation worker is available and Mopelium could not create one"
        }
    ...
    func createWorkTask(requestedBy: AgentID,
        ...
        guard let currentRunID else {
            throw MopeliumError.config("task_create requires a current ContinuationRun")
        }
        ...
        guard owner != Self.automaticPermissionReviewerID,
            ...
            throw MopeliumError.notFound("WorkTask owner is not an attached data-plane agent")
        }
    ...
    func updateWorkTask(requestedBy: AgentID,
        ...
        guard current.runID == currentRunID else {
            throw MopeliumError.permissionDenied("only WorkTasks in the current ContinuationRun can be updated")
        }
        if !canManage {
            guard canUpdateOwned,
                  currentWorkTaskID == current.id,
                  current.owner == requestedBy else {
                throw MopeliumError.permissionDenied("workers may update only their assigned WorkTask")
            }
    ...
    func carryForwardNonterminalWorkTasks(
        ...
        for source in sources {
            guard let cloneID = cloneIDs[source.id] else {
                throw MopeliumError.config("carry-forward clone ID allocation failed")
            }
            ...
            for dependencyID in source.dependsOn {
                ...
                if sourceIDs.contains(dependencyID) {
                    guard let mappedID = cloneIDs[dependencyID] else {
                        throw MopeliumError.config("carry-forward dependency mapping failed")
                    }
            ...
            if !plan.blockers.isEmpty {
                ...
                case .success(.waitingFor):
                    guard let pending = preflight.task(plan.initial.id) else {
                        throw MopeliumError.notFound("carried WorkTask \(plan.initial.id.rawValue)")
                    }
                    final = pending
                case .success(.blockedBy(let dependencies)):
                    let blocker = Self.carryForwardBlockerPrefix
        ...
        if canManage {
            if request.includeGoalHistory {
                guard let currentGoalID else {
                    throw MopeliumError.permissionDenied("goal history requires a current Goal binding")
                }
                ...
                guard explicitRunID == currentRunID
                    ...
                    throw MopeliumError.permissionDenied("ContinuationRun is outside the current Goal scope")
                }
    ...
    func createGoal(request: GoalCreateRequest,
        ...
        guard canCreate else {
            throw MopeliumError.permissionDenied("the current capability lease cannot create Goals")
        }
        ...
        if let suppliedTarget {
            ...
            if let existing = registry.agent(candidate) {
                ...
                if let validationError = Self.agentNameValidationError(candidate.rawValue) {
                    throw MopeliumError.permissionDenied(validationError)
                }
    ...
    private func validateProposedDelegationTarget(
        ...
        guard registry.agent(target) == nil else {
            throw MopeliumError.permissionDenied("proposed delegation target is already attached")
        }
    ...
    static func toolRegistry(for lease: CapabilityLease,
        ...
        return ToolRegistry(
            ...
            registryVersion: "mopelium.cowork.v1")
    }
