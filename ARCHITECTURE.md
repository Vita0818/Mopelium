# Mopelium 架构设计 (ARCHITECTURE.md)

> draft-0 · 2026-06-11
> 本文件回应规格第 12 节要求的 7 项产出，作为 v0.1 动工前的设计基线。
> **来源与复用政策（2026-07-12 更新）**：Mopelium 是 Apple-first、Swift-native 优先的本地 AI workbench。项目允许按 [`docs/OPEN_SOURCE_REUSE.md`](docs/OPEN_SOURCE_REUSE.md) 选择性复制、翻译、修改或运行兼容许可证的公开源码、公开 model-facing prompt 与测试，并保留 provenance/NOTICE；不使用泄露或私有材料，也不复制第三方名称、Logo、图标、截图、UI 资产、商标性外观或品牌文案。外部实现不得绕过 Mopelium 的权限、lease、EventLog 和 Apple 平台边界。
...
> **Mopelium = 一个 headless 的 Agent Kernel + 一套结构化事件协议 + 多个消费该协议的前端（macOS GUI / iOS GUI / CLI）。**
...
Agent Kernel 不知道 GUI 是否存在，只通过 `MopeliumProtocol` 收发结构化消息。GUI 只是众多客户端之一；CLI 的人类界面也只是同一协议之上的一层渲染器。→ 从结构上杜绝"把 GUI 写成 chatbox"——GUI 根本拿不到绕过 kernel 的捷径，所有能力都得先在 kernel 里成为协议事件。
...
Tool 只是"哑执行器"：它声明自己的副作用元数据 (`ToolDescriptor`) + 提供一个执行函数，**不自己判断能不能执行**。是否放行由独立的 `MopeliumPermission` 层在调用工具**之前**决定。不变式：任何"模型产出 `tool_call` → 工具真正执行"的代码路径都必须穿过权限闸，**没有旁路**。→ 让规格第 6、13 节的权限规则成为编译期/结构上的不变式，而不是散落在各工具里的 `if`。

### 1.3 最小数据闭环

```text
            ┌─────────────────────────── clients ───────────────────────────┐
            │   macOS GUI        iOS GUI        mopelium-cli (human / pipe)    │
            └───────▲───────────────▲───────────────────▲────────────────────┘
                    │ events        │ events            │ events
        commands    │   (project    │                   │
                    ▼   into view)   ▼                   ▼
      ┌─────────────────────────────────────────────────────────────────────┐
      │                       MopeliumProtocol (JSON-RPC / JSONL)             │
      └─────────────────────────────────────────────────────────────────────┘
...
基本沿用规格第 5 节的提议，**做了一处关键调整**：把权限/审查系统从 `MopeliumAgentKernel` 中拆出，独立为 `MopeliumPermission`。理由见 §3.8。
...
│
├── Apps/
│   ├── MopeliumMac/            ← macOS app target（链接全部包，内嵌 kernel）
│   ├── MopeliumiOS/            ← iOS app target（只链接子集，见 §4）
│   └── mopelium-cli/           ← Swift-native CLI + headless kernel 入口
│
├── Packages/
│   ├── MopeliumCore/           ← 基础类型、ID、配置、错误
│   ├── MopeliumProtocol/       ← 事件/RPC 契约（纯 Codable，无 I/O）
│   ├── MopeliumProviders/      ← capability-based provider 抽象 + OpenAI-compatible 实现
│   ├── MopeliumConversation/   ← event log、thread、ChatLoop、resume
│   ├── MopeliumArtifacts/      ← artifact store（image/video/audio/transcript/diff/patch…）
│   ├── MopeliumMultimodal/     ← ASR / 生图 / 生视频 任务抽象（v0.4 起）
│   ├── MopeliumTools/          ← read_file/search_text/apply_patch/run_shell/git_*（哑执行器）
│   ├── MopeliumPermission/     ← ★新增：deterministic gate + reviewer + profiles
│   ├── MopeliumAgentKernel/    ← agent loop、context builder、tool 调度（调用 Permission）
│   ├── MopeliumCowork/         ← agent registry、message bus、mediation、orchestrator
│   └── MopeliumSharedUI/       ← macOS/iOS 共用 SwiftUI 组件（消费事件流）
│
...
```text
                       ┌────────────────┐
                       │  MopeliumCore   │  (无依赖：ID/config/error/枚举)
                       └───────┬────────┘
                               │
                       ┌───────▼────────┐
                       │ MopeliumProtocol│  (纯数据契约)
                       └─┬───┬───┬───┬──┘
              ┌──────────┘   │   │   └──────────────┐
              ▼              ▼   ▼                  ▼
      ┌──────────────┐ ┌──────────┐ ┌────────────────┐ ┌──────────────┐
      │MopeliumProvid.│ │MopeliumTo.│ │MopeliumConversa.│ │MopeliumArtifa.│
      └──────┬───────┘ └────┬─────┘ └───────┬────────┘ └──────┬───────┘
             │              │               │                 │
             ├──────────────┘               │                 │
             ▼                              │                 │
     ┌────────────────┐                     │                 │
     │MopeliumPermission│◄───────────────────┼─────────────────┘ (读 artifact/diff 摘要)
     └───────┬────────┘                     │
             │            ┌─────────────────┘
             ▼            ▼
     ┌──────────────────────────┐         ┌──────────────────────┐
     │   MopeliumAgentKernel     │         │  MopeliumMultimodal   │
     ...
     └───────────┬──────────────┘
                 ...
                 ▼
     ┌──────────────────────────┐
     │      MopeliumCowork       │  (AgentKernel + Permission + Conversation)
     └──────────────────────────┘

     MopeliumSharedUI  依赖→ Core / Protocol / Conversation / Artifacts （只读事件，不依赖 Kernel/Tools）

     Apps:
       MopeliumMac  → 全部
       mopelium-cli → Core/Protocol/Providers/Conversation/Artifacts/Tools/Permission/AgentKernel/Cowork（无 UI）
       MopeliumiOS  → Core/Protocol/Providers/Conversation/Artifacts/Multimodal/SharedUI（无 Tools/Permission-fs/Kernel-workspace/Cowork）
```
...
### 3.1 MopeliumCore
- **职责**：全项目共享的基础值类型与配置。
- **拥有**：`SessionID` `ThreadID` `AgentID` `MessageID` `ModelID` `ArtifactID`（都是 typed 包装的 String/UUID）；`MopeliumError`；`SessionPolicy`（chat/code/cowork）；`PlatformProfile`（见 §4）。
...
### 3.8 MopeliumPermission（★ 从 Kernel 拆出）
...
- **iOS app target 在编译期就不链接** `MopeliumTools` / `MopeliumPermission` / `MopeliumAgentKernel` / `MopeliumCowork`。
...
iOS 用 `UIDocumentPicker` / `fileImporter` 选文件 → 只能变成 `Artifact(kind: .fileAttachment)` 进 `ArtifactStore`，作为聊天附件/视觉输入。**没有任何 API 把 attachment 提升为 workspace**——因为提升逻辑住在 iOS 不链接的 `MopeliumTools/AgentKernel` 里。沙盒限制因此是"结构性满足"，不是"我们承诺不碰"。

### 4.4 共享与平台特定代码的比例（预期）

```text
共享（Packages/，两端同一份源码）：  ~80–85%
平台特定（Apps/MopeliumMac, Apps/MopeliumiOS）： ~15–20%
...
- **内嵌场景（v0.1 默认）**：GUI 直接链接 Swift 包，kernel 在**进程内**运行。"协议"此时退化为进程内的 `async` 接口，但**消息类型用的就是 `MopeliumProtocol` 的同一批 `Codable`**。即：内部边界从第一天起就是协议形状，日后拆进程零改造。
- **子进程 / daemon 场景（v0.2+ 可选）**：`mopelium agent --stdio`（JSON-RPC 2.0，换行分帧）或 `mopelium daemon`（local HTTP + SSE）。消息类型完全一致。
...
**涉及包**：Core / Protocol / Providers / Conversation / Artifacts / SharedUI / Apps/MopeliumMac

| 项 | 内容 |
|----|------|
| Core | 各 ID、`SessionKind`、`MopeliumError`、`PlatformProfile` |
...
| MopeliumMac | 接线 Chat 面；API key 仅从环境变量解析；本地会话保存（= event log 落盘） |
...
| MopeliumMac（加） | Code 面；workspace 选择（**security-scoped bookmark**） |
...
- **G. 持久化位置/格式：** event log + artifacts 放 `~/Library/Application Support/Mopelium/<session>/`（JSONL + 文件）可接受吗？
...
App Store 强制 sandbox。逐项核对 Mopelium 需要的操作在 sandbox 下能否成立：
