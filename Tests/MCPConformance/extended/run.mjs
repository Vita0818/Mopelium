...
if (!client) throw new Error("MOPELIUM_MCP_CONFORMANCE_CLIENT is required");

const scenarios = [
  "mopelium/task-complete",
  "mopelium/task-timeout",
  "mopelium/task-cancel"
];
...
for (const scenario of scenarios) {
  ...
  try {
    ...
    assertSubsequence(
      methods,
      scenario === "mopelium/task-complete"
...
console.log(
  "Mopelium MCP extended interoperability complete: task creation, poll, result, timeout, and cancellation"
);
  ...
  switch (method) {
    case "initialize":
      result = {
        ...
        serverInfo: { name: "mopelium-task-peer", version: "1.0.0" },
        capabilities: {
          tools: {},
          tasks: {
            cancel: {},
    ...
    case "tasks/get":
      result = taskWire(
        taskID,
        scenario === "mopelium/task-complete" ? "completed" : "working",
        now
      );
      break;
    case "tasks/result":
