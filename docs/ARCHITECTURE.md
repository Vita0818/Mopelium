...
## 2026-08-18 Mopelium canonical Application Support boundary

构建图、源码目录、project-owned 类型、App、CLI、配置和新 protocol output 已原位迁移到 Mopelium；
同一 runtime 没有 fork。App 与 CLI composition root 现在只把
`~/Library/Application Support/Mopelium` 交给 canonical session/config owners；它们不再调用
`ApplicationSupportIdentityMigrator`，也不再探测旧 `Intatis` 根。旧根存在、缺失或损坏均不影响
Mopelium 启动；Mopelium 不读取、迁移、合并或删除其中内容。Core 中保留的旧 migration primitive
不属于 shipping composition call graph。

macOS App 启动不再从旧 bundle domain 导入 provider catalog/selection、workspace、Cowork settings 或
renderer mode。config/env/auth 使用 canonical-first candidate selection；
...
- Cowork wide rail 的几何由 stable outer detail width 唯一解析：visible 时 rail 为 348pt、section
  ...
  selected-agent conversation ID；selection 通过 rail 内独立轻量状态只重绘淡香槟行 affordance 与
  accessibility value。每个 passive glass backdrop 又是 content-independent Equatable view，使用
  identity transition、无 inspector 隐式动画，并在 glass 之外以主题 `cardStroke` 的单物理像素
...
- **UI 配色与无色原生 Liquid Glass**：macOS detail 由 `MopeliumSystemCanvas` 按当前 appearance 渲染轻暖深灰或暖白奶油三段渐变；`NavigationSplitView` sidebar 仍由系统材质拥有，不另画侧栏底板。根视图同时通过 SwiftUI 官方 `containerBackground(for: .window)` 把同一 canvas 注册为 WindowGroup 背景，并隐藏 `.windowToolbar` backing，使右侧 titlebar 不再回退系统白色、左侧 sidebar 材质仍可独立延伸；不使用 NSWindow 私有接线或自绘 titlebar。`MopeliumTheme` 集中定义 `champagne/champagneAccent`、暖色三级文字与结构描边；品牌色只进入 canvas、文字、选中和描边，不进入 Glass。assistant/agent/system 对话正文（包括失败/中断回复）直接继承暖色 canvas，不叠 Material 或描边；用户消息是唯一对话气泡，直接使用无 tint 的 `Glass.regular`，不叠加自定义 stroke。正常 tool、权限、数据卡片和 artifact 等专用结构化内容层继续使用 `.regularMaterial` 与低对比语义描边；Code/Cowork error 仅使用右栏统一错误卡。composer、模型菜单、主要操作及确实需要融合的紧凑 action group 在 macOS 26 使用原生 `glassEffect`、`GlassEffectContainer` 与统一 `.glass` button style；所有非破坏性 Glass button 均无 tint，不使用 `.glassProminent`。Stop 保持相同 Glass 形态和系统 destructive red，是唯一彩色 button 语义例外。用户消息与 Cowork trailing status rail 是仅有的内容层玻璃例外，后者各 section 使用独立、无 tint 的原生 `Glass.clear` backdrop，不由一个会重组 shape 的 container 包住。不得调用 `Glass.tint`、在祖先层注入品牌 button tint，或恢复旧 `intatisGlassCard` / `intatisGlassCapsule` 的自绘 blur、高光、折射或阴影。共享 composer 几何、40×40 action、30×30 sidebar New、底部对齐、serif/sans/monospaced 字体角色和不整片玻璃化的边界保持不变。当前唯一 App 是 macOS；共享代码中的旧平台 fallback 不属于当前产品验收面。精确 token、组件映射与验收见 `docs/CURRENT_UI_COLOR_SYSTEM.md`；`docs/UI_COLOR_SYSTEM.md` 只保留原方案方向和旧实现 provenance。
