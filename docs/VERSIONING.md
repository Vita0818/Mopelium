...
当前工作树的产品基线为 `v0.12 (50)`。工程元数据此前已从长期停留的 `0.12 (1)`
校准为 `0.32 (32)`，后续推进为 `0.36 (36)`、`0.38 (38)`、`0.40 (40)`、
`0.48 (48)` 和 `0.10 (49)`；本次按用户要求把 marketing version 推进为 `0.12`，同时把构建号从
`49` 单调提升到 `50`。构建号不要求等于 marketing version 的 minor，也不能因 marketing
version 调整而回退。
