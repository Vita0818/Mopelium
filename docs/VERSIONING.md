...
当前工作树的产品基线为 `v0.10 (49)`。工程元数据此前已从长期停留的 `0.12 (1)`
校准为 `0.32 (32)`，后续推进为 `0.36 (36)`、`0.38 (38)`、`0.40 (40)` 和
`0.48 (48)`；本次按产品版本要求把 marketing version 调整为 `0.10`，同时把构建号从
`48` 单调提升到 `49`。构建号不要求等于 marketing version 的 minor，也不能因 marketing
version 调整而回退。
