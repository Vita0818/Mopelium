...
- iOS 继续由 `NavigationStack` / SwiftUI 容器提供原生根背景，不引入 Mopelium 私有页面色。
...
- `Packages/MopeliumSharedUI/Sources/ThreadSurfaces.swift`：结构化内容 Material、30×30 sidebar New 圆形 glass control、原生圆形 icon controls、40pt composer/selection-menu 几何合同、两排 composer、首排 usage strip 与可选 accessories。
- `Packages/MopeliumSharedUI/Sources/Views.swift`：共享 Chat 消息和 composer；正常 assistant / agent 回复继承系统 canvas。
