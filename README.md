...
client。Chat、Code 与 Cowork 源码能力继续保留，当前面向用户的 App 入口只有 Cowork。所有运行时能力围绕结构化 EventLog、共享 AgentKernel、显式工具注册和权限链
...
- 设置：provider catalog、Mopelium JSON/JSONC 配置、MCP、renderer fallback、第三方声明，
  以及只在本机生成且不上传的脱敏诊断 ZIP。

macOS 唯一发行 target 是 `MopeliumMac`，Bundle ID 为 `com.Vita0818.Mopelium`，通过
Developer ID、Apple notarization 和直接下载分发。仓库不提供 iOS App，也不保留 Mac App Store target。

### CLI

`mopelium` 支持 Chat/Code/Cowork REPL、managed execution、Skills、per-agent inference
...
- 当前没有 iOS App 或 Mac App Store 产品图。
...
```text
Apps/                 macOS App 与 CLI 入口
Packages/             15 个公共库、3 个内部 C/guard target 与测试
...
xcodebuild -project Mopelium.xcodeproj -scheme MopeliumMac \
...
```sh
MOPELIUM_NOTARY_PROFILE=<profile-name> scripts/package-macos-release.sh
```
...
MOPELIUM_NOTARY_PROFILE=<profile-name> \
...
产物并打印 `MOPELIUM_RESUME_RELEASE_DIR` 恢复命令。恢复同一 submission，不要重复上传。
...
- macOS/CLI 高级配置读取 `MOPELIUM_CONFIG`、Mopelium-owned JSON/JSONC 路径及兼容 fallback；
...
- macOS Chat/Code/Cowork 的输入栏语音按钮共用顶层 `transcription_model` 宿主路由；
...
`permission_reviewer_model` 是 Mopelium 的顶层授权控制面字段，格式为
`<provider>/<model-id>`，并引用 provider `models` 中已配置的 base profile。Mopelium 在启动/恢复
...
`image_model` 是 Mopelium 的顶层扩展字段，格式为 `<provider>/<model-id>`。专用图片 provider
...
`transcription_model` 同样是 Mopelium 的顶层扩展字段，格式为 `<provider>/<model-id>`。专用语音
...
`search_knowledge`。上例中的 URL 和模型 ID 是需要替换的配置值；`mopelium:siliconflow-v1`
...
`mopelium:cohere-v2`。Knowledge-only provider 的 `models` 可保持空对象，不会进入普通推理模型菜单；
...
Knowledge 工具仅接入 macOS/CLI 的 Code 与 Cowork，不进入 Chat。
...
Mopelium 自有代码和第三方采用状态见 [`NOTICE.md`](NOTICE.md)、
