...
struct CLIModernProviderConfig: Sendable {
    ...
    static func load(from url: URL,
        ...
        guard FileManager.default.fileExists(atPath: url.path) else {
            throw IntatisError.config("selected Mopelium provider config is unavailable")
        }
        ...
        guard case .object(let root) = try JSONDecoder().decode(JSONValue.self, from: data),
            ...
            throw IntatisError.config("selected Mopelium provider config has no provider map")
        }
        ...
        guard !inferenceRoutes.isEmpty else {
            throw IntatisError.config("selected Mopelium provider config has no usable routes")
        }
