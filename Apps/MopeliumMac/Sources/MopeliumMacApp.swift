...
final class AppEnvironment: ObservableObject {
    ...
    func saveSettings(catalog rawCatalog: AppProviderCatalog,
                      apiKeyEnvironmentNamesByProviderID: [String: String]) throws {
        ...
        for index in catalog.providers.indices {
            ...
            let requestedName = apiKeyEnvironmentNamesByProviderID[provider.id]
                ?? AppConfig.apiKeyEnvironmentName(for: provider)
            let environmentName = try AppConfig.validatedAPIKeyEnvironmentName(requestedName)
            catalog.providers[index].apiKeyAccount = environmentName
            catalog.providers[index].apiKeySource = .environment(environmentName)
        }
        catalog = AppConfig.normalizedCatalog(catalog)
        try AppConfig.writeEditableProviderConfig(catalog: catalog)
        ...
        needsAPIKey = !Self.hasAPIKey(ref: catalog.selectedProvider.map(AppConfig.apiKeyRef(for:))
                                      ?? .environment(AppConfig.defaultAPIKeyEnvironmentName()))
...
struct CodeSessionView: View {
    ...
    var body: some View {
            CodeShell(items: vm.items,
                  ...
                  composerAccessory: AnyView(MopeliumComposerModelControl(
                    catalog: catalog,
                    isBusy: vm.isWorking,
            ...
            CoworkShell(items: vm.items,
                        ...
                        composerInputAccessory: AnyView(HStack(
                            ...
                            if !vm.draftAttachments.isEmpty {
                                Menu(MopeliumLocalization.format(
                                    "%lld attached",
        ...
        CoworkProjectSettingsSheet(
            ...
            onAddWorkspace: {
                if let url = WorkspaceAccess.choose(
                    prompt: MopeliumLocalization.string("Choose Project Workspace")) {
