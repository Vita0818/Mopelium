...
enum CoworkProjectSettingsStore {
    ...
    static func loadAndMigrate(
        ...
        do {
            ...
            do {
                ...
                legacyWarning = " " + MopeliumLocalization.format(
                    "Legacy settings were retained but could not be decoded safely: %@",
                    error.localizedDescription)
            }
            return LoadResult(
                settings: fallback,
                warning: MopeliumLocalization.format(
    ...
    private static func key(_ sessionID: SessionID) -> String {
        "mopelium.cowork.projectSettings.\(sessionID.rawValue)"
    }
    ...
    private static func legacySettings(
        ...
        guard decoded.sessionID == sessionID else {
            throw MopeliumError.config("Legacy Cowork settings belong to another session.")
        }
        guard decoded.schemaVersion <= CoworkSessionSettings.currentSchemaVersion else {
            throw MopeliumError.config("Legacy Cowork settings use a newer unsupported schema.")
        }
        guard decoded.workspaces.allSatisfy({ NSString(string: $0.path).isAbsolutePath }) else {
            throw MopeliumError.config("Legacy Cowork settings contain a non-absolute workspace path.")
        }
        return normalized(
            decoded,
            sessionID: sessionID,
...
struct CoworkProjectSettingsSheet: View {
                    ...
                    HStack(spacing: 10) {
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(agent.inferenceDisplayLabel
                                ?? MopeliumLocalization.string("Inference profile unavailable"))
                                .font(.caption2)
                ...
                ForEach(vm.project.workspaces) { workspace in
                    HStack(spacing: 9) {
                        Image(systemName: workspace.isPrimary ? "house" : "folder")
                            .foregroundStyle(workspace.isPrimary ? MopeliumTheme.accent(scheme) : .secondary)
                            .frame(width: 18)
                        VStack(alignment: .leading, spacing: 2) {
                        ...
                        .help(workspace.canRemove
                            ? MopeliumLocalization.string("Remove workspace")
    ...
    private var retainedDefaultBinding: (key: String, title: String)? {
        ...
        return (
            ...
            MopeliumLocalization.string("Saved inference profile (retained revision)"))
    }
    ...
    private var permissionOptions: [(rawValue: String, title: String)] {
        [
            (PermissionProfile.reviewed.rawValue, MopeliumLocalization.string("Reviewed")),
            (PermissionProfile.manual.rawValue, MopeliumLocalization.string("Manual")),
            (PermissionProfile.readOnly.rawValue, MopeliumLocalization.string("Read only")),
            (PermissionProfile.locked.rawValue, MopeliumLocalization.string("Locked")),
        ]
    }
        ...
        Task { @MainActor in
            ...
            if saved {
                ...
                settingsError = vm.composerError
                    ?? MopeliumLocalization.string("Session settings could not be saved.")
            }
        }
    }
}
