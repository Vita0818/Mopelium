...
typealias CoworkDraftAttachment = MopeliumComposerDraftAttachment
...
final class CoworkViewModel: ObservableObject, PermissionResponder {
    ...
    var mainInferenceDisplayLabel: String {
        ...
        return main.inferenceDisplayLabel ?? MopeliumLocalization.format(
    ...
    private let composerAttachmentStore: MopeliumComposerAttachmentStore
    ...
    init(sessionID: SessionID,
        ...
        self.composerAttachmentStore = MopeliumComposerAttachmentStore(
            store: artifactStore)
    ...
    func start() {
        ...
        do {
            ...
            projectionError = MopeliumLocalization.format(
    ...
    private func commitProjectionSnapshot(
        ...
        let changedThreadAgents = MopeliumExecutionTracePresentation.isEnabled
    ...
    private func restoreSubmittedIntentOutbox() async {
        do {
            ...
            composerError = MopeliumLocalization.format(
    ...
    func agentThreadPage(
        ...
        guard let projectionPump else {
            return CodeProjection().coworkAgentThreadPage(
                ...
                showsExecutionTrace:
                    MopeliumExecutionTracePresentation.isEnabled,
        ...
        let projected = await projectionPump.coworkAgentThreadPage(
            agentID: agentID,
            requestedUpperBound: requestedUpperBound,
            showsExecutionTrace: MopeliumExecutionTracePresentation.isEnabled,
            additionalItems: additionalItems)
        let interruptedFailure = SubmissionFailure(
            code: "interrupted",
            message: MopeliumLocalization.string(
    ...
    private func rebuildOutboxThreadItems(
        ...
        for entry in outboxEntries.values.sorted(by: {
            ...
            let item = CodeItem(
                id: id.rawValue,
                kind: .user,
                title: MopeliumLocalization.string("You"),
    ...
    private static func goalPresentation(
        ...
        if let audit {
            ...
            if let blocker = audit.blocker, !blocker.isEmpty {
                parts.append(MopeliumLocalization.format("Blocker: %@", blocker))
            }
    ...
    private func taskLine(_ task: CoworkTaskView) -> CoworkTaskLine {
        let assignee = task.assignee.map { "@\($0.rawValue)" }
            ?? MopeliumLocalization.string("Unassigned")
    ...
    private func retainWorkspaceAccess(forPath path: String) -> URL? {
        ...
        do {
            ...
            sessionStorageWarning = MopeliumLocalization.format(
    ...
    private func ensureAutomaticPermissionReview(existingProjection projection: CoworkProjection) async {
        guard let orchestrator else {
            setPermissionReviewerStatus(.failed(
                MopeliumLocalization.string("Cowork session is not ready.")))
            return
        }
        setPermissionReviewerStatus(.enabling)
    ...
    private func resumeRuntimeIfReady() async {
        ...
        guard recoverySafe else {
            let message = MopeliumLocalization.string(
    ...
    private func bootstrapMainAgentIfNeeded(existingProjection projection: CoworkProjection,
        ...
        guard let url = retainWorkspaceAccess(forPath: workspace.path) else {
            ...
            composerError = MopeliumLocalization.format(
                ...
                mainID.rawValue)
            setPermissionReviewerStatus(.failed(
                composerError ?? MopeliumLocalization.string("Workspace access unavailable.")))
            return
        }
    ...
    func reauthorizePrimaryWorkspace() {
        guard acceptsNewOperations,
              ...
              let selected = WorkspaceAccess.choose(
                prompt: MopeliumLocalization.string("Reauthorize Primary Workspace")) else {
            return
        }
    ...
    func updateProjectSettings(_ settings: CoworkProjectSettings) async -> Bool {
        guard let operationID = beginDirectOperation() else {
            composerError = MopeliumLocalization.string(
    ...
    func removeAgent(name rawName: String) {
        ...
        let operation = Task { @MainActor [weak self] in
            ...
            await self.synchronizePermissionReviewerHealth(using: orchestrator)
            guard detached else {
                self.composerError = MopeliumLocalization.format(
    ...
    func removeWorkspace(path: String) {
        ...
        let operation = Task { @MainActor [weak self] in
            ...
            guard let configuredWorkspace = self.configuredWorkspace(matching: path) else {
                self.composerError = MopeliumLocalization.string(
    ...
    func addAgent(name rawName: String, workspace authorization: WorkspaceAccessLease) {
        ...
        guard let binding = projectSettings.defaultInferenceProfileBinding else {
            addAgentStatus = .failed(MopeliumLocalization.string(
        ...
        do {
            ...
            addAgentStatus = .failed(MopeliumLocalization.format(
                "Workspace access could not be saved: %@",
                error.localizedDescription))
            authorization.release()
            return
    ...
    func importDraftAttachments(_ urls: [URL]) {
        ...
        let operation = Task { @MainActor [weak self] in
            ...
            for url in urls {
                do {
                    let file = try MopeliumComposerAttachmentFileReader.read(url)
    ...
    private static func presentationMessage(
        ...
        switch error {
        case .empty:
            return MopeliumLocalization.string("Enter a message.")
        case .missingGoal:
            return MopeliumLocalization.string("Enter a goal after /goal.")
        }
    }
    ...
    private static func presentationMessage(
        ...
        case .emptyMessage:
            return MopeliumLocalization.string("Enter a message before sending.")
        case .emptyMention:
            return MopeliumLocalization.string("Type an agent name after @.")
    ...
    private func attachFailureStatus(agentName: String, events: [Envelope]) -> CoworkAddAgentStatus {
        ...
        return .failed(MopeliumLocalization.format(
            "Could not attach @%@.",
            agentName))
    }
    ...
    private static func makeProjectInfo(sessionID: SessionID,
        ...
        return CoworkProjectInfo(
            ...
            tokenBudget: settings.tokenBudget.map {
                MopeliumLocalization.format("%@ tok", formatNumber($0))
            },
    ...
    private static func defaultModelDescription(_ settings: CoworkProjectSettings) -> String {
        ...
        guard let model, !model.isEmpty else {
            return MopeliumLocalization.string("current model")
        }
    ...
    private static func permissionDescription(_ rawValue: String) -> String {
        ...
        case .some(.manual): return MopeliumLocalization.string("manual")
        case .some(.reviewed): return MopeliumLocalization.string("reviewed")
        case .some(.autopilot): return MopeliumLocalization.string("autopilot")
        case .some(.readOnly): return MopeliumLocalization.string("read only")
        case .some(.locked): return MopeliumLocalization.string("locked")
...
enum CoworkAddAgentStatus: Equatable {
    ...
    var message: String? {
        ...
        case .idle:
            return nil
        case .validating:
            return MopeliumLocalization.string("Validating agent…")
        case .attaching(let name):
            return MopeliumLocalization.format("Attaching @%@…", name)
        case .attached(let name):
            return MopeliumLocalization.format("@%@ attached.", name)
        case .denied(let reason):
            return MopeliumLocalization.format("Permission denied: %@", reason)
        case .failed(let message):
            return message
        }
    }
