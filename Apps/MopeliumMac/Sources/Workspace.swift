...
/// Workspace folder selection. Access is held through a user-selected
/// security-scoped resource independently of distribution packaging.
...
enum WorkspaceAccess {
    ...
    static func remember(_ url: URL,
        ...
        return try SessionWorkspaceAccessStore.upsert(
            ...
            entry: SessionWorkspaceAccessEntry(
                ...
                isPrimary: isPrimary))
        #else
        throw MopeliumError.io("Security-scoped workspace bookmarks are unavailable on this platform.")
        #endif
    }
    ...
    static func restoredWorkspace(for session: SessionID) throws -> WorkspaceAccessLease? {
        ...
        do {
            ...
            guard let verified = try SessionWorkspaceAccessStore.entry(
                ...
                throw MopeliumError.io("Migrated workspace access could not be verified safely.")
            }
    ...
    static func migrateLegacyBookmarks(
        ...
        for rawPath in orderedWorkspacePaths {
            ...
            guard let legacyLease = resolveLegacyLease(
                ...
                throw MopeliumError.io("Legacy workspace access is invalid or points to another directory.")
            }
        ...
        guard completedLegacyPaths == requiredLegacyPaths else {
            throw MopeliumError.io("Not every session-owned legacy workspace capability was migrated.")
        }
        ...
        guard sourceBackedPaths.isSubset(of: verifiedPaths) else {
            throw MopeliumError.io("Legacy workspace access could not be verified in session storage.")
        }
        guard sourceBackedPaths.allSatisfy({ path in
            ...
            throw MopeliumError.io("Migrated workspace access could not be resolved safely.")
        }
    ...
    private static let bookmarkStoreKey = "mopelium.workspace.bookmarks"

    private static func sessionBookmarkKey(_ session: SessionID) -> String {
        "mopelium.workspace.sessionBookmark.\(session.rawValue)"
    }

    private static func sessionPathKey(_ session: SessionID) -> String {
        "mopelium.workspace.sessionPath.\(session.rawValue)"
    }
