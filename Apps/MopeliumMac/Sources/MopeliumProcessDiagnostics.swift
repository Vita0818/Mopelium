...
@MainActor
final class MopeliumMacProcessDiagnostics {
    static let shared = MopeliumMacProcessDiagnostics()
    ...
    private let driver: MopeliumMainThreadHeartbeatDriver
    ...
    private init() {
        ...
        if let root = try? MopeliumHangDiagnosticBundleStore.defaultRootURL() {
            store = MopeliumHangDiagnosticBundleStore(rootURL: root)
        } else {
            store = nil
        }
        driver = MopeliumMainThreadHeartbeatDriver(
            store: store,
            applicationVersion:
                Bundle.main.object(
...
private final class MopeliumMainThreadHeartbeatDriver: @unchecked Sendable {
    private let lock = NSLock()
    private let queue = DispatchQueue(
        label: "com.Vita0818.Mopelium.main-thread-heartbeat",
        qos: .utility)
    private let diagnostics = MopeliumPerformanceDiagnostics.shared
    private let store: MopeliumHangDiagnosticBundleStore?
    private let applicationVersion: String?
    private let buildVersion: String?

    private var state = MopeliumMainThreadHeartbeatStateMachine()
    private var timer: DispatchSourceTimer?
    private var mainPing: (@Sendable (UInt64) -> Void)?
    private var started = false

    init(
        store: MopeliumHangDiagnosticBundleStore?,
    ...
    func start(mainPing: @escaping @Sendable (UInt64) -> Void) {
        ...
        timer.schedule(
            ...
            repeating: .milliseconds(
                Int(MopeliumDiagnosticConstants.heartbeatTickMilliseconds)),
