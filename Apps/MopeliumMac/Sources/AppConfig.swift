...
enum AppConfig {
    ...
    /// The sole macOS product is the Developer ID workbench profile.
    ...
    static var editableConfigDescription: String {
        if canonicalConfigOverridePath() != nil {
            return preferredWritableConfigFileURL().path
        }
        if let existing = existingConfigFileURL(),
           isModernConfigFile(existing) {
    ...
    static func prepareEditableConfigFile() throws -> URL {
        if canonicalConfigOverridePath() != nil {
            let preferred = preferredWritableConfigFileURL()
    ...
    private static func configFileCandidates() -> [URL] {
        ...
        return [
            ...
            legacyUserConfigDir.appendingPathComponent("intatis.json"),
            legacyUserConfigDir.appendingPathComponent("intatis.jsonc"),
            legacyUserConfigDir.appendingPathComponent("config.json"),
            legacyUserConfigDir.appendingPathComponent("config.jsonc"),
    ...
    private static func configOverridePath() -> String? {
        canonicalConfigOverridePath() ?? legacyConfigOverridePath()
    }

    private static func canonicalConfigOverridePath() -> String? {
        nonemptyEnvironmentValue(configEnvKey)
    }

    private static func legacyConfigOverridePath() -> String? {
        nonemptyEnvironmentValue(legacyConfigEnvKey)
    }
    ...
    private static func preferredWritableConfigFileURL() -> URL {
        if let override = canonicalConfigOverridePath() {
            return URL(fileURLWithPath: expandedPath(override))
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/mopelium/mopelium.json")
    ...
    private static func editableConfigFileURL() -> URL {
        if canonicalConfigOverridePath() != nil {
            return preferredWritableConfigFileURL()
        }
        if let existing = existingConfigFileURL(),
           isModernConfigFile(existing) {
            return existing
...
private struct AppProviderConfigFile: Decodable {
    ...
    func catalog(
        ...
        // interpreting the first path component as an Mopelium provider ID.
