...
final class AppMCPService: ObservableObject {
    ...
    /// not by Settings. Restricted hosts must pass `nil`.
...
struct MopeliumMCPSettingsView: View {
        ...
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                if env.mcp.hostProfile.supportsStdio {
                    ...
                    Text(
                        "Streamable HTTP only; this restricted host does not link the stdio transport")
                        .font(MopeliumType.caption(12, .regular))
                        .foregroundStyle(.secondary)
                }
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            case (.macAppStore, .stdio):
                Label(
                    "Managed stdio is not linked into this restricted host. Use an HTTPS Streamable HTTP endpoint.",
            ...
            case (.macAppStore, .streamableHTTP):
                ...
                Text(
                    "This restricted host is remote-only by linkage and cannot launch a local MCP executable.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            ...
            case (.linuxCLI, .stdio):
                ...
                guidanceStep(
                    2,
                    "Run mopelium mcp doctor and resolve every bwrap or launch-artifact error.")
                guidanceStep(
                    3,
                    "Add the exact executable and launch closure, then run mopelium mcp test before attaching it.")
                Text(
...
private struct MCPServerEditorSheet: View {
    ...
    private var stdioForm: some View {
        ...
        Section("Network") {
            ...
            Text(
                "Managed stdio remains inside the permission, workspace, sandbox, and durable execution boundaries. It is absent from restricted hosts.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
