...
struct MopeliumDiagnosticExportResult: Sendable {
    let archiveFileName: String
    let archiveByteCount: Int
    let collectionErrorCount: Int
}

enum MopeliumDiagnosticExportError: Error, LocalizedError {
...
@MainActor
enum MopeliumDiagnosticExportService {
    static func suggestedArchiveName(now: Date = Date()) -> String {
        "Mopelium-Diagnostics-\(fileTimestamp(now)).zip"
    }

    static func export(to destinationURL: URL) async throws
        -> MopeliumDiagnosticExportResult
    {
        ...
        let library = try FileManager.default.url(
            ...
            appropriateFor: nil,
            create: true)
        let hangRoot = try? MopeliumHangDiagnosticBundleStore.defaultRootURL()
        let context = MopeliumDiagnosticExportContext(
            generatedAt: Date(),
            ...
            rendererMode: MopeliumMessageRendererMode.resolve(
            ...
            performanceMetrics: try? JSONEncoder().encode(
                MopeliumPerformanceDiagnostics.shared.snapshot()))

        let work = Task.detached(priority: .utility) {
            try MopeliumDiagnosticBundleBuilder(context: context).build()
        }
...
private struct MopeliumDiagnosticExportContext: Sendable {
...
private struct MopeliumDiagnosticExportManifest: Codable {
...
private struct MopeliumDiagnosticBundleBuilder {
    ...
    func build() throws -> MopeliumDiagnosticExportResult {
        try Task.checkCancellation()
        let temporaryRoot = fileManager.temporaryDirectory.appendingPathComponent(
            "Mopelium-Diagnostic-\(UUID().uuidString)",
            isDirectory: true)
        try createOwnerOnlyDirectory(temporaryRoot)
        defer { try? fileManager.removeItem(at: temporaryRoot) }

        let bundleName = "Mopelium-Diagnostics-\(Self.fileTimestamp(context.generatedAt))"
        ...
        let archiveURL = temporaryRoot.appendingPathComponent(
            "\(bundleName).zip")
        let execution = try MopeliumDiagnosticFixedProcessRunner.run(
    ...
    private func makeManifest() -> MopeliumDiagnosticExportManifest {
        MopeliumDiagnosticExportManifest(
            schemaVersion: 1,
    ...
    private func collectSessions(
        bundleRoot: URL,
        manifest: inout MopeliumDiagnosticExportManifest
    ) {
        ...
        for (directory, _, kind) in candidates.prefix(Self.maximumSessionCount) {
            ...
            do {
                let snapshot = try MopeliumDiagnosticSnapshotReader.readTail(
                    from: events,
                    maximumBytes: min(
                        Self.maximumEventBytesPerSession,
                        remainingInputBytes))
                remainingInputBytes -= snapshot.data.count
                let redacted = MopeliumDiagnosticEventLogRedactor.redact(snapshot)
    ...
    private func collectHangDiagnostics(
        ...
        guard let root = context.hangDiagnosticsRoot else {
            manifest.collectionErrors.append(.init(
                ...
                message: "The Mopelium hang diagnostics directory could not be resolved."))
            return
        }
        let keys: Set<URLResourceKey> = [
            .contentModificationDateKey,
    ...
    private func collectCrashReports(
        ...
        for root in context.crashReportRoots {
            ...
            reports.append(contentsOf: urls.compactMap { url in
                ...
                guard (name.hasPrefix("MopeliumMac") || name.hasPrefix("Mopelium")),
    ...
    private func writeOwnerOnly(
        ...
        guard !components.isEmpty,
            ...
            throw MopeliumDiagnosticExportError.couldNotPrepare
        }
    ...
    private func safeIdentifier(_ value: String) -> String {
        MopeliumHangDiagnosticTextSanitizer.sanitize(
            value,
            sensitivePaths: sensitivePaths,
            maximumBytes: 512)
    }
    ...
    Mopelium local diagnostic bundle / Mopelium 本地诊断包

    This ZIP was generated only after the user selected Export Diagnostic Logs.
    It was saved to the selected local destination. Mopelium did not upload it.
    ...
    - Mopelium process Unified Log entries from the last 24 hours
    - current macOS proxy configuration
    - performance counters and retained hang diagnostics
    - recent Mopelium crash reports
...
private struct MopeliumDiagnosticProcessExecution: Sendable {
...
private enum MopeliumDiagnosticFixedProcessRunner {
    static func run(
        executableURL: URL,
        arguments: [String],
        timeoutSeconds: TimeInterval,
        maximumOutputBytes: Int
    ) throws -> MopeliumDiagnosticProcessExecution {
        try Task.checkCancellation()
        let process = Process()
        ...
        let output = MopeliumDiagnosticBoundedDataBuffer(
            maximumBytes: maximumOutputBytes)
        let errors = MopeliumDiagnosticBoundedDataBuffer(
        ...
        return MopeliumDiagnosticProcessExecution(
    ...
    private static func drain(
        _ handle: FileHandle,
        into buffer: MopeliumDiagnosticBoundedDataBuffer
    ) {
        while true {
            do {
...
private final class MopeliumDiagnosticBoundedDataBuffer: @unchecked Sendable {
    struct Snapshot {
        let data: Data
        let truncated: Bool
    }
