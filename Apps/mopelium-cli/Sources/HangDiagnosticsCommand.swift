...
enum CLIDiagnoseHangError: Error, LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .usage:
            return "usage: mopelium diagnose-hang --pid <pid> [--output <directory>]"
        case .invalidProcessIdentifier:
        ...
        case .targetIsNotMopelium:
            return "the target process is not the Mopelium macOS application"
        case .sampleFailed:
...
struct CLIMopeliumProcessIdentity: Equatable, Sendable {
    let processIdentifier: Int32
...
protocol CLIMopeliumProcessInspecting: Sendable {
    func inspect(processIdentifier: Int32) throws -> CLIMopeliumProcessIdentity
}

enum CLIMopeliumProcessValidator {
    static func validate(
        _ identity: CLIMopeliumProcessIdentity,
        ...
        guard identity.executableName == "MopeliumMac",
              identity.bundleIdentifier == "com.Vita0818.Mopelium" else {
            throw CLIDiagnoseHangError.targetIsNotMopelium
        }
    }
}
...
func runDiagnoseHangCommand(
    ...
    #if os(macOS)
    let report = try await captureMopeliumHang(
        options: options,
        inspector: CLIMacMopeliumProcessInspector(),
...
func captureMopeliumHang(
    options: CLIDiagnoseHangOptions,
    inspector: any CLIMopeliumProcessInspecting,
    ...
    try CLIMopeliumProcessValidator.validate(
        identity,
    ...
    if let defaultRootURL {
        ...
        resolvedDefaultRoot =
            try MopeliumHangDiagnosticBundleStore.defaultRootURL()
    }
    let recentStore = MopeliumHangDiagnosticBundleStore(
    ...
    let unifiedLog = runHangCapture(
        ...
        arguments: [
            ...
            "compact",
            "--predicate",
            "processIdentifier == \(options.processIdentifier) AND subsystem == \"\(MopeliumDiagnosticConstants.subsystem)\"",
        ],
        timeoutSeconds: 15)
    ...
    if let outputParentURL = options.outputParentURL {
        outputRoot = outputParentURL.appendingPathComponent(
            "Mopelium-HangDiagnostics",
            isDirectory: true)
    } else {
        outputRoot = resolvedDefaultRoot
    }
    let store = MopeliumHangDiagnosticBundleStore(rootURL: outputRoot)
    let sensitivePaths = [
        outputRoot.path,
        options.outputParentURL?.path,
    ].compactMap { $0 }
    var attachments: [MopeliumHangDiagnosticAttachment] = []
    ...
    let manifest = MopeliumHangDiagnosticManifest(
...
#if os(macOS)
private struct CLIMacMopeliumProcessInspector: CLIMopeliumProcessInspecting {
    func inspect(
        processIdentifier: Int32
    ) throws -> CLIMopeliumProcessIdentity {
        ...
        return CLIMopeliumProcessIdentity(
            processIdentifier: processIdentifier,
