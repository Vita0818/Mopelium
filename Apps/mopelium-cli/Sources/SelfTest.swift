...
import MopeliumCowork

// Built-in fake models — let `mopelium selftest` prove the chat + code paths work
...
private func tempLog(_ tag: String) throws -> EventLog {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("mopelium-\(tag)-\(UUID().uuidString)", isDirectory: true)
...
func runSelfTest() async throws {
    ...
    let writeArgs = String(decoding: try JSONSerialization.data(
        withJSONObject: ["path": "note.txt", "content": "hello from mopelium"]), as: UTF8.self)
    ...
    let okCode = onDisk == "hello from mopelium"
    ...
    let profileDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent("mopelium-profile-selftest-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: profileDirectory, withIntermediateDirectories: true)
    let catalogURL = profileDirectory.appendingPathComponent("catalog.json")
    let configURL = profileDirectory.appendingPathComponent("mopelium.json")
    let routeAEnvironmentName = "MOPELIUM_SELFTEST_ROUTE_A_API_KEY"
    let routeBEnvironmentName = "MOPELIUM_SELFTEST_ROUTE_B_API_KEY"
    let routeAFixtureValue = "offline-route-a-fixture"
    let routeBFixtureValue = "offline-route-b-fixture"
            ...
            "provider": [
                "route-a": [
                    ...
                    "options": [
                        ...
                        "apiKey": "{env:\(routeAEnvironmentName)}",
                    ],
                    "models": [
                        "model-alpha": [
                            "name": "Alpha",
                ...
                "route-b": [
                    ...
                    "options": [
                        ...
                        "apiKey": "{env:\(routeBEnvironmentName)}",
                    ],
                    "models": [
                        "model-beta": [
                            "name": "Beta",
    ...
    let offlineEnvironment = [
        "MOPELIUM_MODE": "cowork",
        "MOPELIUM_USAGE": "0",
        "MOPELIUM_MAX_STEPS": "3",
        "MOPELIUM_REASONING": "low",
        routeAEnvironmentName: routeAFixtureValue,
        routeBEnvironmentName: routeBFixtureValue,
    ]
    ...
    uniqueUnqualifiedEnvironment["MOPELIUM_MODEL"] = "model-beta"
    uniqueUnqualifiedEnvironment["MOPELIUM_REASONING"] = "high"
    ...
    slashModelEnvironment.removeValue(forKey: "MOPELIUM_REASONING")
    slashModelEnvironment["MOPELIUM_MODEL"] = "anthropic/claude-sonnet"
    ...
    slashTopLevelEnvironment.removeValue(forKey: "MOPELIUM_REASONING")
    ...
    unavailableReasoningEnvironment["MOPELIUM_MODEL"] = "route-a/model-alpha"
    unavailableReasoningEnvironment["MOPELIUM_REASONING"] = "minimal"
    ...
    invalidReasoningEnvironment["MOPELIUM_REASONING"] = "not-a-reasoning-effort"
    ...
    guard let firstRouteA = firstProfiles.option(
        ...
        throw MopeliumError.config("offline profile fixture did not compile")
    }
    ...
    secondEnvironment["MOPELIUM_REASONING"] = "high"
    ...
    guard let currentRouteA = secondProfiles.option(
          ...
          let currentRouteB = secondProfiles.option(
            ...
            model: "model-beta",
            variantID: "deep") else {
        throw MopeliumError.config("offline multi-route profiles did not compile")
    }

    let exactRegistry = ProviderRegistry(
        config: secondConfig.providerConfig(),
        resolver: CLIExactSecretResolver(
            config: secondConfig,
            environment: secondEnvironment),
    ...
    let credentialsUseExactEnvironmentReferences =
        oldDefinition.connection.credentialRef.source == .environment
            && oldDefinition.connection.credentialRef.account == routeAEnvironmentName
            && currentADefinition.connection.credentialRef.source == .environment
            && currentADefinition.connection.credentialRef.account == routeAEnvironmentName
            && currentBDefinition.connection.credentialRef.source == .environment
            && currentBDefinition.connection.credentialRef.account == routeBEnvironmentName
    ...
    let exactResolver = CLIExactSecretResolver(
        config: secondConfig,
        environment: secondEnvironment)
    ...
    let credentialsStayRouteScoped = recoveredOldRouteAKey == routeAFixtureValue
        && exactRouteBKey == routeBFixtureValue
    ...
    let bindingIsRouteSafe = !bindingJSON.contains("127.0.0.1")
        ...
        && !bindingJSON.contains(routeAEnvironmentName)
        && !bindingJSON.contains(routeBEnvironmentName)
        && !bindingJSON.contains(routeAFixtureValue)
        && !bindingJSON.contains(routeBFixtureValue)
    ...
    let eventLogIsRouteSafe = !eventJSON.contains("127.0.0.1")
        ...
        && !eventJSON.contains(routeAEnvironmentName)
        && !eventJSON.contains(routeBEnvironmentName)
        && !eventJSON.contains(routeAFixtureValue)
        && !eventJSON.contains(routeBFixtureValue)
    ...
    let okProfiles = profileIDsStable
        ...
        && credentialsUseExactEnvironmentReferences
