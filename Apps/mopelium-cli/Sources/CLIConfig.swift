...
enum ConfigFile {
    ...
    static func read() -> [String: String] {
        let candidate = FileManager.default.fileExists(atPath: url.path)
            ? url : legacyURL
        guard let data = try? Data(contentsOf: candidate),
              let object = (try? JSONSerialization.jsonObject(
                with: data)) as? [String: String] else {
            return [:]
        }
        return object
    }
...
struct CLIConfig {
    ...
    /// The explicitly selected Mopelium config is retained only so a lazy
    ...
    static func load() throws -> CLIConfig {
        ...
        func value(_ envKey: String, _ fileKey: String, fallback: String?) -> String? {
            if let e = hostEnvironmentValue(envKey, environment: env) {
                return e
            }
        ...
        guard let apiKey = value("MOPELIUM_API_KEY", "apiKey", fallback: nil), !apiKey.isEmpty else {
            throw MopeliumError.config("no API key — run `mopelium settings`, or set MOPELIUM_API_KEY")
        }
    ...
    static func load(configurationFileURL: URL,
        ...
        if let baseOverride = hostEnvironmentValue(
            "MOPELIUM_BASE_URL",
            environment: environment) {
        ...
        let selectedInlineKey = hostEnvironmentValue(
            "MOPELIUM_API_KEY",
            environment: environment)
    ...
    private static func hostEnvironmentValue(
        _ canonicalKey: String,
        environment: [String: String]
    ) -> String? {
        if let canonical = environment[canonicalKey]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !canonical.isEmpty {
            return canonical
        }
        guard canonicalKey.hasPrefix("MOPELIUM_") else { return nil }
        let suffix = canonicalKey.dropFirst("MOPELIUM_".count)
        let legacyKey = "INTATIS_\(suffix)"
        guard let legacy = environment[legacyKey]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !legacy.isEmpty else {
            return nil
        }
