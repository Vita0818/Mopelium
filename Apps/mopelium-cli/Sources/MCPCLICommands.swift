...
import Crypto
#else
#error("MopeliumCLI requires CryptoKit or swift-crypto")
#endif
import Foundation
...
enum MCPCLIError:
    ...
    var errorDescription: String? {
        switch self {
        case .missingSubcommand:
            return "missing MCP subcommand; run `mopelium mcp help`"
...
func printMCPHelp() {
    out("""
    Mopelium external MCP client

    INVENTORY
      mopelium mcp list [--json]
      mopelium mcp get --server <alias|id> [--json]
      mopelium mcp add --alias <id> --name <name> (--url <https> | --command </absolute/path>) [configuration options] [--yes] [--json]
      mopelium mcp edit --server <alias|id> [same mutable fields]
      mopelium mcp duplicate --server <alias|id> --alias <new>
      mopelium mcp enable|disable --server <alias|id>
      mopelium mcp remove --server <alias|id> --yes
      mopelium mcp test --server <alias|id>
      mopelium mcp doctor [--json]
      mopelium mcp import --file <explicit.json> [--format mcp-json|claude-json] [explicit launch-closure flags] [--replace-conflicts] [--allow-insecure-loopback-development-http]
      mopelium mcp export [--file <path>] [--include-disabled]

    SESSION AUTHORITY
      mopelium mcp attach --session <id> --server <alias|id> [--required] [--agent <id>]
      mopelium mcp detach --session <id> --server <alias|id>
      mopelium mcp approval set --session <id> --server <alias|id> [--approval <mode>] [--required|--optional] [--parallel|--serial]
      mopelium mcp grant --session <id> --server <alias|id> --agent <id> [--task <id>] [--capabilities tools,resources,prompts,...]
      mopelium mcp revoke --session <id> --grant <id>
      mopelium mcp auth status --server <alias|id>
      mopelium mcp auth login --server <alias|id> [--allow-dynamic-registration] [--no-open] [--yes]
      mopelium mcp auth logout --server <alias|id> [--yes]

    LIVE SESSION OWNER
      mopelium mcp status [--session <id>] [--agent <id>] [--json]
      mopelium mcp connect --session <id> --agent <id> [--task <id>] [--server <alias|id>] --yes
      mopelium mcp inspect|tools|resources|prompts --session <id> --agent <id> [--task <id>] [--server <alias|id>] --yes
      mopelium mcp refresh|disconnect --session <id> --agent <id> [--task <id>] [--server <alias|id>]
      mopelium mcp reload [--json]
