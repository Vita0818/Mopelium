...
struct CLIModernProviderConfig: Sendable {
    ...
    static func existingURL(environment: [String: String]) -> URL? {
        ...
        if let legacyOverride = environment[
        ...
        candidates.append(contentsOf: [
            legacyConfig.appendingPathComponent("intatis.json"),
            legacyConfig.appendingPathComponent("intatis.jsonc"),
        ...
        if let support = FileManager.default.urls(
            ...
            let legacyDirectory = support.appendingPathComponent(
                "Intatis",
                isDirectory: true)
            candidates.append(legacyDirectory.appendingPathComponent("intatis.json"))
            candidates.append(legacyDirectory.appendingPathComponent("intatis.jsonc"))
        }
    ...
    static func load(from url: URL,
        ...
        if permissionReviewerFieldPresent {
            ...
            // configured default, not to MOPELIUM_MODEL or a runtime @main
    ...
    private static func selectModel(
        ...
        if let raw = raw?.trimmingCharacters(in: .whitespacesAndNewlines), !raw.isEmpty {
            ...
            // Mopelium' provider/model shorthand.
        ...
        for route in routes.sorted(by: { $0.id.count > $1.id.count }) {
            ...
            if raw.hasPrefix(prefix) {
                ...
                guard !modelID.isEmpty else {
                    throw MopeliumError.config("invalid CLI \(roleName) model")
                }
                return CLIProviderModelSelection(
                    providerID: route.id,
                    modelID: modelID)
...
struct CLIExactSecretResolver: SecretResolver {
    ...
    private func readAuthSecret(providerID: String) -> String? {
        ...
        var urls: [URL] = []
        if let override = environment["MOPELIUM_AUTH_FILE"]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !override.isEmpty {
            urls = [URL(fileURLWithPath: expandedPath(override))]
        } else if let legacyOverride = environment[
            MopeliumProductIdentity.Legacy.authorizationFileEnvironmentKey]?
            .trimmingCharacters(in: .whitespacesAndNewlines),
                  !legacyOverride.isEmpty {
            urls = [URL(fileURLWithPath: expandedPath(legacyOverride))]
        } else {
            let home = FileManager.default.homeDirectoryForCurrentUser
            urls = [
                home.appendingPathComponent(".config/mopelium/auth.json"),
