...
final class CLIProviderAdapterTests: XCTestCase {
    func testPermissionReviewerFallsBackToJSONModelNotEnvironmentMainOverride()
        ...
        let directory = try makeTemporaryDirectory(
            prefix: "mopelium-cli-reviewer-fallback")
        ...
        let config = try CLIConfig.load(
            ...
            environment: ["MOPELIUM_MODEL": "runtime/runtime-model"])
    ...
    func testExplicitPermissionReviewerModelMustResolveCanonicalConfiguredModel()
        throws {
        let directory = try makeTemporaryDirectory(
            prefix: "mopelium-cli-reviewer-invalid")
        defer { try? FileManager.default.removeItem(at: directory) }
        let file = directory.appendingPathComponent("mopelium.json")
    ...
    func testMissingReviewerCannotInventAnUnavailableJSONDefaultModel()
        throws {
        let directory = try makeTemporaryDirectory(
            prefix: "mopelium-cli-reviewer-missing-default")
        defer { try? FileManager.default.removeItem(at: directory) }
        let file = directory.appendingPathComponent("mopelium.json")
    ...
    func testCoworkBootstrapKeepsConfiguredReviewerIndependentOfMainRebind()
        async throws {
        let directory = try makeTemporaryDirectory(
            prefix: "mopelium-cli-reviewer-runtime")
        defer { try? FileManager.default.removeItem(at: directory) }
        let file = directory.appendingPathComponent("mopelium.json")
        try writeProviderConfig([
            "model": "main/main-model",
    ...
    func testOpenRouterKnowledgeRoleModelsStayOutOfInferenceProfiles()
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-cli-openrouter-knowledge-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(
            at: directory,
    ...
    func testKnowledgeRoleOnlyRoutesComposeStrictToolsWithoutResolvingSecrets()
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-cli-knowledge-models-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(
            at: directory,
            ...
            "provider": [
                "chat": [
                    "options": [
                        ...
                        "apiKey": "{env:MOPELIUM_TEST_CHAT_KEY}",
                    ],
                ...
                "embedding": [
                    ...
                    "options": [
                        ...
                        "apiKey": "{env:MOPELIUM_TEST_EMBEDDING_KEY}",
                    ],
                    "models": [String: Any](),
                ],
                "reranker": [
                    "npm": "mopelium:cohere-v2",
                    "options": [
                        "baseURL": "https://reranker.example.invalid/v2",
                        "apiKey": "{env:MOPELIUM_TEST_RERANKER_KEY}",
                    ],
                    "models": [String: Any](),
                ],
            ],
    ...
    func testKnowledgeRolesRejectImplicitSelectedProviderFallback() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-cli-invalid-knowledge-route-\(UUID().uuidString)",
                isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        try FileManager.default.createDirectory(
            at: directory,
    ...
    func testUnsupportedRerankerAdapterIsNotAdvertisedAsReady() throws {
        ...
        XCTAssertTrue(notice.contains("explicit mopelium:siliconflow-v1"))
    ...
    func testCLIExternalKnowledgeCreationTouchesOnlyExactLeaf() throws {
        let parent = FileManager.default.temporaryDirectory
            .appendingPathComponent(
                "mopelium-cli-exact-knowledge-root-\(UUID().uuidString)",
                isDirectory: true)
    ...
    func testModernConfigRoutesDedicatedImageModelWithoutSelectingItForChat()
        ...
        let directory = FileManager.default
            .temporaryDirectory
            .appendingPathComponent(
                "mopelium-cli-image-model-\(UUID().uuidString)",
                isDirectory: true)
        defer {
            ...
            "provider": [
                ...
                "images": [
                    "options": [
                        ...
                        "apiKey": "{env:MOPELIUM_TEST_IMAGE_KEY}",
                    ],
    ...
    func testModernConfigWithoutImageModelHasNoHiddenFallback()
        ...
        let directory = FileManager.default
            .temporaryDirectory
            .appendingPathComponent(
                "mopelium-cli-no-image-model-\(UUID().uuidString)",
                isDirectory: true)
        defer {
            try? FileManager.default.removeItem(
                at: directory)
    ...
    func testModernConfigPreservesProviderAndModelNPMAdapters()
        ...
        let directory = FileManager.default
            .temporaryDirectory
            .appendingPathComponent(
                "mopelium-cli-provider-adapter-\(UUID().uuidString)",
                isDirectory: true)
        defer {
            try? FileManager.default.removeItem(
                at: directory)
