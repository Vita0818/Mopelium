...
final class HangDiagnosticsCommandTests: XCTestCase {
    func testParserRequiresExactPIDAndResolvesRelativeOutput() throws {
        let current = URL(fileURLWithPath: "/tmp/mopelium-diagnostics-test")
    ...
    func testProcessValidationRequiresCurrentUserExactExecutableAndBundle() {
        let valid = CLIMopeliumProcessIdentity(
            processIdentifier: 42,
            ownerUserIdentifier: 501,
            executableName: "MopeliumMac",
            bundleIdentifier: "com.Vita0818.Mopelium")
        XCTAssertNoThrow(try CLIMopeliumProcessValidator.validate(
            valid,
            currentUserIdentifier: 501))

        XCTAssertThrowsError(try CLIMopeliumProcessValidator.validate(
            .init(
                processIdentifier: 42,
        ...
        XCTAssertThrowsError(try CLIMopeliumProcessValidator.validate(
            .init(
                processIdentifier: 42,
                ownerUserIdentifier: 501,
                executableName: "MopeliumMac-copy",
                bundleIdentifier: "com.Vita0818.Mopelium"),
            currentUserIdentifier: 501)) {
                XCTAssertEqual(
                    $0 as? CLIDiagnoseHangError,
        ...
        XCTAssertThrowsError(try CLIMopeliumProcessValidator.validate(
            .init(
                ...
                ownerUserIdentifier: 501,
                executableName: "MopeliumMac",
                bundleIdentifier: "example.not-mopelium"),
            currentUserIdentifier: 501)) {
                XCTAssertEqual(
                    $0 as? CLIDiagnoseHangError,
                    .targetIsNotMopelium)
    ...
    func testCaptureUsesExactSampleAndFiveMinuteLogAndSanitizesOutput()
        ...
        _ = try await recentStore.writeBundle(
            manifest: .init(
                ...
                metrics: .init(counters: [
                    MopeliumDiagnosticCounter.mainThreadIncidents.rawValue: 1,
                ])))
        ...
        XCTAssertEqual(
            ...
            [
                ...
                "compact",
                "--predicate",
                "processIdentifier == 4321 AND subsystem == \"com.Vita0818.Mopelium\"",
            ])

        let bundle = explicitParent
            .appendingPathComponent("Mopelium-HangDiagnostics")
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-cli-hang-tests-\(UUID().uuidString)",
            isDirectory: true)
...
private struct FixedProcessInspector: CLIMopeliumProcessInspecting {
    let identity: CLIMopeliumProcessIdentity
    ...
    func run(
        ...
        if executableURL.lastPathComponent == "sample" {
            ...
            return CLIHangCaptureExecution(
                ...
                standardOutput: Data("""
                Path: /Users/example/Private/MopeliumMac
