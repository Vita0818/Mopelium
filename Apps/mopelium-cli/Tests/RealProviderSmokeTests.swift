...
final class RealProviderSmokeTests: XCTestCase {
    func testRealKnowledgeRerankQualityWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "MOPELIUM_REAL_KNOWLEDGE_QUALITY"
        ] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_KNOWLEDGE_QUALITY=1 to run the configured embedding and semantic reranker over the frozen mixed-language quality set.")
        }
    ...
    func testRealKnowledgeEmbeddingAndRerankerWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "MOPELIUM_REAL_KNOWLEDGE_SMOKE"
        ] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_KNOWLEDGE_SMOKE=1 to spend one embedding request and one reranker request using the exact configured Knowledge routes.")
        }
        ...
        let embeddingResponse = try await models.embedding.embedWithUsage(
            ["Mopelium knowledge route smoke test."],
            instruction: models.embedding.configuration.documentInstruction)
    ...
    func testRealModelBuildsSearchesAndCitesExternalKnowledgeWhenEnabled()
        async throws {
        guard ProcessInfo.processInfo.environment[
            "MOPELIUM_REAL_KNOWLEDGE_AGENT_E2E"
        ] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_KNOWLEDGE_AGENT_E2E=1 to let the configured Agent, embedding, and reranker routes perform the full read-organize-build-search-cite flow. This can make multiple billable requests.")
        }

        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "mopelium-real-knowledge-agent-\(UUID().uuidString)",
            isDirectory: true)
        ...
        let source = """
        Mopelium has three permission layers. DeterministicPolicyGate applies
        ...
        let baseRegistry = ToolRegistry(
            ...
            registryVersion: "mopelium.real-knowledge-agent-e2e/1")
        ...
        three Mopelium permission layers have? Cite only evidence returned by the
    ...
    func testRealModelBuildsSearchesAndCitesPDFKnowledgeWhenEnabled()
        ...
        guard environment["MOPELIUM_REAL_KNOWLEDGE_PDF_E2E"] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_KNOWLEDGE_PDF_E2E=1 and MOPELIUM_REAL_KNOWLEDGE_PDF_SOURCE to let the configured Agent read selected real PDFs, build, rerank, and cite. This can make multiple billable requests.")
        }
        guard let sourcePath = environment[
            "MOPELIUM_REAL_KNOWLEDGE_PDF_SOURCE"
        ], !sourcePath.isEmpty else {
            return XCTFail("MOPELIUM_REAL_KNOWLEDGE_PDF_SOURCE is required")
        }
        ...
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "mopelium-real-knowledge-pdf-\(UUID().uuidString)",
            isDirectory: true)
        let workspace = root.appendingPathComponent(
            "workspace",
            isDirectory: true)
        ...
        let baseRegistry = ToolRegistry(
            ...
            registryVersion: "mopelium.real-knowledge-pdf-e2e/1")
        let augmentation = try await augmenter.augment(
            HostToolRegistryAugmentationInput(
                sessionID: sessionID,
                agentID: agentID,
    ...
    {
        guard ProcessInfo.processInfo.environment[
            "MOPELIUM_REAL_MULTIMODAL_SMOKE"
        ] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_MULTIMODAL_SMOKE=1 to spend one real provider request containing both user and function-output images.")
        }
    ...
    func testConfiguredAgentRouteWithRealProvider() async throws {
        guard ProcessInfo.processInfo.environment["MOPELIUM_REAL_PROVIDER_SMOKE"] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_PROVIDER_SMOKE=1 to spend one minimal real provider request.")
        }
    ...
    func testRealAgentAuthorizationSidecarShapeWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "MOPELIUM_REAL_TOOL_SHAPE_DIAGNOSTIC"
        ] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_TOOL_SHAPE_DIAGNOSTIC=1 to verify that the configured Agent route emits a same-call authorization sidecar. This makes one billable request.")
        }
        ...
        func toolRequest() throws -> AgentRequest {
            ...
            return AgentRequest(
                ...
                messages: [.user("""
                    ...
                    __mopelium_authorization_context field, explain that this is a
    ...
    func testRealPermissionReviewControlPlaneWhenEnabled() async throws {
        guard ProcessInfo.processInfo.environment[
            "MOPELIUM_REAL_PERMISSION_REVIEW_SMOKE"
        ] == "1" else {
            throw XCTSkip(
                "Set MOPELIUM_REAL_PERMISSION_REVIEW_SMOKE=1 to spend one real permission-review request.")
        }
    ...
    private func makePermissionReviewRequest(
        ...
        let gate = PermissionReviewGateSnapshot(
            ...
            policyVersion: "mopelium.real-smoke.v1")
        ...
        let authorization = ResolvedToolAuthorization(
            ...
            registryVersion: "mopelium.real-smoke.v1",
            concreteToolID: "mopelium.standard/write_file",
